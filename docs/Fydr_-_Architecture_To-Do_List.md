...
- [ ] **One third of the reported schedule bug is still open: there is no per-session Save or Cancel, by design. (b) and (c) are both fixed.** The report was "no Save button, an unresponsive one, no Cancel option". (b), `handlePublish` with no `try/finally`, was real but latent and is fixed in `5be74e1`. (c), the Read-mode default, was what a coach actually hit and is fixed in `fea25a6` — coaches now enter in Edit. What remains is (a): for an existing committed session the panel offers only "Remove session" and "Duplicate", the commit is the week-level "Publish to athletes" and the revert is "Discard", and both are hidden until `dirtyCount > 0`. Making them visible-but-disabled would answer the complaint without touching the editing model. Still a design decision.
