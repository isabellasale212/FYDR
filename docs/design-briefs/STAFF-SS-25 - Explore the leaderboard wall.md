# Design brief — STAFF-SS-25, Explore the leaderboard wall
...
1. **Family chips keep their counts and render disabled at zero** — the
   document's reason stands: an unexplained absence is worse than a greyed
   option.
2. **The wall is readable by all staff; publishing controls only with
   `LEADERBOARD_EDIT`.**
3. **The lens becomes a real tablist or a chip row** (§0am — build work); a
   proposal must not keep three orphan tabs.

## 5. What a proposal should address

1. **6,592px at phone — the longest staff screen.** Lens, scope and family all
   scroll away above the boards they control.
2. **The zero-count state** was not observed on scratch; a proposal should draw
   it.
