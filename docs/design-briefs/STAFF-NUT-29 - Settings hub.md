# Design brief — STAFF-NUT-29, Settings hub
...
1. **Five rows for the nutritionist**, three of which lead to a refusal, a bounce or an empty page (Thresholds, Integrations, Notifications) — absent rows for absent content (§0av).
...
1. **A hub for a role with two live rows** (Password, Groups-read) on a 2,804px page.
