# Design brief — STAFF-SS-01, Read the dashboard
...
1. **The first screen answers the selection question** — who is out, who is
   doubtful, when is the match. Keep "Ready for {matchday}", the doubtful and
   ruled-out lines, and the six-day strip with MD-n above the fold at desktop.
2. **The three summary cards stay collapsible** with `aria-expanded`; their
   stat is their label.
3. **The group filter is global and a cookie** — a change here applies
   everywhere.

## 5. What a proposal should address

1. **Phone width** — see §0af; the dashboard's first content is at y=2209 at 375.
2. **"Compliance ›" and the flag links** at phone width sit below three
   collapsed cards and a six-day strip; the pitch-side question is "who is
   missing today", and it is several screens down.
