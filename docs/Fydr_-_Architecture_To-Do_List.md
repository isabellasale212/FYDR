...
- [ ] **The other two thirds of the reported schedule bug are NOT fixed, and (c) is the one that actually caused the report.** The report was "no Save button, an unresponsive one, no Cancel option". The `handlePublish` fix above was real but latent; (c) is what a coach actually hits.
...
- [ ] **Read mode still does not announce itself, and that is the remaining half.** The panel is a faithful read-only view but looks like an editable one that is ignoring you. Coaches no longer meet it, so this is no longer a workflow blocker — but anyone outside `SESSION_EDIT` lives in it permanently with no toggle and no label. A single line saying the schedule is read-only for their role would close it. Not built; not urgent.
...
- [ ] **Rehab groups fetches `restrictions` and never renders them, and the spec says it should. Display decision, yours.** `docs/screens/28-rehab-groups.md` line 30 says the coach sees "body area, restrictions and expected return"; `rehabGroups.ts` returns `restrictions: string[]`; `RehabGroupBoard` has **zero** references to `member.restrictions`. So the data crosses the wire to the browser and is dropped. Either the member row should render them — `AvailabilityList.tsx` already shows how, two labels plus a `+N` overflow — or the spec should stop promising them. Not a caption fix, which is why the caption now describes only what is drawn. Note the row does render `side`, which the spec does not mention either.

- [ ] **`teamAllocation.ts` is the ONLY authority for "team allocation sees availability only".** Nothing independent requires it: `docs/access-matrix.md` line 94 gives team allocation role-level view/edit codes and no field boundary; §4.4 covers who *decides* selection, not what is visible; and at the database level `injuries_staff_select` (`0012_rls_policies.sql:651`) lets **any coach or medical role** read every column of `injuries` for their org — body area, restrictions, side, expected return. Clinical detail is separately gated because it lives in `injury_clinical`, not because this screen is special. **So availability-only here is an application choice, not a permission boundary**, and the line now in `29-team-allocation.md` is a codification of current behaviour written on 2026-09-09 — not a rule that predates it. Worth adopting deliberately if it is to be relied on.
