# Design brief — STAFF-SS-26, Publish and manage leaderboards
...
**Full review: `docs/walkthrough-reviews/staff-ss-26-to-33-review.md`.**
...
1. **The ineligible metrics stay visible and stay explained** — the document's rule ("an unexplained absence is worse than a greyed option") applies here as on the wall. A proposal may not hide Body mass, Readiness, Sleep or Soreness.
2. **The reason must be reachable.** §0ap: today the row says "Tap one below to see why" and the buttons are `disabled`, so nothing fires. The proposal chooses one of two — a focusable `aria-disabled` chip whose tap shows the reason, or the reason printed inline on each ineligible row — and says which. A third option, keeping `disabled` and the invitation, is not one.
3. **Six steps stay six chip rows with `aria-pressed`** — this is the clearest form in the staff app and the rest of the staff forms should move toward it, not the reverse.
4. **"Board actions" (Unpublish, Delete board) stay grouped and stay last** on the board page; the ranking is what the page is for.
...
1. **The ineligible row** — reachable reason or inline reason, drawn at both widths, including the state after the tap.
2. **The board page at 375** — the ranking's `.lb-gap` column runs 8px past the viewport (§0ap); the proposal shows the ranking row fitting 375 without the page scrolling sideways.
3. **The manage page's three rules** (never-rank, three-athlete minimum, under-18 opt-in) are the right content in a paragraph nobody reads; a proposal may restructure them without dropping one.
