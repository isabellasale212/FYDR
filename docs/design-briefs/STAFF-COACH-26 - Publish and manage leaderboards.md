# Design brief — STAFF-COACH-26, Publish and manage leaderboards
...
**Read the STAFF-SHELL brief and the STAFF-SS-26 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. Where the coach's
screen is identical to the sport scientist's by measurement, the SS-26 brief is
...
1. **Identical to STAFF-SS-26 by fingerprint**, both widths; the SS-26 brief is the design surface, including the reachable-reason decision for the ineligible chips (§0ap, Builder Q8).
2. **The coach can create, unpublish and delete a board today** (`LEADERBOARD_EDIT`); D-05 said view-only and is on the pilot list as Isabella's call — a proposal does not assume either.
