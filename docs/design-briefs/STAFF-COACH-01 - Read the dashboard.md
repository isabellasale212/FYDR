# Design brief — STAFF-COACH-01, Read the dashboard
...
**Read the STAFF-SHELL brief and the STAFF-SS-01 brief first.** The phone shell
...
is identical to the sport scientist's by fingerprint — the SS-01 brief is the
...
1. **Identical to STAFF-SS-01 by measurement** — the coach's dashboard is the sport scientist's; a proposal for SS-01 (`docs/designs/STAFF-SS-01 final/`, built `af09c17` for the shell, C2 the dashboard rebuild still recorded) is the coach's too, and nothing coach-specific is drawn separately.
2. **The More sheet drops Analytics for the coach rather than greying it** — an absent destination, not a locked one, as the D-20 rule says for whole screens.
3. **Flags is the coach's bar slot** (`phoneSlot`), as for the sport scientist and the medic.
...
1. **Nothing coach-specific.** The one remaining sub-44 control on this screen at 375 ("Compliance ›", 19px) is §0au's, shell-level.
2. **The first card at 470px under a 152px heading** — the dashboard rebuild (SS-01 C2) is where that gap is addressed; not here.
