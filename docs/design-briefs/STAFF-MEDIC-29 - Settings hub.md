# Design brief — STAFF-MEDIC-29, Settings hub
...
**Read the STAFF-SHELL brief and the STAFF-SS-29 brief first.** The phone shell
...
1. **The medic's hub is seven sections and seven rows**; absent rows stay absent — and the two that lead to a refusal or a bounce (Thresholds, Integrations) become absent too (§0av).
2. **Subject access requests stays the medic's row**; the review is theirs alone.
...
1. **A hub for a role whose three rows that matter are Subject access, Notifications and Exports** — on a 2,875px page.
2. **Log out** as a row-sized target (§0ap, open).
