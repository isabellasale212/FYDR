...
- **Deploy incident, recorded so it is not re-diagnosed:** an **accidental rollback** occurred during the deploy and production was **re-promoted** to `7df310e`. Isabella's checklist for the day is `docs/runbook-deploy-2026-09-11.md` (to be committed on receipt).
- **Left on production by the verification:** `login_attempts` rows for the synthetic addresses `nobody.deploycheck@`, `warm.up@`, `nobody.c1check@`, `nobody.c1check2@`, `nobody.c1check3@` (all `@example.invalid`), and two placeholder strings pasted verbatim — `put.a.real.synthetic@ashcomberfc.example` and the literal `<synthetic account email>` — each at 4 attempts; four `auth.sign_in_failed` audit rows for `j.barnes@`. None locks anything; the rows age out of the 30-minute window and clear on that account's next successful sign-in.
