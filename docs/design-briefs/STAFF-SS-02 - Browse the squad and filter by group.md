# Design brief — STAFF-SS-02, Browse the squad and filter by group
...
1. **The count line must keep changing with the filter** — "30 athletes in the
   squad" → "15 athletes in the selected groups". It is the confirmation that the
   filter applied.
2. **The filter stays a cookie, global** — selecting Forwards here selects it on
   every screen.
3. **Athlete-name links must reach the floor at phone width** (§0af) — they are
   the screen's primary navigation, thirty times over.

## 5. What a proposal should address

1. **Row height and target size** for thirty athlete rows on a phone.
2. **"Clear filter" appearing only when active** is correct; the proposal
   should keep it conditional rather than always-present.
