...
**Removed 13 September 2026** (Isabella's decision, `docs/reports-catalogue-source.md`):
athletes export no report. `/me/export` and `lib/queries/myDataExport.ts` are gone.
An athlete asks the club, out of band, and the club opens an article 15 subject
access request.

### Subject access, the athlete side

`/me/privacy` ("Privacy and my data", PATTERN-S8 C10, 13 September 2026,
`docs/athlete/screens/20-privacy-and-my-data.md`): what the club holds (the pack's
own manifest, `SAR_CATEGORIES`), who sees what (this document, as sentences), how
to ask, and — from the athlete's own `sar_requests` rows, readable since
migration 0114 — where a request that concerns them stands, in the same words the
staff queue uses. Read only; the clinical review decisions stay closed to the
athlete.
