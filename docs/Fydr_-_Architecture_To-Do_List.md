...
- **Current live deployment (read from Vercel, 2026-09-11 16:35 BST):** `dpl_BuZQTttnBLTyGgGRdTtUcLJCRYJX` = `fydr-jvgpijjra-fydr.vercel.app`, a dashboard **redeploy** of `dpl_BPhPtr9igAFd7a7RWJKbZcvTcMqW` at 16:28 BST, same commit `7df310e`, holding all four aliases (`fydr.app`, `www.fydr.app`, `fydr.vercel.app`, `fydr-fydr.vercel.app`). It supersedes `fydr-p42jd9tj0` as "current live" in the runbook's rollback section.
- **Function region — Dublin is NOT in effect.** The redeploy was made to move functions to Dublin (`dub1`), but the live deployment reports `regions: ["iad1"]` and a POST to `fydr.app/auth/sign-in` answers with `x-vercel-id: lhr1::iad1::…` (edge London, function **Washington DC**), measured three times after the redeploy. Either the project's function-region setting was not saved before the redeploy or it was changed after it; a project setting only takes effect on the next deployment. **Required step:** confirm the setting reads Dublin in the Vercel dashboard, redeploy again, and re-measure (`curl -s -o /dev/null -D - -X POST -H "content-type: application/json" -d '{"email":"nobody.regioncheck@example.invalid","password":"x"}' https://fydr.app/auth/sign-in | grep x-vercel-id` must show `dub1`, not `iad1`). `docs/09-security-and-compliance.md`'s sub-processor row for Vercel records the measured state, not the intended one, until then.
