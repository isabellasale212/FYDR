# Design brief — STAFF-SS-12, Use the edit-mode toolbar
...
1. **The toolbar exists only in Edit mode and only for `SESSION_EDIT`** —
   absent from the DOM otherwise.
2. **The two "+ Session" entry points stay distinct** (header chip → full form;
   toolbar → grid wizard).
3. **Applying a template goes through a preview** — the chip is a link to
   `/schedule/planner/apply`, not an immediate change to the week.

## 5. What a proposal should address

1. **"+ Session" at 35px** in the toolbar (§0aj).
2. **"No templates yet."** was not observed; a proposal should design the empty
   toolbar as well as the populated one.
