# Design brief — STAFF-SS-29, Settings hub
...
1. **Eight sections in the measured order** and the settings list below them; the list rows are whole-row targets and every one of them must be — the Log out row is the one that is not (§0ap).
2. **The plan switch is a preview** and must keep saying so; a proposal that makes it look like a billing control is wrong.
3. **"Your role requires two-factor authentication" stays** — prompted, not enforced (O-323); the banner is the enforcement the app has today and a proposal may not soften it.
4. **Profile (read-only) and Edit profile (Name, Phone) stay distinct** — club, role and timezone are the club's to set.
...
1. **Log out as a row-sized target** — the fix is build work (§0ap) but the row's design should make the whole row read as the control it becomes; the destructive tone (`--bad-text`) on the label is right.
2. **One page carrying eight sections and eleven links** — 3,772px at 1280 and 5,089px at 375; the settings list, which is the actual hub, is at the bottom of both. A proposal should say whether the list moves up or the sections collapse.
3. **The Light/Dark segment (36px) and the plan switch (30px)** at phone — in the sweep.
