...
{
  /* The flush loops live in lib/outboxFlush.ts since 2026-09-14 (Today and
     the queue screen's Send now run the one function); the snapshot, the
     banners and the discard stay on Today's flusher component. */
  const f = strip(read('src/lib/outboxFlush.ts'));
  const ui = strip(read('src/components/OutboxFlusher/OutboxFlusher.tsx'));
  ...
  assert(/domain: 'gym' as const/.test(ui), 'gym conflicts are in the snapshot');
  // Repointed 2026-09-13 (PATTERN-S6 C1): pending counts a gym session's queued sets as one entry.
  assert(/new Set\(gym\.filter\(\(item\) => !item\.conflictAt\)\.map\(\(item\) => item\.input\.gym_session_log_id\)\)\.size/.test(ui), 'and not counted as pending');
  assert(/if \(domain === 'gym'\) dequeueGymSetLog\(id\)/.test(ui), 'discard works for gym');
  assert(/Use my numbers/.test(ui) && /reviseGymSetLog\(/.test(ui), '"Use my numbers" corrects the live set with the queued values');
  assert(/from another tab or device/.test(ui), 'the sentence the other three use');
