...
console.log('\nthe rules, from existing tokens');
{
  ...
  assert(/font-size:\s*var\(--fs-28\)/.test(heading) && /font-weight:\s*(?:800|var\(--w-black\))/.test(heading), '.after-heading --fs-28 / 800 (B1: the board\'s 30 has no step)');
  const fact = rule('.after-fact');
  assert(/font-size:\s*var\(--fs-20\)/.test(fact) && /font-weight:\s*(?:700|var\(--w-bold\))/.test(fact) && /color:\s*var\(--text\)/.test(fact), '.after-fact --fs-20 / 700 / --text (B3)');
