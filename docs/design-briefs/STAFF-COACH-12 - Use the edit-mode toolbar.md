# Design brief — STAFF-COACH-12, Use the edit-mode toolbar
...
**Read the STAFF-SHELL brief and the STAFF-SS-12 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-12 brief is the
...
1. **Identical to STAFF-SS-12 by driven comparison**, both widths; every toolbar control clears 44 at 375. The SS-12 brief is the design surface.
2. **The wizard button and the header "+ Session" link stay distinct** — one starts a draft in the grid, the other opens the full form; a proposal that merges them must say which survives.
...
1. **Where the toolbar sits on a phone** relative to the wizard it opens (267px) and the banner it changes (644px) — the three should read as one act.
