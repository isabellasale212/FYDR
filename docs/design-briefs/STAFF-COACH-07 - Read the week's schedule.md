# Design brief — STAFF-COACH-07, Read the week's schedule
...
**Read the STAFF-SHELL brief and the STAFF-SS-07 brief first.** The phone shell
...
is identical to the sport scientist's by fingerprint — the SS-07 brief is the
...
1. **Identical to STAFF-SS-07 by measurement**, and the coach holds `SESSION_EDIT`, so Edit mode, the toolbar, publish and discard (SS-08 to -15) are the coach's without change; a proposal for the schedule is one proposal for both roles.
2. **Pitch-side is the coach's case for this screen** — Edit mode at 375, where the Read / Edit segment measures 31px and the week arrows 32×32 (§0au). The floor is shell-level, but the proposal should not depend on those controls staying small.
3. **The group chips on this screen are plain `<button>`s at 39px** — the shared chip row (§0ak) is the pattern to draw, once.
...
1. **Edit mode on a phone** — the toolbar, the steppers and the publish banner at 375 for a coach who is adjusting a session an hour before it starts.
2. **The week arrows and the view switch** at the floor.
3. **3,224px at phone** — how much of the week grid a coach needs to see at once on a phone, against a day view.
