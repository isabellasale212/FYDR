# Design brief — STAFF-SS-27, Analytics
...
1. **Four fixed boards, one or two named athletes against a population** — the design's own thesis; no builder returns to this page.
2. **Each board keeps Metric and Window as real `<select>`s** or an equivalent with a visible current value; the current values (e.g. gps_distance · 84 days) are the state a reader needs at a glance.
3. **Athlete B is the same violet on all four boards** — learned once, read everywhere; a proposal keeps that rule.
...
1. **2,323px at phone** under the stacked sidebar: the selects for board four are a long way from the athlete picker they share.
2. **The two selects at 30px** at phone — in the STAFF-SHELL sweep; the proposal should show them at the floor without breaking the board's header row.
3. **"Compare two"** as a chip that navigates (`?compare=1`) — whether it reads as the mode switch it is.
