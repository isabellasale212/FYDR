...
## 0al. Two findings from exercising the schedule publish on scratch — 2026-09-11
...
- [ ] **On a network failure at publish, every pending change in the week is lost and the "Not published:" line is wiped before anyone reads it.** `ScheduleWorkspace.tsx` `publish()`: the writes go through the browser Supabase client; offline they throw, the `catch` sets "Not published: Failed to fetch" — correct — and then `finally` calls `router.refresh()` **unconditionally**. Offline, the refresh's own RSC fetch fails and Next falls back to a full browser navigation (`[browser] Failed to fetch RSC payload for /schedule. Falling back to browser navigation. TypeError: Failed to fetch`, then `GET /schedule 200`). The reload destroys `edits`, `added`, `removed` and `newDraft` — all `useState`, nothing persisted — and the error message with them. **Reproduced 2026-09-11**: draft staged, network emulated offline, Publish pressed; the page came back on "The athlete app is up to date" with no draft and no alert, and no row written. The document's "the button returns to enabled" holds only for a server-side error, not a network drop — and a pitch-side phone drops the network, not the server.
  ...
  **The comment justifying the unconditional refresh is right for a partial failure** (some sessions written, some not — the grid must resync). It is wrong for a total one. **Fix:** persist the pending state to `sessionStorage` so a reload restores it (the robust fix — it also survives an accidental refresh); or skip `router.refresh()` when nothing was attempted successfully; or catch the refresh. The first is the one that matches "Held on your screen".

- [ ] **Publishing a session to athletes writes no audit row. Neither does removing one.** `sessions` carries only `sessions_set_updated_at`; `schedule.ts` never touches `audit_log`. Measured after the exercise above: the window holds six `report.squad_weekly.view` rows and two `.export` rows from reading a report, and **nothing** for creating a session that appeared on five athletes' phones or for deleting it. Reading is audited; the staff app's single highest-reach write is not. Not an access gap — an accountability one: "who changed the week, when" is unanswerable today. Belongs with the audit-coverage programme (§0p, §0q, `0099`); a trigger on `sessions` and `session_participants` of the same shape as the athlete-entry ones.
