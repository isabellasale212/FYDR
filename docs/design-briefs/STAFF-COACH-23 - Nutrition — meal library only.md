# Design brief — STAFF-COACH-23, Nutrition — meal library only
...
screen is identical to the sport scientist's by measurement, the SS-23 brief is
...
1. **The coach holds `MEAL_LIBRARY_EDIT` and not `NUTRITION_EDIT`:** the library is theirs; plans, targets and assignment are not. A proposal keeps that split and makes it legible — absence or a stated reason, never a greyed control with no sentence (§0av).
2. **The day-type targets stay stated in g/kg on the control** (SS-23's own good pattern).
...
1. **What the coach wants from this screen** is one number per group for today; on a 5,728px page it sits under the plan list. Whether the coach's view opens on Day type, or the plan list collapses for a role that cannot author.
2. **The seven greyed controls** — drawn as absent, or with the sentence, at 375.
