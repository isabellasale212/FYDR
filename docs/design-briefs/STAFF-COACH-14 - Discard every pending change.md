# Design brief — STAFF-COACH-14, Discard every pending change
...
**Read the STAFF-SHELL brief and the STAFF-SS-14 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-14 brief is the
...
1. **Identical to STAFF-SS-14**; the confirmation names the count ("Discard 1 change? This can't be undone.") and stays in place; a proposal keeps the count and the in-place confirmation.
2. **Discard is week-wide and says so**; "Cancel changes" and "Restore session" are the per-session undo — three verbs, kept distinct.
...
1. **Nothing coach-specific.** The confirmation's placement follows the banner (13).
