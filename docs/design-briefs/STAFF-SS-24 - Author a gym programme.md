# Design brief — STAFF-SS-24, Author a gym programme
...
1. **The detail link's label follows permission** — "Edit this programme →" /
   "View full detail →" — and a rehab programme follows `REHAB_PROGRAMME`, not
   `PROGRAMME_EDIT`. A proposal must not collapse the two labels into one.
2. **S&C's proposer power** (`INJURY_PROGRAMME_PROPOSER`) is theirs alone; the
   sport scientist's screen does not show it and must not gain it.

## 5. What a proposal should address

1. **Nothing of substance.** It fits one desktop screen and every claim held
   bar a count. Recorded so it is not redesigned for its own sake.
