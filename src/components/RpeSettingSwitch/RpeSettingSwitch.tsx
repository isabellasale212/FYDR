...
import { RPE_SWITCH_OFF, RPE_SWITCH_ON, RPE_SWITCH_SCOPE } from '@/lib/rpeSetting';
    ...
    <div className="rpe-setting">
      <button type="button" role="switch" aria-checked={on} className="mute-switch" onClick={flip} disabled={busy}>
        <span className="mute-switch-text">
          <span className="mute-switch-title">Session RPE after training</span>
      ...
      <p className="tiny" style={{ marginTop: 'var(--sp-6)' }} data-rpe-scope>
        {RPE_SWITCH_SCOPE}
      </p>
      {error ? (
