...
- **Navigation lists only what the role can use — decided 2026-09-12 (Isabella), applies to every role.** A nav row or tab that always refuses a role is **removed** for that role, not disabled. Inside a page, a control the role cannot use stays a `BlockedButton` with its reason — that rule is unchanged. Filed against §0av, starting with the nutritionist's eight rows for two working screens and the dead Gym tab on the profile.
- **Injured-athlete gym assignment, decided 2026-09-12 (Isabella): the proposal gate applies to EVERY role except the medic — the sport scientist included. No direct assignment of a programme to an athlete with an open injury without medical sign-off; enforced at the database, not only the screen.** A before-pilot change — §0bb.
- **PATTERN-S4 D1 (sessions live on create, no publish, no discard): DECLINED 2026-09-12 (Isabella). Publish stays.** Why: "nothing changes on their phone until you publish" is the schedule's core promise, and dropping it would discard the offline-publish protection just built (§0al, `71035f5`). The sheet's row is struck as declined; everything on the S4 board that depends on D1 (the grid-click popover writing on Add, a drag writing at once, the three-timeframe confirmation, "N sessions live") falls with it.
- **SAR withholding reason, decided 2026-09-12 (Isabella):** the admin sees a **category** for a withholding (e.g. third-party data, clinical harm), not the medic's free text; the free text stays medic-only; the audit log holds the full record. A before-pilot schema change, not blocking — §0ba.
- **Silent no-op controls, decided 2026-09-12 (Isabella):** a control the database refuses while the screen reports success is the worst shape; every such control gets the `BlockedButton` treatment (`aria-disabled`, reason on tap), starting with the group reorder arrows (§0az).
- **Staff app offline, decided 2026-09-12 (Isabella):** accepted as a limitation for the pilot and stated plainly to the club; the offline foundation (§0b L751 / §3 L1988) stays post-pilot. The medic's pitch-side case (no signal, an injury just happened — the form does not load) is recorded against it.
- **§0aa direction, decided 2026-09-12 (Isabella):** build `resolveGymSetConflict` as the builder recommended — the same targeted lookup the other three domains do, so a genuinely different value becomes a visible conflict instead of being dropped. Built `75c6a6e`.
- **§0at direction, decided 2026-09-12 (Isabella):** derive gym tonnage from the live sets rather than storing `total_volume_kg` — one source of truth, nothing to backfill; stop if deriving is too slow on the biggest real session (it was not: 0.455 ms for the whole org). Built `5003959`, migration 0106.
- **Two target-size tokens, decided 2026-09-12 (Isabella), the third token-value decision under CLAUDE.md §0.01:** `--hit-lg: 56px` (the gym logger's one primary) and `--hit-md: 52px` (its stepper squares) — new steps, not aliases; the 44px floor everywhere else unchanged; the 48px set chip stays a literal (a state display). Dated with Isabella's name beside the values in `tokens.css`; `06-design-system.md` §11.1 records them. **Built `52b6f82`** (ATH-ADULT-09 C1 / 10 C1 / 11 C1), full sweep green (`check:scale-tokens`, `check:contrast` both themes, surface / radius / spacing / font-scaling).
- **Design decision sheet, decided 2026-09-12 (Isabella) — `docs/design-decisions-outstanding.md`, recorded there under "Decisions — 2026-09-12":** SS-01 C5 struck as already live (0101); declined — 08 D1, 12 D2, 12 C4, 12 B6, 12 D5, the 3px bar pattern (12 B5 and in SS-01 B1), 10 C2, motion tokens (09 B3), the second nutrition question (08/12 D2/D6), SS-01 C4; gold kept on navigation (12 D4 / SS-01 D2 declined) and reversed inside the gym logger (09 D4 approved); **everything else in groups (a) to (d) approved and queued**, including the gym logger rebuild (09 C1) with two new tokens **`--hit-lg: 56px` and `--hit-md: 52px`** (09 B2) — new tokens under CLAUDE.md §0.01, dated beside their values when built, their own line here on landing.
- **Injury creation by a coach, decided 2026-09-12 (Isabella): keep the permission** (`injuries_staff_insert`, §3.2 "New injury VC VC VC VC X") **and fix the inconsistency the other way — "+ Injury" appears on the `/injuries` board for the coach too.** The coach's form is the non-clinical one (PATTERN-S3 C9); clinical fields stay medic-only. Filed on §0av as a defect with that direction.
- **Q27 closed, decided 2026-09-12 (Isabella): the coach does NOT see body mass — the section on the athlete profile as well as its buttons.** Medic, S&C and nutritionist do. Queued on the builder as STAFF-SS-02-05 C9 with the rule widened to the whole section; the data-architecture briefing's Q27 marked resolved.
...
- [x] **1. Launch sign-in page: new headline and a three-column feature grid.** Full spec as given: **CLOSED 2026-09-11: built — `1299d95` (desktop: headline in Roboto, the claim column visible, form states at every width) and `9db60c5` (phone); on production since the 2026-09-11 deploy (§0an).**
...
- [x] **Decided, 2026-09-05, needs code verification**: only sport scientist and coach may create, edit, or delete a squad group. Medic, S&C and nutritionist stay view only. This is the agreed model already, but currently unenforced in the code across all three group screens (list, new, detail), real build work, not just documentation. **CLOSED 2026-09-11: `GROUP_EDIT = ['sport_scientist', 'coach']` (`access.ts:306`); `/settings/groups/new` redirects without it.**
...
- [x] **Resolved, 2026-09-04**: medic gets full log-and-correct access to test results, the same as sport scientist, coach, and S&C, not view-only as previously planned. Testing sessions can be created and completed by any staff role except the nutritionist, who stays excluded under D-01. **CLOSED 2026-09-11: `ENTRY_CORRECTION = ['sport_scientist', 'coach', 'medic']` (`access.ts:335`).**
...
- [x] **Build order, per the specification document's own stated logic**: fix the role model (G-02) and the GPS duplicate-row bug (G-25) together first, the injury gate fix depends on the role model existing. Credential handling (G-03) next, cheap and independent. Everything else after. **CLOSED 2026-09-11: the order was followed — role model (`access.ts`, five roles), G-25 (0064/0072), injury gates (`requireInjuryAccess`), G-03 (invite links).**
- [x] **G-03 resolved, 2026-09-05, real build work**: temporary passwords removed entirely from both the single and bulk account-creation paths, not just force-changed. Every new account, athlete or staff, is sent a link instead; using it is how the person sets their own password and proves they own the address. No password is ever generated, emailed, or shown on screen. **CLOSED 2026-09-11: no temporary password on either path; `lib/email/send.ts` is the single invite sender; `test:credentials` in prebuild.**
- [x] **New, 2026-09-05, follow-up to G-03**: confirm a mail provider key is configured in production before relying on this. If it isn't, invite links are silently undeliverable on both paths and a club would have no way to tell. Check this before the first real club goes live. **CLOSED 2026-09-11: `RESEND_API_KEY` and `EMAIL_FROM_ADDRESS` are set in Vercel for production, and an `invite.email_sent` audit row with `provider: resend, delivered: true` exists (`send.ts` header). Local and preview still take the logged no-op by design.**
...
- [x] **D-02, agreed but not built**: Analytics should be sport scientist only, not even S&C, coach, medic, or nutritionist. Tightest role restriction of any screen. Right now every staff role currently reaches it. **CLOSED 2026-09-11: `ANALYTICS = ['sport_scientist']` (`access.ts:267`); `/analytics` redirects `/dashboard?e=no-analytics` otherwise; the sidebar row is role-gated.**
...
- [x] **D-27, small fix**: deleting a session correctly refuses when GPS records, injuries, test results, or compliance data are linked to it, the database blocks it. But the coach only sees a generic error, not the "cancel it instead" message the other two link types already give. Extend the existing worded-refusal pattern to all six linked tables, not just two. **CLOSED 2026-09-11: `schedule.ts:1412` returns "This session has recorded data. Cancel it instead." and `SessionActions` shows it.**
...
- [x] **Decision, 2026-09-04**: "Retire a board" is renamed to "Delete a board" and made irreversible, unlike retiring elsewhere in this app. The confirmation must state explicitly that it cannot be undone. **CLOSED 2026-09-11: the board page's action is "Delete board"; `LeaderboardStaffActions` confirms with "cannot be undone" (measured on STAFF-SS-26).**
...
- [x] **D-04, agreed but not built**: only S&C and sport scientist should be able to edit gym programmes, coach, medic, and nutritionist should be view only. Same shape as D-03 and D-06. **CLOSED 2026-09-11: `PROGRAMME_EDIT = ['sport_scientist', 'strength_conditioning']` (`access.ts:75`); rehab programmes `REHAB_PROGRAMME = ['sport_scientist', 'medic']`.**
...
- [x] **D-03, agreed but not built**: only the nutritionist and sport scientist should be able to author nutrition plans, not coach, medic, or S&C. Right now any staff role can currently reach and edit the nutrition authoring screen. Same shape as D-06. **CLOSED 2026-09-11: `NUTRITION_EDIT = ['sport_scientist', 'nutritionist']` (`access.ts:98`).**
- [x] **D-06, agreed but not built**: only coach and sport scientist should be able to edit the schedule, everyone else (medic, S&C, nutritionist) should be view only. Right now nothing enforces that, any staff role can currently edit the week. **CLOSED 2026-09-11: `SESSION_EDIT = ['sport_scientist', 'coach']` (`access.ts:70`); `test:schedule-access` in prebuild.**
...
- [ ] **POST-PILOT by decision, 2026-09-12 (Isabella): accepted as a limitation for the pilot and stated plainly to the club. The medic's pitch-side case is recorded against this item: with no signal from the start `/injuries/new` does not load at all; with signal lost after opening, the app refuses honestly and keeps the typed values until signal returns (measured 2026-09-12 as Ruth Callaghan at 375; `docs/walkthrough-reviews/staff-medic-01-to-05-review.md`).** Staff web app needs a service worker and real local storage (IndexedDB) for any of this to survive a dropped connection or reload, see the PWA note in section 3.
...
- [x] **D-31 resolved**: link the Timetable screen from the schedule (it currently has no link anywhere, not even the sidebar). Kept deliberately as a quick single-day view, not retired. **CLOSED 2026-09-11: `schedule/page.tsx:87` builds `timetableHref` (group-aware) and renders it. The dashboard link (L716 addendum) is separate and still open.**
...
- [x] **D-34 resolved**: the Injuries screen stays deliberately out of the sidebar, reached only through the injury report and direct links. **CLOSED 2026-09-11: no sidebar row; reached from the athlete profile, `/reports/injuries` and by URL (STAFF-SS-28).**
- [x] **Squad setup, spec'd 2026-09-05, not yet built.** A new screen, Add athlete, is fully specified: sport-scientist-owned, name/DOB/position/squad number/optional invite email, resolves D-16. No code exists for it yet, this is a build task, not a documentation gap anymore. One open product question before it's built: how should a transfer from another club already on the platform be handled, a fresh record or a link to the existing one? **CLOSED 2026-09-11: `/squad/new` exists and was reviewed as STAFF-SS-03; `test:add-athlete-outcomes` in prebuild.**
...
## 0r. Every session an athlete has to rate is labelled identically — found 2026-09-10; CLOSED 2026-09-11

**Closed by the ATH-ADULT-02 build, `cfd410b`.** The row and the RPE screen's `<h1>` are both `rpeRowName(title)` → "Rate Contact prep"; `test-control-names-resolve.ts` holds the pair as a template. Verified by the builder live on Matt Reid's two outstanding sessions. Original entry follows.

- [x] ~~**`compliance.ts:144` hard-codes the RPE to-do row's title~~ **Closed `cfd410b`.**, so two sessions in one day are indistinguishable on Today.** The row reads **"How hard was it?"** for every session. The `session_id` is on the row and its name is never looked up.
...
- [x] **CLOSED by the ATH-ADULT-03 build, `6618b7f` + `5ae00ea` (builder, 2026-09-11 evening; merged `b13cc29`, no conflicts). Part one removed `.phone-body`'s dead `overflow-y: auto` so sticky resolves against the viewport; part two made `.subm` sticky again, with the safe-area inset and the count on its own line. Reviewer spot-check on scratch at 375×812, scroll 0: `.phone-body` overflow-y `visible`, `.subm` sticky/bottom 0 at 653–812px, "Submit entry" at 751–804px inside the viewport, `aria-disabled` not `disabled`, count line "0 of 6 answered · 6 to go". Guards: `test-phone-shell-scroll.ts`, `test-ath-adult-03.ts` (135), `test-ath-adult-03-render.mjs` (45).** ~~The submit button on `/check-in` sits at 1030px absolute on an 812px viewport — 229px below the fold, 414px on a 375×667 handset.~~ The five wellness scales end at 801px, clearing the 812 fold by **11px**, so the screen looks like the whole task fits when the control that completes it is off-screen. Answering all five enables the button and changes its label and **scrolls nothing** — the athlete's view is identical before and after the form becomes valid.
...
- [x] **The gym prescription line substitutes an error sentence into the load slot, producing "3 × 8 @ No 1RM test linked to this exercise yet."** Measured on `/gym/b4373061-…` (Conor Moroney, "Lower A", Romanian deadlift): a single `<span class="scheme num">` contains that entire string — it is one element, not two run together by `innerText`. **CLOSED `a546137` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): `lib/gymPrescription.ts` `schemeLine` omits the "@ …" clause without a load; *measured on `/gym/b4373061` as Conor: "3 × 8–10 @ 100 kg · 90s rest" and "3 × 8" for the Romanian deadlift, the reason sentence only in the weight row.* 12 assertions in prebuild.**
...
- [x] **Gym set buttons are 42px tall, below the app's own 44px floor.** Measured: every "Log set {n} of {total}, {exercise}" button is **105.7 × 42**. Consistent across both rendered exercises. **CLOSED `45816c8` (reviewer poll 2026-09-12 19:50, merged `22f06d7`): `.gym-set-key` min-height 44px — *measured on `/gym/b4373061` as Conor: 44px, `min-height: 44px`.***
...
- [x] **The weekly nutrition question says "this week" about a week that has already ended.** The screen deliberately and correctly asks about `lastCompletedWeek` — the week line says so, e.g. "WEEK 36 · MON 31 AUG TO SUN 6 SEPT" — but the question underneath reads **"Did you hit your protein target most days this week?"** **CLOSED `fc3c5e0` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): `lib/nutritionWeekCopy.ts`; *measured on `/nutrition-check-in` as Conor: "Did you hit your protein target most days last week (31 Aug to 6 Sept)?" under "Week 36 · Mon 31 Aug to Sun 6 Sept".* Default week unchanged as decided.**
...
- [x] **A failed gym set log queues correctly but is only retried from `/today`, so the session count stays wrong for the rest of the workout.** **CLOSED `84b258e` (builder, 2026-09-12; ATH-ADULT-09 C4).** The retry now runs in the logger too: `flushGymSets` in `src/lib/gymOutboxFlush.ts` is one function shared by `OutboxFlusher` (Today) and `GymSessionLogger`, which calls it on open and on the window `online` event, and the progress row reads "N of M sets · K waiting to send · HH:MM" while anything is queued. *Reviewer measurement, read-only, as Conor at 375 on `/gym/b4373061-…`: the window `online` listeners are the Supabase realtime reconnect, `()=>onOnline(true)`, and a third whose body calls `gymOutboxFlush`'s `queuedGymSets` — on `/today` that third listener is `OutboxFlusher`'s `flush()` instead. The progress row reads "0 of 12 sets · 600:58" with nothing queued (§0g's unbounded clock, still open).* The builder's own offline run (Dan Okonkwo, CDP offline, set 1 → "0 of 12 sets · 1 waiting to send"; back online → "1 of 12 sets", outbox empty) is in the commit message. Measured 2026-09-10 with the network forced offline: the set does not show as done, the count does not advance, and the athlete sees "Couldn't save — check your signal and try again. Your answer is still here." The entry queues in `fydr-outbox-gym-set`.
...
- [x] **Gym session logs created by merely opening the screen are counted as "sessions logged" in two staff-facing reports.** `startOrGetSessionLog` writes a row on page load, before a single set exists (§0g). Compliance is safe — `reports.ts:252` filters `.eq('status','complete')`, as do `exportBuilder.ts:131` and `programmes.ts:1148`. Two places do not: **CLOSED `068e5ba` + `e052fd0` (reviewer, 2026-09-12 morning, merged `3103ad9`): `squadWeeklyReport.ts` counts a session as logged only with at least one live set (`superseded_by is null`), per the §0u comment at line 156; page-load creation unchanged as decided.**
...
- [x] **"Sessions logged" counts a gym session that has no sets in it. DECIDED 2026-09-10 (Isabella): a session counts as logged only once it has at least one set.** **CLOSED with the item above (`068e5ba`).**
...
- [x] **BUILT `6398493` (builder), `test:gym-corrected-marker` in prebuild, on production since the 2026-09-11 deploy (§0an). Box closed 2026-09-11 while compiling the pilot-readiness list.** ~~Correcting a gym set tells the athlete "My data marks the day corrected and shows what you first reported." My data does neither.~~
...
- [x] **`unmuteAll` in `notificationPreferences.ts:69` upserts `push_enabled: true, email_enabled: true` for every disableable type.** It never reads the row it is overwriting. `muteAll` likewise writes all-false without recording what it replaced. So the pair is not mute/unmute; it is set-all-off / set-all-on. **CLOSED `d8938b1`, migration 0103 (reviewer, 2026-09-12 morning, merged `3103ad9`): `mute_notifications` / `unmute_notifications` functions and a `muted_at` snapshot column present on scratch (0 rows currently muted). 20 pgTAP assertions (590). On scratch, not production.**
...
- [x] **CLOSED `75c6a6e` (reviewer, 2026-09-12 ~11:50, fast-forward merge to `1052354`), on Isabella's direction of 2026-09-12 ("Build resolveGymSetConflict as you recommended"): `lib/gymSetConflict.ts` `classifyGymSetConflict` — own id live → delivered; another id with the same reps/load/RPE → delivered; different numbers → a visible conflict with "Use my numbers" (the athlete's own correction path) and "Keep what is showing"; nothing live → conflict, never guessed away. `OutboxFlusher.tsx:338` calls `resolveGymSetConflict` where the unconditional dequeue was. 29 assertions (`test-gym-set-conflict.ts`) in prebuild; verified by the builder on James Barnes's live set 2 without writing a row. Found and left: the queued item carries no `side`.** ~~A gym set queued offline is silently discarded if its slot was filled by a different value.~~ `OutboxFlusher.tsx`, gym branch: any duplicate-key error is treated as the athlete's own replay and dequeued as *sent*, with no lookup. The wellness, RPE and nutrition branches do the lookup (`resolve*Conflict`) and surface a real conflict on `/today`; gym does not. The code's own comment calls the guard "a reasonable follow-up but a separate, out-of-scope change." It stops being out of scope once the offline path is being redesigned (§0u's retry-only-from-`/today` item). Not data loss of a set — the slot index guarantees one live row — but loss of the athlete's *numbers* for it, without telling them. **NOT BUILT overnight (`ea93b20`, docs only): no decided direction on this list. Builder question 3 recommends `resolveGymSetConflict` — a lookup on the duplicate-key error that dequeues only when the stored value equals the queued one, else surfaces the conflict on Today. Needs Isabella's direction; stays open and on the pilot list as group 1.**
...
## 0ac. ATH-ADULT-01 (phone) and C1 — handed over and recorded 2026-09-11
...
**ATH-ADULT-02, Today — `cfd410b`. IMPLEMENTED.** Handed over 2026-09-11; merged as a two-parent merge (`b576f3e`), no conflicts. Suite 77 suites / 2,385 passed. **Design:** each to-do row its own `.card` at min 44px (measured 76), no domain glyphs; section titles as `.eyebrow.today-sect`; a banner line above To do in the 01 banner shape (`--wash-warn`/`--wash-bad`), linking `#availability`; the availability card on a tone family with `*-pill-text` text — contrast pinned in the guard: warn 7.53 light / 6.11 dark, bad 8.81 / 4.95; "Set by your coach." in `--faint`. Order: greeting → banner (only when not available) → To do → Today → week strip + Working towards → availability → diagnosis → team. **Behaviour:** `lib/rpeDue.ts` — due at `starts_at + duration_min + 30 min`; carry-over of yesterday's expectations oldest first; `rpeClosesAt` = end of the following club-local day, after which the RPE screen refuses with "This session can no longer be rated…" (built to the dashboard's own "due last night" window, since the compliance report has no cutoff — §0ad); row and `<h1>` both `rpeRowName(title)` → "Rate Contact prep" (**closes §0r**; touches ATH-ADULT-05's screen, `03-session-rating.md` §1–2, §10 updated). **Review findings closed:** F1 (To do 561 → 137px, first row 593 → 163px), F2, F3, F4, F5, and board note 1. **Built differently:** strip stays below Today; diagnosis stays separate; S5 dropped; `--fs-28` not 30; `--r-card`/`--r-control` not 8px; the chip is the real `reason_category`; the card fill is the existing `color-mix` rule rather than `--wash-warn` (on the athlete's blue ground the wash composites to grey); "starts in X min" only within two hours. **A tightening to know about:** a null-duration session used to skip the RPE screen's due gate; it is now due 30 min after start — see §0ah. **Scratch state, Matt Reid only:** three availability rows written and closed in turn (`d90b12d8` modified, `f486556b` unavailable, `eebebc9c` available, current), RPE 6 for Contact prep (10 Sept) and 5 for Captain's run (11 Sept), one wellness entry for 11 Sept, one nutrition check-in for the current week; no deletions; browser signed in as Matt.

**ATH-ADULT-03, Submit the morning wellness entry — `6618b7f` + `5ae00ea`. IMPLEMENTED.** Picked up from `origin/build/walkthrough` by the reviewer's overnight poll 2026-09-11 ~22:15 BST (no relayed handover — Isabella's instruction for the night); merged as a two-parent merge (`b13cc29`), no conflicts; dev server restarted. **From the commit messages:** commit 1 — `.phone-body` no longer declares `overflow-y: auto`; fifteen athlete routes snapshotted before and after with every number identical except computed overflow-y; `test-phone-shell-scroll.ts`. Commit 2 — on the 2026-09-11 decisions (no bottom sheet, forms stay pages): A1/A2 on all four `.subm` forms (CheckIn, Rpe, NutritionCheckin, ProblemReport); A3–A9, C-c/d/e on `/check-in` only; B2, B5–B8 substitutions; D7 keeps the nutrition footer copy. Footer sticky and pinned, bleeds to the shell edge, safe-area inset, `--surf` under 1px `--border` with `--shadow`; count on its own line at full `--text`, `--wash-good` chip when complete; blocked action `aria-disabled` in `.btn-ghost`, `disabled` only while pending; nothing in the footer reads `--o-disabled`. `/check-in`: sleep starts empty and is the sixth question; "Not answered" in `--muted`; numbered end chips; one-sentence irreversibility line with a `<details>`; HR and body mass validated as typed against `RESTING_HR_RANGE` / `BODY_MASS_RANGE`, "Fix one field to submit". Specs 02, 03, 04, 15 updated in the same commit. **Spot-checked by the reviewer:** the pinned-footer claim, above under §0s — holds. **Prebuild after the merge:** 80 suites, 0 failed (78 + `test-phone-shell-scroll` + `test-ath-adult-03`); `next build` exit 0. Two earlier runs failed type-check on `.next/dev/types/validator.ts` — a half-written block for `/check-in` in the reviewer's own dev cache, regenerated by the dev server while the build read it; removed (gitignored artefact) and rebuilt with the dev server stopped. Not the merge. **Recorded:** §0s closed; the walkthrough's ATH-ADULT-03 section updated.

**Overnight 2026-09-11/12 — nine flow commits and eleven defect commits from the builder, merged as one fast-forward to `3103ad9` on the morning of 2026-09-12 (the reviewer's cron poll did not fire while the session was idle; the builder merged the reviewer's branch on its own side, so the morning merge was clean).** Each commit's full handover is its own message body; the builder's own account is `docs/overnight-summary-2026-09-12.md` and `docs/overnight-records-2026-09-12.md`. Reviewer spot-checks, one per commit, on the restarted server:

- **ATH-ADULT-02 follow-up — `4b7b4fa`. IMPLEMENTED.** Week strip above To do, no avatar, athlete cards on `--r-toggle`, a warm availability line. *Measured on `/today` as Conor at 375: five `.card`s at `border-radius: 9px`, no avatar element, section order This week → To do → Today → Working towards, availability line "Available".*
- **ATH-ADULT-04 — `99db8a6`. IMPLEMENTED.** "Already submitted" after-card, the fact first, the exit a button. *Source: `check-in/page.tsx` — `.after-card` with `.after-heading` "Already submitted", the exit a `.btn-primary` Link reading "Back to Today" (or "Back to My data" for a past day). Conor has no entry today, so not rendered live.*
- **ATH-ADULT-06 — `13f68e4`. IMPLEMENTED.** *Measured live on `/rpe/00000000-…`: "This session isn't there" as an `<h2>` at 28px, one exit "Back to Today" as `a.btn-primary`.*
- **ATH-ADULT-08 — `a69c958`. IMPLEMENTED** (the second question and once-only correction recorded, not built). *Source: `nutrition-check-in/page.tsx` — "Already answered", "Back to Today" `.btn-primary`, "Correct this answer". Conor's check-in is still on his To do, so not rendered live.*
- **ATH-ADULT-09 — `e50d5da`. IMPLEMENTED in part** (borderless exercise card, sticky logger header; the set-by-set rebuild, the 56/52/48 size tokens and the gold→accent reversal recorded, not built). *Measured on `/gym/b4373061-…` as Conor: `.gym-ex-card` border `0px none`, no shadow; `.gym-head` `position: sticky; top: 0`. `.gym-set-key` still 42px — §0u's 44px item stays open as expected.*
- **ATH-ADULT-10 — `1771cf0`. IMPLEMENTED in part** ("Finish early" as a dashed neutral control; header placement, confirmation and early summary recorded). *Measured: "Finish early · 0 of 12", `.btn-ghost.gym-finish-early`, `1px dashed`, transparent, 44px.*
- **ATH-ADULT-11 — recorded in `e50d5da`'s record, nothing buildable within the boundary.**
- **ATH-ADULT-12 — `e581211`. IMPLEMENTED in part** (absent values as words, hero at `--fs-48`, rows at the floor, value column right-aligned; the five-segment reversal and the rest recorded). *Measured on `/my-data` as Conor: hero "80" at 48px, four "Not submitted/logged" words, history rows 73px, value cells `text-align: right`. **One consequence filed as §0at below.***
- **ATH-ADULT-13 — `98cfeec`. IMPLEMENTED in part** (two-up summary hero, one full-width "Back to gym history", "recomputed after a correction", absent words; the eyebrow, tap-a-row correction and per-row marker recorded). *Measured on Conor's 11 Aug session: one back control, `.btn-ghost` 335×57; hero "6.8" at 48px.* The builder's own post-push correction to this handover (a corrected fixture does exist: James Barnes, 2 Sept) is noted.
...
**Second builder queue, 2026-09-12 morning — `75c6a6e` (§0aa), `5003959` (§0at, migration 0106), `af09c17` (STAFF-SS-01), summary `1052354`; fast-forward merge ~11:50, dev server restarted.** Handovers in each message body; the builder's account in `docs/overnight-summary-2026-09-12.md` (second queue) and `docs/overnight-records-2026-09-12.md`.
...
**Third builder queue, 2026-09-12 early afternoon — eleven commits `a546137` … `ea1fcac`, fast-forward merge ~13:30 (the reviewer's three Chrome profiles had been closed with the desktop; relaunched, sessions intact); dev server restarted.**

- **§0u ×2, §0aj ×2, §0ap ×2, §0as, §0au — IMPLEMENTED**, closed above with a spot-check each.
- **STAFF-SS-02 / SS-05 and PATTERN-S3 — `ea1fcac`. IMPLEMENTED in part (the A items):** "Not recorded" as the one word for no availability status; the staff availability note labelled "Note" with the hint "Coach visible. Describe the restriction, not the injury. Do not name a diagnosis or a protocol."; the corrections intro states the 28-day window and what is not correctable; the empty injury panel reads "No current restrictions. This is not the same as being cleared."; a missing clinical value reads "Not recorded"; an unset return "Expected return not known". *Spot-check on `/squad` as the coach: pills "◌ Not recorded / ◐ Modified / ● Available / ⊘ Unavailable".* Everything else on both boards is on the decision sheet (below), with two items flagged ⚠ permissions: SS-02-05 C9 "body mass hidden from the coach" (the data-architecture briefing's Q27) and PATTERN-S3 C8 "body site not coach-visible". The builder's Step 1 single-role inventory for SS-05 is its first morning step.
- **`abaf932` — `docs/design-decisions-outstanding.md`. RECORDED:** one sheet of every unbuilt design item across ATH-ADULT-04 to -13 and STAFF-SS-01, grouped, costed, with a recommendation each — **for Isabella's decisions**; extended by `ea1fcac` with the SS-02-05 and S3 rows.
- **`220d8e9` — Builder question 8:** §0ap's leaderboard dead tap is a design decision (reachable tap vs inline reason); recommendation recorded, not built.
- **Prebuild after the merge (reviewer machine):** 110 suites, 0 failed; `next build` exit 0.

**Fourth builder queue, 2026-09-12 evening — eight commits `d3298f4` … `47a659e`, merged `7bea968` ~19:10; dev server restarted (the restart hit a Turbopack internal panic in the reviewer's `.next/dev` cache — cleared and restarted; not the merge).** Decision-sheet items from the approved groups, each struck on the sheet by the builder:

- **ATH-ADULT-04 C1 + C4 — `d3298f4`. IMPLEMENTED.** A corrected check-in day shows a "Corrected" pill and "Corrected by {name} on {date}. What you first reported is in My data."; "You sent … at" uses the original submission's time. *Source: `check-in/page.tsx:146-148`; not rendered live (Conor has no corrected day).* `visibility.md` checked by the builder: who corrected is not withheld.
- **ATH-ADULT-09 C5 — `458c6f1`. IMPLEMENTED.** Wake Lock for the session and a buzz on a logged set, feature-detected (`lib/wakeLock.ts`), never a throw. *Source only — headless Chrome grants no lock and has no vibrate; 14 assertions.*
- **STAFF-SS-01 A1 — `59eee8d`. IMPLEMENTED.** Doubtful and Ruled out as tone-family cards on the readiness card. *Measured on `/dashboard` as Jane: `data-tone="warn"` on a warm fill, `"bad"` on a red one, Fit and available plain.*
- **STAFF-SS-01 A2 — `58f0aca`. IMPLEMENTED.** The two summary cards state their state. *Measured: "▸ Closed · opens a list" → after a click "▾ Open · showing the list", `aria-expanded` true.*
- **STAFF-SS-01 A4 — `a458c8e`. IMPLEMENTED.** Missing check-ins ordered by mornings in a row. *Measured: "Not submitted · 28 mornings in a row · last entry Sat 15 Aug" first, then 27, 27.*
- **STAFF-SS-01 D3 — `ecb9c59`. IMPLEMENTED.** "Ready for {matchday}" only within 14 days (`FIXTURE_RANGE_DAYS`), else "Squad readiness · No fixture in the next 14 days". *Scratch's next fixture is today, so the in-range branch rendered ("v Colthorne RFC"); the out-of-range copy is the guard's (31 assertions).*
- **STAFF-SS-02-05 D2 / §0ap / §0av — `11dc42f`. IMPLEMENTED.** `BlockedButton`, one rule for blocked controls; closed above with two live measurements. Left by the builder: the busy/pending `disabled` buttons keep `disabled` (a different state).
- **STAFF-SS-02-05 C4 / PATTERN-S3 C4 — `47a659e`. IMPLEMENTED.** Before an availability change lands, "Before this lands — who will read what": the athlete by name, the coaches and the S&C by role, medical staff and the sport scientist; "Confirm and update" / "Back". *Measured as the medic on `/injuries/e210bb3a` at 375: the step renders with the three rows for Viliami Tameifuna; Back returns to the form; no write.* Found and left by the builder: the phone title bar reads "Dashboard" on an injury record (next commit).
- **Prebuild after the merge (reviewer machine):** 114 suites, 0 failed; `next build` exit 0.

**Fifth builder queue, 2026-09-12 evening — ten commits `6ded52a` … `45816c8`, merged `22f06d7` ~19:50; dev cache cleared before the prebuild this time, no panic; dev server restarted.** The decision sheet's group (b) reversals and the two new pattern folders' A items:

- **Phone title bar — `6ded52a`. IMPLEMENTED.** `/injuries/*` titled "Injuries". *Measured as the medic at 375 on an injury record: "Injuries · Whole squad".*
- **STAFF-SS-02-05 C8 — `0f96ea4`. IMPLEMENTED.** Empty panels state the requirement ("A trend needs three weigh-ins", "Targets are per kilogram, so a plan needs a weigh-in", "n = 0"). *One of the three rendered on Dan Okonkwo's profile ("flags above 1.30 · n = 0 sessions"); the others need an athlete with no weigh-in / no plan.*
- **PATTERN-S4 (Schedule and week grid) — `b3cbaab`. RECORDED; A items IMPLEMENTED:** session minutes not contact minutes, Meeting in the legend, the current tab says so, the primary says what it will make, the missing-group refusal states the consequence, read-only in a well, a Removed pill. *Measured as the coach at 375: "445 session minutes · staff sessions excluded …", the Week plan tab `aria-current="page"`.* **The board's headline is a reversal (D1: sessions live on create, no publish/discard) — on the sheet, group (b), with a recommendation; nothing built on it.** PATTERN-S5's record written in the same commit.
- **PATTERN-S5 (Programme authoring) — `702dd2c`. RECORDED; A items IMPLEMENTED:** a bodyweight exercise "logs reps only" as a plain value; the profile's programme link is the tell. *Measured: coach "View plan" / "View full detail ›", Jane "Change plan" / "Edit this programme ›".* C1–C10 and D1–D2 on the sheet.
- **ATH-ADULT-09 D4 reversed — `7824358`. IMPLEMENTED.** One accent inside the logger; gold stays on navigation. *Measured as Conor: `.gym-progress-fill` rgb(23,72,155) = `--accent #17489b`; the next set key on the accent wash.*
- **ATH-ADULT-12 D1 reversed (with B3, C9) — `ada5db5`. IMPLEMENTED.** Five segments — Wellness · Gym · Sessions · Nutrition · Tests — at `--fs-11` on one row. *Measured as Conor at 375: five `.md-seg` at 11px, track 52px (one row).* D2 kept (accent pill).
- **ATH-ADULT-12 D3 reversed — `029ab76`. IMPLEMENTED.** Deltas uncoloured, figure bold, ↑↓. *Measured: "↓ 12 on last week", muted ink, `<b>` figure.* 12 C4 declined stands ("on last week").
- **PATTERN-S3 D1 enforced / SS-02-05 C7 — `a25ceab`. IMPLEMENTED.** `restrictionLine()` at the query layer drops protocol / stage / diagnosis entries for every reader; the seed no longer writes one. *Measured as Jane on `/injuries/rehab-groups`: no protocol or stage word in any restriction line ("No contact · No scrummaging +2", …).*
- **ATH-ADULT-12 D7 — `ca69a4b`. IMPLEMENTED** as an athlete-only `accentOnly` prop on `WellnessChart`; the sheet's kept / declined rows struck. *Measured as Conor: the chart's day markers all rgb(23,72,155).*
- **ATH-ADULT-09 B4 — `45816c8`. IMPLEMENTED.** Set keys at 44px; closed above (§0u).
- **Prebuild after the merge (reviewer machine):** 118 suites, 0 failed; `next build` exit 0.

**Sixth builder queue, 2026-09-12 20:05 — `3e7b806` (§0az) and `00a7119` (ATH-ADULT-08 C1–C3), fast-forward; dev cache cleared, prebuild, dev server restarted.**

- **§0az — `3e7b806`. IMPLEMENTED**, closed above with a live measurement. The builder added the item to the decision sheet as built.
- **ATH-ADULT-08 C1–C3 — `00a7119`. IMPLEMENTED, migration 0107 on SCRATCH only.** The weekly check-in can be corrected once: `revise_nutrition_checkin` raises `entry_already_corrected` on a live row that is itself a revision (0107, tests first — pgTAP 630); the page refuses `?correct=1` on a corrected week before the form; three after-submit states on one card (answered + the C3 caption "You can correct this once after you submit."; "Correction saved" via `?saved=1` on a spent read; spent — "Already answered" with the Corrected pill and "You have used your one correction…"); Save correction returns to the page, not My data; My data's nutrition rows carry the pill, "was {answer}", and no Correct link on a corrected week. *Source-verified (the caption string and the 0107 refusal name); Conor's check-in is unanswered so the states did not render.* **Scratch is at 0108** on the read-only migration read — one ahead of the tree's 0107; the next builder push should carry it.
- **Prebuild after the merge (reviewer machine):** 118 suites, 0 failed; `next build` exit 0.

**Seventh builder queue, 2026-09-12 ~20:30 — `84b258e` and `3a2daac`, merged `0582a22`; dev cache cleared, prebuild, dev server restarted.** Decision sheet group (c), ATH-ADULT-09 and -10.

- **ATH-ADULT-09 C4 — `84b258e`. IMPLEMENTED.** Queued gym sets are retried from the logger (on open and on `online`) as well as from Today, through one shared `flushGymSets`; the progress row names what is waiting to send. Closes the §0u item above ("only retried from `/today`"), measured read-only as Conor. Test `scripts/test-gym-logger-flush.ts` (15) is in the prebuild chain. Left by the builder: Dan's fixture log shows the unbounded clock (§0g, open); sets 1–2 of Dan's Lower A are real scratch rows from the verification.
- **ATH-ADULT-09 C6 / 10 C3 — `3a2daac`. IMPLEMENTED.** Two summaries replace the set list once a log is closed: complete (total volume MET-041, sets done, "Best you have logged" with the prior best and its date, all exercises with their loads, "Sent to My data.") and finished early (a dashed card, one row per exercise from `setsLine`, count pills, no totals). MET-040 Best logged set and MET-041 Session volume registered; `docs/metrics-parity.md` row added; Finish now stays on the screen. *Measured as Dan Okonkwo (profile 9503, now signed in as Dan) at 375 on `/gym/e99bd0bd-…`: "Session complete · 11 of 11 sets", Total volume `6,228 kg` in `.gym-sum-num` at 48px = `--fs-48` (3rem), "Weight × reps across 11 sets", "3 exercises · 1 min", "Bench press 150 kg × 5 / Best before today 104 kg × 5 · Thu 13 Aug" with the best line in `rgb(23,72,155)` = `--accent`, footer "Back to today" 55px / "Correct a set" 53px.* The rendered figures differ from the commit message's (5,478 kg; bench press "no load logged") — a bench press set was added to the closed log after the handover, which is the builder's own FOUND AND NOT FIXED (a closed session still takes another set after "Correct a set"; recorded on the sheet under 09 C1). Test `scripts/test-gym-summary.ts` (38) is in the prebuild chain.
- **Prebuild after the merge (reviewer machine):** 128 chain entries ran, 0 failed; exit 0.

**Eighth builder queue, 2026-09-13 ~03:20 — twelve commits `aec58c2` … `1bf38b9`, merged `e470134`; dev cache cleared, `npm run -s build` (126 suites passed, 0 failed; `next build` exit 0), dev server restarted.** Decision sheet group (c) throughout. Spot-checks are page loads only (the test-club HOLD is in force).

- **ATH-ADULT-11 C2 — `aec58c2`. IMPLEMENTED.** "Set N corrected · was …" beneath the keys; the key's label says corrected. *Not reachable by a page load this morning: the logger now opens today's (empty) log — the date rolled over at midnight — so the strip on yesterday's corrected session is not on screen. The same correction is on the session detail (next line). `test-gym-corrected-strip.ts` (11) is in the chain.*
- **PATTERN-S6 (System states) — `ea64eaf`. RECORDED; A/B items IMPLEMENTED:** a send that worked changes the count in place; no toast on Today; the entry-form failure sentence; the waiting line as a good-tone card; the conflict notice in the bad tone. Step 1 answered from the code in `docs/overnight-records-2026-09-12.md`; C/D on the sheet — including the builder's **C10 ⚠ decision**: a check-in queued past its three-week write window is refused by policy for ever and counted as waiting for ever. *Measured as Conor: `/today?submitted=1` renders no toast and no "submitted" text (the arrival toast is gone); the sent/waiting card needs a queued send — not exercised.*
- **ATH-ADULT-12 C6 — `aff2c03`. IMPLEMENTED.** *Measured as Conor, Gym tab, 28 days: "Nothing in the last 28 days. Your last gym session was Thu 13 Aug, 31 days ago. It is still on record, just before the period you have chosen. Gym sessions appear here once you finish one." with one action, "Show this season".*
- **ATH-ADULT-12 C7 — `0b601c1`. IMPLEMENTED.** Tests lists the club's tests; one without a result reads "Not logged". *Measured as Conor: seven club tests listed ("See all 7 tests →"); Conor has a result on every one, so no "Not logged" row renders for him — the builder's own render carries it.* Built differently: "assigned" = the club's set (no per-athlete assignment in the schema), recorded on the sheet.
- **ATH-ADULT-13 C1 / C3 — `1357d23`. IMPLEMENTED.** *Measured as Dan Okonkwo at 375: the history row "Sat 12 Sept · Corrected · Upper B · 11 sets · 6.2 t"; the detail's eyebrow "GYM · UPPER B · COMPLETE", "6228 kg · 11 sets · recomputed after a correction", and "Set 1 was corrected on Sat 12 Sept. Both values are kept on record."* Seen, not filed (no reviews inside the poll): Dan's Gym tab reads "SESSIONS 2 of 1 assigned".
- **STAFF-SS-01 A3 / C3 — `6ecc077`. IMPLEMENTED.** *Measured as Jane at 375: "30 athletes · need attention · 74 open flags · 67 not yet reviewed by anyone · top 5 of 30"; the Flags slot carries `.ph-count-mark` "30" in rgb(23,72,155) on white, radius 999px, label "Flags, 30 athletes need attention".*
- **STAFF-SS-01 C2 — `095ad70`. IMPLEMENTED.** *Measured as Jane: "Thresholds set by Mark Iremonger · Wed 2 Sept · Change ›" closes the panel, the link to `/settings/thresholds`.*
- **STAFF-SS-02-05 C2 / B4 — `6248943`. IMPLEMENTED.** The roster below 768: name link at --fs-16 700 in --accent-text on a 44px target, position beneath, pill right, restriction line on the row, no header, no sideways scroll. *Measured as Jane at 375: rows 75px (Dan, no restriction), 99px, 118px (James, three restrictions) — the tr is a `44px + 11px` grid with `min-height 60px` and 10px padding top and bottom, so the board's "60px row" renders at 75; the name link 16px / 700 / rgb(23,72,155) / 44px; five `columnheader` roles; `scrollWidth 375` on a 375 viewport.* **Closes F-17 of the test-club run (`/squad` scrolled sideways at 375 on Harlow Vale's 13-row roster).**
- **STAFF-SS-02-05 C9 / Q27 — `5a0d5d0`. IMPLEMENTED.** `BODY_MASS_VIEW` = SS, S&C, nutritionist, medic; the coach sees no Body weight card, no Weight cell, no Body mass card on the nutrition page; the wellness CSV drops the column, the body composition export is refused. *Measured as Mark on Viliami Tameifuna: the profile's h2 list has no "Body weight"; the bio reads Position · Jersey · Height · Age · Hand — no Weight.* Closes §0av's coach body-mass line (Q27).
- **STAFF-SS-02-05 C5 — `b15b21d`. IMPLEMENTED.** *Measured as Mark: the Injury card ends "READ-ONLY · SET BY MEDICAL STAFF · Ruth Callaghan · Mon 7 Sept · A coach can record a non-injury absence from the Availability panel; one that is injury-linked, as this one is, is the medic's."; the Nutrition plan panel "READ-ONLY · SET BY THE NUTRITIONIST OR THE SPORT SCIENTIST · Peter Ackland · Tue 11 Aug · Set for Forwards, not for this athlete alone."*
- **PATTERN-S3 C5 — `e7cf2d9`. IMPLEMENTED.** The coach's availability form is an absence form. *Measured as Mark: "Record an absence — For illness, personal, academic, representative or other. If this is an injury, medical staff record it…"; reason chips Illness · Personal · Academic · Representative · Other; Modified · Unavailable; "Available is not offered here: an absence that leaves an athlete fully available is not a record."* Permissions resolved without a change (0068).
- **PATTERN-S3 C7 — `1bf38b9`. IMPLEMENTED, no migration** (the availability table is already a ledger). *Measured as Jane: "Availability history ›" on the profile → `/squad/{id}/availability`: "6 changes on record", newest first, "7 Sept 2026, 17:09 · Modified · current · No sprinting · Available → Modified · Restrictions now No sprinting · Injury-linked · Ruth Callaghan"; Export CSV at `…/availability/export`; no sideways scroll.*
- **Build after the merge (reviewer machine):** 126 suites passed, 0 failed; `next build` exit 0.

**Ninth builder queue, 2026-09-13 ~10:00 — `52b6f82`, merged `fe014fb`; dev cache cleared, full build, dev server restarted.** Decision sheet group (d).

- **ATH-ADULT-09 C1 / 10 C1 / 11 C1 — `52b6f82`. IMPLEMENTED, with the two tokens (Decisions Log above).** The logger rebuilt set by set: one exercise at a time; the two numbers at `--fs-48` between `--hit-md` steppers; one `--hit-lg` primary labelled with what it writes ("Log set 3 · 5 reps"); chips as the state display (logged = accent + ✓ and a button, current = wash + ring, not reached = faint spans — nothing disabled); "Finish early" beside the title while sets remain; the correction in place ("Correcting set 1 · was × 5", footer "Save correction / Cancel"); the 8 Sept `.gym-ex-card` / `.gym-weight` / `.gym-stepper` / `.gym-correct` / `.gym-more` rules retired. Built differently: B3's chip-fill motion stays declined; the strip names the set that landed on every log; the session RPE field kept (completeSessionLog writes it). Recorded on the sheet: 13 C2 — a logged set of an earlier exercise is reached through the strip or, once closed, "Correct a set". *Measured as Conor at 375 on Lower A (page load): "Finish early" beside the title, "0 of 12 sets · 00:11", "Back squat · Set 1 of 3 · Rest 90s"; `.gl-primary` "Log set 1 · 100 kg × 8" 56 × 335 px at y = 748 of 812 (above the fold); `.gl-step` 52 × 52; the two values "100 kg" and "8" at 48px / 800; `--hit-lg` 56px and `--hit-md` 52px resolve on the root; 0 disabled or aria-disabled controls on the screen.* Left by the builder on scratch: James Barnes's 12 Sept Upper B has bench set 1 corrected to 0 kg × 5 (the null-minus bug before its fix — a real row) and a 13 Sept log with four bench sets.
- **Build after the merge (reviewer machine):** 126 suites passed, 0 failed; `next build` exit 0.

**Tenth builder queue, 2026-09-13 ~11:15 — `c39ed7e`, `6db21d1`, `f492f23`, fast-forward; dev cache cleared, full build (127 suites passed, 0 failed; `next build` exit 0), dev server restarted. Migrations 0109 and 0110 on scratch (`0110, 0109, 0108, 0107, 0106, 0105` read back); production stays at 0105 — the next deploy carries 0106–0110.**

- **PATTERN-S7 (Reports and analytics) — `c39ed7e`. RECORDED; A1 IMPLEMENTED:** `/reports` grouped under two eyebrows. Step 1 answered from the code in the records file (every export is audited; two renderers — C4 ⚠ decision; the export routes read `?groups=` only — C5; no weekly analytics grain; the §0ad cutoff not said in words — C7). C1–C11 and D1–D3 on the sheet; D1 ("out of 5") declined. *Measured as Jane at 1280: "ABOUT THE SQUAD OVER A PERIOD" — Compliance, Injury & availability, Training report, Squad weekly; "ABOUT ONE ATHLETE, SESSION OR TEST" — Athlete report, Testing.*
- **§0bd — `6db21d1`. IMPLEMENTED, migration 0109 (scratch only).** Closed above with the reviewer's re-probe. Left by the builder: F-11 (the three Users-screen refusals as `disabled` + `title`) not in this commit; Peter Ackland's `claims_version` at 6 from the verification (scratch).
- **§0bc — `f492f23`. IMPLEMENTED, migration 0110 (scratch only).** Closed above with the reviewer's re-probe. The app half: the refusal sentence, the flagged (never retried) outbox item, Today's notice with Discard only. Three pgTAP fixtures reordered (they had inserted sets into complete logs).
- **Build after the merge (reviewer machine):** 127 suites passed, 0 failed; `next build` exit 0.

**Eleventh builder queue, 2026-09-13 ~11:45 — `b636b21`, `d4fee2d`, `6d3b3f0`, fast-forward; dev cache cleared, full build (129 suites passed, 0 failed; `next build` exit 0), dev server restarted.**

- **§0al (reopened) — `b636b21`. IMPLEMENTED.** Closed above with the reviewer's re-run. `test-schedule-offline-publish.ts` §2b (10) pins the humanised path.
- **STAFF-SS-01 C2, the role versions — `d4fee2d`. IMPLEMENTED.** `dashboardVersion(roles)` → full / S&C / nutritionist; the S&C's four tiles (Need you on the load domains, Gym today, Weigh-ins, Available), the nutritionist's two (Need you on the nutrition domain, Weigh-ins); no week strip for either; the Ready-for card absent for the nutritionist (access-matrix §4.2 outranks the board — sheet D6); the Flags badge counts the version's domains. *Measured on Harlow Vale: Amara (S&C) — "NEED YOU · 0 · load readings only", "GYM TODAY · — · No gym session today · Schedule ›", "WEIGH-INS · 0 of 13 · 13 not submitted this morning", "AVAILABLE · 0 / 0"; no week strip; the lead card present. Laura (nutritionist) — "NEED YOU · 0 · nutrition readings only", "WEIGH-INS · 0 of 13"; no week strip, no lead card.* Sheet D7 added (body mass as a flaggable metric — a decision).
- **STAFF-SS-01 C2, the matchday lead card — `6d3b3f0`. IMPLEMENTED.** The readiness card moved to the top as the lead: "Ready for Saturday", the fixture as the title, the denominator said, Full / Doubtful / Ruled out, one athlete a row, the reason beneath for the medic only (`fetchSelectionReasons` called for `CLINICAL_ONLY` only); absent with no fixture inside 14 days, when the week strip says why; the old right-column readiness card, ring and rows retired (MET-014's ring gone — sheet D8 recommends retiring the entry). *Measured as Priya on Harlow Vale at 1280: `.card.dash-lead` first in main — "READY FOR SATURDAY · v Wexcombe Ladies · Sat 19 Sept, 14:00 · home · 0 of 13 have a current status · 13 not recorded · MD in 6 days · Whole squad · 0 FULL · 0 DOUBTFUL · 0 RULED OUT · Nobody this week ×2 · Flags affecting selection 0 › · Sessions left to run 0 ›".* Sheet D4 struck (the availability reason category is readable by every staff role at the database; withholding it is a display rule). Remaining in the C2 series: the week strip yielding on a heavy morning.
- **Build after the merge (reviewer machine):** 129 suites passed, 0 failed; `next build` exit 0.

**Twelfth builder queue, 2026-09-13 ~12:20 — six commits `abcb20c` … `14e4bd3`, fast-forward; dev cache cleared, full build (131 suites passed, 0 failed; `next build` exit 0), dev server restarted.**

- **STAFF-SS-01 C2, the week strip yields — `abcb20c`. IMPLEMENTED; the C2 series complete (sheet row struck; D6–D8 carry the open differences).** `weekStripYields` at five attention athletes with the matchday card leading; the strip never yields while it IS the lead. *Not reproducible by a page load today: Ashcombe has no fixture inside 14 days (the strip is the lead — "NO MATCH IN THE NEXT 14 DAYS · NEXT FIXTURE THU 1 OCT V WALKTHROUGH RFC · 18 DAYS", `data-lead="true"`, measured as Jane) and Harlow Vale has the fixture but 0 athletes needing attention. `test-staff-ss-01-dashboard.ts` pins it.*
- **STAFF-SS-01 C2 correction — `adf6748`. IMPLEMENTED.** The nutritionist reads the lead card's three counts with no names — migration 0074 (2026-09-06) admits the nutritionist to the censored injury and availability views; `access-matrix.md` §2 / §4.1 / §4.2 and `01-dashboard.md` corrected from the live policies ("the code is the fact, the doc is the bug"). Sheet D6 struck. *Measured as Laura on Harlow Vale: the lead card present — "v Wexcombe Ladies · Sat 19 Sept, 14:00 · home · 0 of 13 have a current status · 13 not recorded · MD in 6 days", 0 FULL / 0 DOUBTFUL / 0 RULED OUT, and 0 athlete links inside it.* Yesterday's record of `d4fee2d` ("the Ready-for card is withheld from the nutritionist") is superseded by this line.
- **STAFF-SS-02-05 Step 1 — `2dfa01c`. RECORDED (docs only).** The per-role profile inventory answered from five single-role sessions and the live policies, in `docs/overnight-records-2026-09-12.md`; C1 unblocked; C4's remaining decision is the four role orders (a recommendation on the sheet). The one finding was the 0074 gap above. *No spot-check — a review.*
- **STAFF-SS-02-05 C1 / B1, the status header — `fb12186`. IMPLEMENTED.** One emphasised card (`.pp-hero`, --wash-accent on --border-accent-soft) that absorbs the plan bar: the sub line, the restriction line with the coach-visible note and the expected return, the owner line as the eyebrow, the plan line with its link; `.pp-banner` gone. *Measured as Jane on Viliami Tameifuna at 1280: `.pp-hero` present, background rgba(23,72,155,0.12), border rgba(23,72,155,0.42), no `.pp-banner`; "Tighthead prop · #3 · Forwards | No sprinting. | SET BY MEDICAL STAFF · RUTH CALLAGHAN · MON 7 SEPT | … | Development plan · Pre-season strength · week 6 of 8 · ends Thu 1 Oct | Change plan".*
- **PATTERN-S4 C5 — `dd98ed4`. IMPLEMENTED.** "N of M athletes are expected" in the panel's preview footer, distinct athletes with the squad as the denominator; "Nobody is expected — staff only" for a session with no group. Open on the sheet: whether an unavailable athlete is still expected (today they are). *Measured as Mark, week of 28 Sept, the fixture session selected in Edit mode: "Publishes to Forwards + Backs · 29 of 30 athletes are expected · appears under Today on the morning of Thursday 1 Oct"; the group-less "Walkthrough session" reads "Staff only · this session is never published to the athlete app". Nothing changed or published.*
- **PATTERN-S4 C8 — `14e4bd3`. IMPLEMENTED.** A group with no session this week reads "—" and "N athletes · nothing scheduled"; a day still reads 0m. *Measured as Mark, week of 28 Sept: "Rehab · — · 5 athletes · nothing scheduled", "Academy · — · 5 athletes · nothing scheduled".*
- **Build after the merge (reviewer machine):** 131 suites passed, 0 failed; `next build` exit 0.

**Thirteenth builder queue, 2026-09-13 ~13:00 — six commits `4add291` … `2c741b7`, fast-forward; dev cache cleared, full build (135 suites passed, 0 failed; `next build` exit 0), dev server restarted.**

- **PATTERN-S4 C6 / B4, the phone schedule — `4add291`. IMPLEMENTED.** Day-first below 768: the week strip as 74px tiles (day, date, MD offset, minutes; today selected), the day as 44px rows linking to the session pages, a held row listed not linked, a 44px "+" to `/schedule/new?date=`, "Nothing on Monday.", the Next line; the grid and the Read/Edit segment not drawn on the phone; the S4 rows that fell with D1 (C2/B1/B3, C3, C9/C10) struck as fallen. Built differently: the board's "Gym and rehab sessions … are on the Gym tab" dropped (staff have no Gym tab); the week nav, banner and template toolbar stay above the day view; day selection is local state. Frames 12–14 (the phone session detail, the sheet form) are not this row. *Measured as Mark at 375, week of 28 Sept: `.sg-phone` drawn, `.sg-desktop` display none, the Read/Edit segment's parent display none; seven 74×74 tiles ("Thu 1 Oct MD · 80m"); "Monday 28 Sept · Nothing scheduled · + · Nothing on Monday. · NEXT · Thu 1 Oct · Walkthrough session · 10:00 ›"; the + and the Next row 44px; `scrollWidth 375`.*
- **PATTERN-S5 C4 — `395119a`. IMPLEMENTED.** An assignment's headline is the distinct athlete count against the squad, with its arithmetic when groups and names overlap; the row count ("Assigned 2") gone. *Measured as Priya on Harlow Vale's "Pre-season strength": "Assigned · 5 of 13 athletes · Forwards" (one group alone, so no arithmetic line — as specified).*
- **PATTERN-S6 C6 / D2 — `971fbab`. IMPLEMENTED.** A refused publish for one session stays held: the grid draws the base position solid and the attempted one as a dashed "Did not save" ghost, the banner names both times with Try again; a dropped connection is not ghosted (§0al's line stands). *Not reproducible by a page load (needs a refused write — the builder used a service-role `updated_at` bump to trip the optimistic lock); `test-schedule-failed-write.ts` (20) is in the chain.*
- **PATTERN-S7 C5 — `dfff9df`. RECORDED — already true.** All five export routes resolve the scope through `resolveGroupFilter` (URL, then the sticky cookie); pinned by `test-pattern-s7.ts` C5 (12). *Source-pinned; no render (an export writes an audit row).*
- **PATTERN-S7 C7 — `3efddca`. IMPLEMENTED.** *Measured as Jane on `/reports/compliance`: "An RPE counts when it was sent before the end of the following day, club time — the moment the athlete app stops accepting one. A wellness check-in has no cutoff: it counts on the day it was expected, whenever it was sent." under the summary figures.*
- **PATTERN-S7 C9 — `2c741b7`. IMPLEMENTED.** The compliance window walks by its own length (never past today; a season, a year and "all" are not walked); the narrow-window note (100/N, for windows of 14 days or fewer). Only the compliance report walks (the one report with a day anchor). *Measured as Jane with `?period=week`: "‹ Previous 7 days" (50px) → `?period=week&to=2026-09-06`, no Next (the window ends today), and "Over 7 days one missed morning moves an athlete's rate by 14 points — read four weeks for the habit, 7 days for this week."* Seen, not filed: Jane's default landing on the report is the season window (her `fydr-period` cookie), so the walk is absent until a day-length window is chosen.
- **Build after the merge (reviewer machine):** 135 suites passed, 0 failed; `next build` exit 0.

**Fourteenth builder queue, 2026-09-13 ~13:40 — five commits `2d2f846` … `847fe77`, merged `813d3eb`; dev cache cleared, full build (139 suites passed, 0 failed; `next build` exit 0), dev server restarted. PATTERN-S8 (Settings, users and club setup) arrived and is committed (`94143d4`) for the builder.**

- **ATH-ADULT-12 B2 — `2d2f846`. IMPLEMENTED.** Prior completed gym weeks in `--wash-accent-strong`, the latest in the accent, a partial week its own mix, a zero week its baseline rule; 12 B4, SS-01 B1 and SS-01 C5 struck as already done. *Measured as Dan: the three prior-week bars carry `data-prior` but are also zero weeks (3px baseline in the muted mix — the rule the handover names); `.gb-bar[data-prior] { background: var(--wash-accent-strong) }` at `base.css:1906`, `--wash-accent-strong` = rgb(23 72 155 / .16). The builder's own render (James Barnes, with logged prior weeks) shows the wash.*
- **PATTERN-S7 C8 — `f163c86`. IMPLEMENTED.** One squad floor, `MIN_ATHLETES_WITH_DATA = 5` (`lib/smallSample.ts`): the positional band and the row shading repointed; the training report's heat ramp, the testing medians, the compliance squad mean and the analytics builder's population mean go off below it with one note. S7 D1 struck as declined. *Measured as Priya on Harlow Vale's compliance (By athlete): "SQUAD MEAN 0% · n = 13 athletes" — thirteen have expectations, so the floor does not bite there; the sweep is pinned by `test-squad-floor.ts` (15).*
- **ATH-ADULT-13 C2 — `4d86119`. IMPLEMENTED.** One correction component: the detail's set rows link "Correct ›" into the logger (`/gym/[sessionId]?log=…&correct=…`); `startOrGetSessionLog` reads a named log by id, never creating; a log not on record is a 404; the table's own inline form (a second `revise_gym_set_log` path) is gone. *Measured as Dan on his 12 Sept Upper B detail: 11 links "Correct set N of …" → `/gym/<id>?log=<id>&correct=<id>`, 0 forms on the page.*
- **ATH-ADULT-12 C5 — `7ebfce0`. IMPLEMENTED.** The gym hero is the best lift and how it moved — MET-040 inside the period against before it; an empty period in words. *Measured as Dan, this season: "BEST LIFT · 147 kg · Weighted pull-up best · × 6 · Sat 12 Sept · no earlier best to compare — the first Weighted pull-up logged · 25 working sets in this period across 6 exercises."* Seen, not filed: the Sessions card beneath still reads "2 of 1 assigned".
- **PATTERN-S7 C2 (compliance) — `847fe77`. IMPLEMENTED — the first of six reports.** Every figure with its denominator, an exclusions sentence, missing values as words ("Not expected", "None", "No entry in this window"); no dash left on the report. *Measured as Jane, 28 days: "1% · Wellness · 11 of 815 submitted", "2% · Session RPE · 4 of 203 submitted", "Not expected · Gym · No expectations configured for this domain"; 0 em-dash cells in the table.* Seen, not filed (for the resumed run's step 17): Harlow Vale's 7-day report reads "0% · Wellness · 0 of 13 submitted" although Megan's 12 Sept entry exists, and "n = 13 athletes · 1 days".
- **Build after the merge (reviewer machine):** 139 suites passed, 0 failed; `next build` exit 0.

**Fifteenth builder queue, 2026-09-13 ~14:15 — six commits `c4494d4` … `6c6fff4`, fast-forward; dev cache cleared, full build (140 suites passed, 0 failed; `next build` exit 0), dev server restarted. PATTERN-S7 C2 is complete across the six reports.**

- **PATTERN-S7 C2 (injuries) — `c4494d4`. IMPLEMENTED.** The availability figure says who it counts oddly; missing values in words. Said, not corrected: recomputing the denominator from `joined_at` would change MET-013 (a registry decision). *Measured as Jane, 28 days, Period summary: "AVAILABILITY · 85% · 718 of 840 athlete-days · Nobody is excluded. 2 athletes have no recorded status and are counted as available."; the Now view reads "Site not recorded", "Return not known", "No position set" with 0 dash cells.*
- **PATTERN-S8 (Settings, users and club setup) — `ac11b0d`. RECORDED; A1–A7 IMPLEMENTED:** Log out as a 44px button with "Ends this session on this browser only" (48px on a phone); settings rows 52 / 64px; Apple Health "Not available yet · needs the Fydr iOS app"; the exports intro names the role through `staffRoleLabel`; the audit log says what it cannot show (sessions and schedule changes); the group reorder arrows 44×44; Catapult reads "Import files", not "Connected" — **which closes F-04 of the test-club run**. Step 1's eight questions answered from the code; C1–C13 and D1–D9 on the sheet; D7 answered (a role change lands on the next request — 0010/0109). *Measured as Jane on `/settings`: `button.set-logout` 44×77 with its note; rows 70 / 71 / 71px; "Catapult Openfield · GPS session files, CSV import · Import files".*
- **PATTERN-S7 C2 (squad weekly) — `b66f43a`. IMPLEMENTED.** *Measured as Jane: `.sw-kpi-sub` "8 of 380 submitted", "over 29 of 30 athletes with an entry", "10 not fully available", "across 30 athletes"; "Nobody is excluded."; 0 BLANK.*
- **PATTERN-S7 C2 (training) — `0c34c61`. IMPLEMENTED.** *Measured as Jane: "n = 21 athletes · 9 of 30 in this filter have no GPS record for this session and are not on the board"; 26 word-cells (No data / No best yet / Not set / Result not entered), 0 dash cells.*
- **PATTERN-S7 C2 (testing) — `b967955`. IMPLEMENTED.** *Measured as Jane, this season, By test: "25 of 30 athletes have a result for this test in this window; 5 have none and are not ranked. Median 1.7 s · Q1 1.7 · Q3 1.8"; 97 "No result" cells in the by-athlete table.*
- **PATTERN-S7 C2 (athlete report) — `6c6fff4`. IMPLEMENTED — the sixth of six; the sheet row struck.** *Measured as Jane on James Barnes: "COMPLIANCE · 11% · 4 of 35 submitted · last 28 days", four "Building baseline" tiles; the one remaining " — " on the page is the period control's own explanatory copy ("Today — readiness is read against a 14-day baseline"), not a value.*
- **Build after the merge (reviewer machine):** 140 suites passed, 0 failed; `next build` exit 0.
...
**The security claim, as stated by the builder for verification** — for any email and any wrong password, `POST /auth/sign-in` returns at attempts 1–4 a `401` with byte-identical body `{ok:false, locked:false, error, attemptsRemaining: 5−n}` whether or not the email has an account; at attempt 5 a `429` `{ok:false, locked:true, lockedUntil, secondsRemaining: 30, error}`, identical either way; while locked a `429` before any auth call; native submits `303` to `/login?e=invalid&a=5−n` for both. Every answer after the auth call is held to ≥ `FAILED_SIGN_IN_MIN_MS` (800ms) from request start — `cfb22a2` extended the hold to the threshold-crossing 429, which the first commit had left unheld. **Why it holds:** `login_attempt_record_result` (0048) keys on the email alone and never queries a users table; the 401 body is built from the limiter's record only; the real-account-only audit row moved into `after()` so bcrypt is masked by the floor. **Guard:** `test-sign-in-attempts-left.ts`, 33 assertions, in prebuild — SQL body free of `users`/`auth.users`/`athletes`, byte-identical bodies at the same step, `startedAt` read before auth, `holdUntil` applied exactly twice, audit inside `after()` and never awaited, floor ≥ 700ms.
...
## 0ae. SECURITY — `user_roles` at the database: part 1 (last admin) CLOSED by 0101; part 2 (self-grant of medic) CLOSED by 0102, on production since the 2026-09-11 deploy

**Reported to Isabella directly, per the role-gate rule. Priority alongside §0x.**

**Part 1 closed — `8632c4b`, migration `0101_user_roles_guard.sql`, merged 2026-09-11 and applied to scratch by the builder.** A `SECURITY DEFINER` trigger `user_roles_guard` refuses any DELETE or UPDATE that would leave a row's organisation with zero `sport_scientist` rows — for every caller, service role included — with `supabase/tests/570_user_roles_guard_test.sql`. Verified present on scratch by the reviewer (`pg_trigger` lists `user_roles_guard`).

**Part 2 stays open**, and the trigger's own comment says so ("NOT HERE: the second half of §0ae, refusing a self-grant of medic. It waits…"). Confirmed live below.

- [x] **CLOSED by `0102_user_roles_no_self_medic` (trigger `user_roles_guard`, `tgtype 31`, confirmed on production 2026-09-11; UI chips `8632c4b`/`73392a2`; test 580 in prebuild). Scope: the self-grant of `medic` — the one role that opens `CLINICAL_ONLY`. Self-granting coach, S&C or nutritionist is still possible and grants nothing the sport scientist does not already hold, so it is not pursued. Original entry follows.** ~~A sport scientist can grant ANY role, including `medic`, to THEMSELVES.~~ TESTED LIVE 2026-09-11 as Jane Pemberton, through her own browser session and RLS, after 0101 was applied:** `POST /rest/v1/user_roles {user_id: <self>, role: 'medic'}` → **201 Created**; her roles read back `["medic", "sport_scientist"]`. Removed immediately: `DELETE` → 200, roles back to `["sport_scientist"]`. The test left two audit rows (`user_roles.insert` and `.delete`, role `medic`, 14:03) and bumped her claims version, which signed her out as designed. **0101 does not cover this** — its own comment says the self-grant half "waits". Part 2 is open and confirmed, not inferred. `user_roles_admin_insert`'s `WITH CHECK` is `org_id = auth_org_id() AND auth_has_any_role('sport_scientist')` — nothing about `user_id`, nothing about which role. The only triggers on the table are `user_roles_audit` and `user_roles_bump_claims_version`; neither refuses anything. `setUserRoles` has no self or medic check. Measured on the running app: on Jane Pemberton's own row, the "Medic" toggle is `aria-pressed="false"` and **enabled** — the escalation is one click in the UI, not even a console call. (Not performed: it writes a permanent audit row and bumps her claims version.)
...
- [x] ~~**`setUserRoles` refuses to remove the last `sport_scientist` in an org — but it runs in the browser, and nothing at the database enforces the same rule.**~~ **Closed by 0101.** Original entry follows. `src/lib/queries/userManagement.ts:setUserRoles` counts remaining admins and returns "This is the only admin in the club — remove the role from someone else first…" when the count would fall to zero. It is called from `UserDetailPanel.tsx:95` and `UserManagementPanel.tsx` with the **client** Supabase `db`. So the guard is JavaScript in the admin's own browser.
...
- [x] **BUILT `af09c17` (reviewer, 2026-09-12 ~11:50, fast-forward merge to `1052354`): below 768px the sidebar is `display: none`, a 64px `.ph-titlebar` (page name · active group from the §0ak cookie), a `.ph-tabbar` of Dashboard / Squad / Schedule / a role slot (Flags for SS, coach, medic; Gym for S&C; Nutrition for the nutritionist) / More, and a More sheet that is a real disclosure (`aria-expanded`/`aria-controls`, `role=dialog`, focus in on open, Escape returns focus); the nine destinations come from one data module (`Sidebar/rows.ts`) so bar and sidebar cannot disagree; the 44px floor for staff controls below 768px lands here generically (`.back-btn`, `.btn-*`, `select`, the steppers, lens tabs, theme segment, reorder buttons; inline links get a 44px hit box by padding + negative margin). Desktop untouched (sidebar 236px at 1280). *Reviewer check as Jane at 375 on `/dashboard`: sidebar `display: none`, five tabs at 58px with `aria-current="page"` on Dashboard, title bar 64px "Dashboard · Whole squad", More `aria-expanded=false` → `ph-sheet`. Second check on Isabella's ask, same page: bottom bar at 734–812 of 812 (inside the viewport); the More sheet opens as `role="dialog" aria-modal="true"` with focus landing on "Reports", seven rows at 52px; Escape hides it, `aria-expanded` returns to false and focus returns to More; the dashboard heading sits at 152px under the 64px title bar and the first card starts at 470px.* The board's eight missing tokens each mapped to an existing one (B1); C2–C5 and the phone group-chip question recorded, not built. 59 assertions (`test-staff-phone-shell.ts`) in prebuild. Original finding and the superseded decision follow.** ~~Below 768px the sidebar stacks full-width above the content, 640px tall.~~ **DECIDED 2026-09-12 (Isabella), superseding the 2026-09-11 shape: below 768px the staff shell is a bottom bar with a "More" sheet — the shape of the approved STAFF-SS-01 board (`docs/designs/STAFF-SS-01 final/`), which decides that board's D1.** ~~DECIDED 2026-09-11 (Isabella): below 768px, a compact top bar with the navigation behind a menu control.~~ The STAFF-SHELL brief's §2 and §3 describe the top bar and are marked superseded; everything else in it (the 44px floor at shell level, the sweep table, the menu-as-disclosure and content-starts-at-the-top constraints, the tokens rule) carries over to the bottom bar unchanged. Shell-level brief written — `docs/design-briefs/STAFF-SHELL - Phone layout below 768px.md` — and it goes to Claude Design **before any staff flow is implemented**. Original finding follows. `base.css`: `@media (max-width: 767px) { .sidebar { position: static; height: auto } }`, with the comment "the sidebar stacks full-width above the content". Between 768 and 1023px there is a 64px collapsed rail; below 768 nothing collapses. Measured at 375×812 as Jane Pemberton: the Dashboard heading at y=728, its first content section at **y=2209**, page 3,426px; Squad overview heading at y=737. Staff use phones pitch-side, and every screen opens on nine navigation rows and "Log out" before any content. **Deliberate as built and documented; wrong for the persona.** The 64px rail already exists and would carry down; a disclosure would also do. **Design question, not a defect** — filed here so the answer is made once for the shell, not per flow.
...
## 0ah. A session can be created with no duration, and a null duration now makes its RPE due mid-session — 2026-09-11

- [x] **`NewSessionForm` submits `durationMin: null` when the field is cleared, and nothing refuses it.** The form is `noValidate`; the input has `min={5} max={240}` and a default of `60` but no `required`; `createSession` inserts `duration_min: input.durationMin` unchanged; `sessions.duration_min` is nullable with no default. Verified 2026-09-11 in source at each step. **Scratch holds 0 of 60 sessions with a null duration** — the seed always sets one — so no existing session is affected today. **CLOSED at the form, `586520d` (reviewer, 2026-09-12 morning, merged `3103ad9`): the duration input renders `required`, `min=5`, default 60 — measured on `/schedule/new`. The column stays nullable (Builder question 1) and the week-template path can still produce a null (question 2): both for Isabella, recorded in `docs/overnight-questions-2026-09-12.md`.**

  **Why it became a defect on 2026-09-11.** The ATH-ADULT-02 build made the RPE due time `starts_at + (duration_min ?? 0) + 30 min` (`lib/rpeDue.ts`). For a session with no duration that is thirty minutes after kick-off — the athlete's Today shows "Rate …" while they are still on the pitch, and the RPE screen accepts a rating for a session that has not finished. Before the build, the RPE screen skipped its due gate for those sessions; the builder noted the tightening.

  **DECIDED 2026-09-11 (Isabella): make duration required at creation.** Add `required` to the field (and drop `noValidate`, or validate in `onSubmit` and refuse), have `createSession` reject a null, and — since the column is nullable for existing rows elsewhere — decide whether to add a `NOT NULL` with a backfill or leave the column as is and guard at the boundary. The default of 60 stays; the field just cannot be emptied.
...
- [ ] **The schedule's "Week plan" view tab is a `<span>`.** `.sg-viewtab` for the current view is a plain span with no role and no `aria-current`; "Today" beside it is a link. A view switcher where the current view is not a control tells assistive tech there is one tab, not two. Give the pair `role="tablist"`/`tab` semantics or at least mark the current one. **PARTLY CLOSED `b3cbaab` (PATTERN-S4 A) (reviewer poll 2026-09-12 19:50, merged `22f06d7`): the current tab carries `aria-current="page"` and both carry `role="tab"` — *measured as the coach at 375*; still a `<span>` for the current view (not a control), and there is no `tablist`.**

- [x] **`NewSessionForm` validates name, date and time only.** A session can be created with no type, no groups, no location and (§0ah) no duration. Type and groups at least should be refused — a session with no group has no expected attendees, and the dashboard's "Sessions left to run" and compliance both key on them. Decide which of the six are required and validate them in `onSubmit` the way name and date are. **CLOSED at the form, `eff27bb` (reviewer, 2026-09-12 morning, merged `3103ad9`): submitted `/schedule/new` with a name and no group — refused in place, `role="alert"` "Choose at least one group.", focus moved to "Who's in it", no navigation; type defaults to Training and is refused when empty. The grid's "Add to day" can still stage a group-less session (Builder question 4, for Isabella).**
...
- [x] **The schedule panel's "What the athlete sees" tells staff an RPE is "due by 19:45" for every training session, whatever its time.** `scheduleGeometry.ts:125`, `EXPECTS.training: 'RPE due by 19:45'` — a fixed string per session type carried over from the spec's §6 mockup. It was always a placeholder; since `lib/rpeDue.ts` landed (ATH-ADULT-02, `cfd410b`) the real due time is `starts_at + duration_min + 30 min`, so a 09:00–10:00 session is due 10:30 and the preview says 19:45. The line's whole purpose is to show staff what the athlete will be told; it now shows something the athlete is not told. **Fix:** compute it from `rpeDueAt` for training and match sessions; the other types' strings are fine. **CLOSED `5f68cb6` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): `scheduleGeometry.ts:146` `expectsLabel` reads `rpeDueAt` — "RPE due from HH:MM" ("from", because a rating is first accepted 30 min after the session ends); read in source. 10 assertions in prebuild.**

- [x] **"Yes, remove" on a staged draft says "You can undo with Discard, until you publish" — there is nothing to undo to.** A draft that has not been published vanishes on removal (no ghost, no Restore — the "same act as Discard" SS-10 records), so the promise is empty for exactly the case where it is shown. Either branch the copy on `isDraft` or drop the second sentence for drafts. **CLOSED `b2062d3` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): `SelectedSessionPanel.tsx:559` branches on the draft — "Remove this draft? It was never published, so there is nothing to undo."; read in source.**
...
- [x] **`/squad` writes the filter to the `fydr-group-filter` cookie; `/reports/squad` writes it to `?groups=` and never to the cookie.** The report reads the cookie as its default when the URL is bare, so a filter set on Squad overview appears on the report — but a filter set on the report is gone by the time the sport scientist is back on Squad overview, which shows "Whole squad · 30 athletes" again. Measured both directions. **CLOSED `f2b72ea` (reviewer, 2026-09-12 morning, merged `3103ad9`): exercised both directions as Jane — Forwards pressed on `/reports/squad` (`?groups=…000001`), then `/squad` bare opened on "✓ Forwards" (16 athlete links); Backs added on `/squad`, then `/reports/squad` bare read "Backs + Forwards · 29 athletes". Reset to Whole squad.**
  ...
  **DECIDED 2026-09-11 (Isabella): the group filter is shared across every multi-athlete screen, both directions.** Every such screen **reads and writes the same cookie** (`fydr-group-filter`). A `?groups=` URL parameter **overrides only for that page load** — it exists for shared links — and does not write the cookie. So: pressing a chip anywhere writes the cookie and applies everywhere; opening a link with `?groups=` shows that scope once, and the next screen without the param is back on the cookie. The report's current behaviour — writing the URL and never the cookie — is the defect against this rule; `/squad`'s is the reference.
...
## 0al. Two findings from exercising the schedule publish on scratch — 2026-09-11; the first is HIGH PRIORITY, above design work

**DECIDED 2026-09-11 (Isabella):** the offline-publish wipe is **high priority, above all design work**. Fix direction: **persist pending edits in `sessionStorage`, and never `router.refresh()` after a network failure.** The `sessions` table goes into **the next audit-trigger batch** (with §0p/§0q's shape).


**The exercise, so the scratch state is known.** As Jane Pemberton, a draft "Reviewer publish test" was staged through the toolbar wizard for **Sunday 13 Sept, 09:00–10:00 club time, training, group Academy** (five members; neither Conor Moroney nor Matt Reid), and **published** — `sessions` row `40edd0b4-e4d6-4966-866f-e499307dee85`, one `session_participants` row (group Academy). Verified reaching an Academy athlete: signed in as Elliot Wray, his Today's week strip showed "S 13 · Training · MD+1" where Sunday had held nothing (confirmed: the only non-deleted session on 13 Sept was this one). Then, back as Jane, the session was removed in the grid — which produced the ghost, "Removed on your screen. Athletes still see this session until you publish." and "Restore session" that STAFF-SS-11 documents and the earlier batch could not reach — and the removal published. **Left behind:** the row, soft-deleted (`deleted_at` 14:33:36), per "work on the app, not the data"; three `auth.signed_in` audit rows (Elliot once, Jane twice); Elliot's Sunday empty again. **"Publishing…" disabled in flight** verified.

- [x] **On a network failure at publish, every pending change in the week is lost and the "Not published:" line is wiped before anyone reads it.** `ScheduleWorkspace.tsx` `publish()`: the writes go through the browser Supabase client; offline they throw, the `catch` sets "Not published: Failed to fetch" — correct — and then `finally` calls `router.refresh()` **unconditionally**. Offline, the refresh's own RSC fetch fails and Next falls back to a full browser navigation (`[browser] Failed to fetch RSC payload for /schedule. Falling back to browser navigation. TypeError: Failed to fetch`, then `GET /schedule 200`). The reload destroys `edits`, `added`, `removed` and `newDraft` — all `useState`, nothing persisted — and the error message with them. **Reproduced 2026-09-11**: draft staged, network emulated offline, Publish pressed; the page came back on "The athlete app is up to date" with no draft and no alert, and no row written. The document's "the button returns to enabled" holds only for a server-side error, not a network drop — and a pitch-side phone drops the network, not the server. **CLOSED `71035f5` (reviewer, 2026-09-12 morning, merged `3103ad9`): `ScheduleWorkspace.tsx` round-trips pending edits through `sessionStorage` (`readPending`/`writePending`/`clearPending`, `pending.ts`) and `router.refresh()` runs only `if (!networkFailed)` — read in source; the offline exercise not repeated.** **REOPENED 2026-09-12 22:05 — the offline exercise, repeated on Harlow Vale (test-club run step 11), fails the second half.** As the coach at 1280 with CDP network offline, one held stepper change (Gym A 07:00 → 07:15), Publish to athletes: within 1.5 s the tab was on `chrome-error://chromewebdata/` (ERR_INTERNET_DISCONNECTED) — the full browser navigation §0al describes — so the "Not published:" line was never on screen. The half that holds: after reconnecting and reloading, the banner read "1 change not yet in the athlete app" with the block "07:15 EDITED" — `sessionStorage` kept the week. **Why the guard does not fire:** `updateSession` (`schedule.ts`) returns its error already humanised — `humanizeDbError` turns "Failed to fetch" into "That didn't save — the connection dropped or timed out…" (`writeErrors.ts:149-155`) — and `isNetworkFailure` (`pending.ts:116`) matches only the raw engine strings (`failed to fetch`, `load failed`, `networkerror`, `network request failed`), so `networkFailed` stays false and `finally` calls `router.refresh()` offline. The same is true of `createSession`'s and `deleteSession`'s returned errors. Only a *thrown* raw fetch error takes the guarded path. **Fix:** have the three helpers return a typed reason (or the raw message alongside the sentence) and test `isNetworkFailure` against it; or match the humanised sentences too. `scripts/test-schedule-offline-publish.ts` should assert the humanised path, which is the one the helpers actually take. My closure was a source read; the memory rule is render it.
  **CLOSED `b636b21` (builder, 2026-09-13): `isNetworkFailure` also asks `saysConnectionFailed`, exported by `writeErrors.ts` — the sentence's own module recognises it, so rewording cannot break the guard again.** *Re-run by the reviewer as Mark Iremonger at 1280, week of 28 Sept: one held stepper change, CDP offline, Publish — at 1.5 s and 6.5 s the tab was still on `/schedule?date=2026-10-01`, the banner read "Not published: Walkthrough session: That didn't save — the connection dropped or timed out. Check your connection and try again." with "1 change not yet in the athlete app" and the button back to "Publish to athletes"; back online, Discard → "Yes, discard" → "The athlete app is up to date", `sessionStorage` empty. Nothing reached scratch.* The builder's own note that his script's first Discard click did not clear the held change: Discard is a two-step control (Discard → "Yes, discard"), measured here; not a defect.


  **The comment justifying the unconditional refresh is right for a partial failure** (some sessions written, some not — the grid must resync). It is wrong for a total one. **Fix, decided:** (1) persist `edits`, `added`, `removed` and `newDraft` to `sessionStorage`, restored on mount, cleared on a successful publish or an explicit Discard — so a reload, accidental or forced, never loses the week; (2) after a network failure (the `catch` path with a fetch error) **do not** call `router.refresh()` — leave the grid, the pending state and the "Not published:" line exactly as they are; keep the refresh for the partial-failure path where some writes landed. Guard: a source test that `publish()` does not refresh on a `TypeError: Failed to fetch`, and that the pending state round-trips through `sessionStorage`.

- [x] **Publishing a session to athletes writes no audit row. Neither does removing one.** `sessions` carries only `sessions_set_updated_at`; `schedule.ts` never touches `audit_log`. Measured after the exercise above: the window holds six `report.squad_weekly.view` rows and two `.export` rows from reading a report, and **nothing** for creating a session that appeared on five athletes' phones or for deleting it. Reading is audited; the staff app's single highest-reach write is not. Not an access gap — an accountability one: "who changed the week, when" is unanswerable today. **Decided: goes into the next audit-trigger batch** — a trigger on `sessions` and `session_participants` of the same shape as the athlete-entry ones (§0p, §0q, `0099`), recording create, update, soft-delete and participant changes with the actor. **CLOSED `f5a59c4`, migration 0104 (reviewer, 2026-09-12 morning, merged `3103ad9`): triggers `sessions_audit` and `session_participants_audit` present on scratch; actions `sessions.create` / `.update` / `.delete`, `session_participants.add`; the builder's test left one row of each on scratch. 21 pgTAP assertions (600). On scratch, not production.**
...
## 0an. Production state as of the 2026-09-11 deploy

- **Code:** `7df310e05b6807f6ed916a071824425922eb7b35` (`7df310e`, "STAFF-SS-23 to -25…", 2026-09-11 15:48) — the head of `origin/athlete-spec-builder` at deploy time. Carries §0w, §0x (both forms), §0ae parts 1 and 2, ATH-ADULT-01 (phone and desktop), C1, ATH-ADULT-02, and the gym corrected-marker (§0v).
- **Migrations:** applied **through `0102`** (`0101_user_roles_guard`, `0102_user_roles_no_self_medic`), confirmed read-only after the deploy. Both were confirmed forward-compatible with the previous code (`0510583`) before applying — see §0ae — and applied before the deploy, so the new UI's disabled self-medic chip was enforced from the first request. Pre-check: zero organisations without a sport scientist, so 0101 refused nothing on existing data.
- **C1 on production:** verified by Isabella — see the closed step under §0ac.
- **Deploy incident, recorded so it is not re-diagnosed:** an **accidental rollback** occurred during the deploy — the rollback block of the numbered sequence was pasted with the rest and ran — and production served `0510583` for a few minutes against a database already at `0102` (safe, see §0ae) until Isabella **re-promoted** `dpl_BPhPtr9igAFd7a7RWJKbZcvTcMqW` (`fydr-p42jd9tj0`) in the Vercel dashboard. Isabella's checklist for the day, with the outcome and the emergency-only section, is committed as `docs/runbook-deploy-2026-09-11.md`.
- **Current live deployment (read from Vercel, 2026-09-11 16:51 BST):** `dpl_5EoAexC2bSYkfrvVXJmc4Vqt6WPP` = `fydr-bkslf6d89-fydr.vercel.app`, a CLI deploy at 16:46 BST, same commit `7df310e`, holding all four aliases (`fydr.app`, `www.fydr.app`, `fydr.vercel.app`, `fydr-fydr.vercel.app`). It supersedes `fydr-p42jd9tj0` (the re-promoted build) and `fydr-jvgpijjra` (a 16:28 dashboard redeploy that still reported `iad1`) as "current live" in the runbook's rollback section.
- **Production moved on 2026-09-12 (read from Vercel and the database, read-only, ~11:55 BST):** deployment `dpl_69Lhi8HXSimabVmD5pp3Xz8RVY3m` = `fydr-dptdcm23k-fydr.vercel.app`, CLI deploy at 10:51 BST by Isabella, commit **`a58fd6b`** (which contains the builder's `3103ad9` — the whole overnight queue), `regions: ["dub1"]`, holding all four aliases; migrations applied **through `0105`** (0103 mute snapshot, 0104 session audit, 0105 `submitted_at` clamp). Not deployed: `94099bd` (`--gap-body`), `75c6a6e` (§0aa), `5003959` + migration **0106** (§0at), `af09c17` (STAFF-SS-01 shell) — all merged on `athlete-spec-builder` after `a58fd6b`.
- **Function region: Dublin (`dub1`), in effect from that deploy.** Measured 16:51 BST: the deployment reports `regions: ["dub1"]` and three POSTs to `fydr.app/auth/sign-in` answered `x-vercel-id: lhr1::dub1::…`. Before it, functions ran in `iad1` (Washington DC) by Vercel's default — measured on both earlier deployments of the day — which no document had recorded; `docs/09-security-and-compliance.md`'s sub-processor row now carries the correction. The 16:28 redeploy was made for the same purpose and did not pick the setting up (its record says `iad1`); the CLI deploy did.
  **Standing rule (Isabella, 2026-09-11):** if production ever moves to a London database, the function region moves to `lhr1` in the same change — the point of `dub1` is adjacency to eu-west-1, not Dublin itself.
- **Left on production by the verification:** `login_attempts` rows for the synthetic addresses `nobody.deploycheck@`, `warm.up@`, `nobody.c1check@`, `nobody.c1check2@`, `nobody.c1check3@` (all `@example.invalid`), and two placeholder strings pasted verbatim — `put.a.real.synthetic@ashcomberfc.example` and the literal `<synthetic account email>` — each at 4 attempts; four `auth.sign_in_failed` audit rows for `j.barnes@` (15:08:07–:14 UTC, re-confirmed read-only at 16:35 BST, followed by his `auth.signed_in` at 15:11:17 UTC, which cleared his `login_attempts` as designed). None locks anything; the synthetic addresses' rows age out of the 30-minute window.


## 0ao. Low priority, filed 2026-09-11 — the prebuild chain prints expected errors as if they were real

- [x] **Tests that simulate a failure on purpose let the code under test print to `console.error`, so every build log carries ~51 "error" lines and stack traces that mean nothing.** Vercel's build log flags them as errors. Measured on one suite: `npm run test:sign-in-audit` passes (80/80) and writes 21 lines to stderr — `sign-in audit write failed, sign-in itself unaffected Error: audit down` with a four-frame stack, then `last_seen_at stamp failed … Error: users down` with its own — because `recordSignIn` and `stampLastSeen` (`src/lib/signInAudit.ts`) log their swallowed errors, and the test feeds them a client that throws. Other suites do the same for their own failure paths. **CLOSED `dd02445` (reviewer, 2026-09-12 morning, merged `3103ad9`): `npm run test:sign-in-audit` now writes **0 lines** to stderr (was 21) and passes; the builder reports the whole chain silent.**

  **Why it matters at low priority:** a real error in the build log is indistinguishable from the fifty expected ones, so nobody reads them, so a real one goes unread. It costs nothing at runtime.

  **What to do:** a test that expects a failure captures `console.error` for the duration of the case (replace it, collect the calls, restore it in `finally`) and **asserts** on what was logged — the message is part of the behaviour under test, so asserting it is strictly better than printing it. Do not silence `console.error` globally in the runner: a suite that goes quiet by suppression would hide the very thing this item is about. Start with `scripts/test-sign-in-audit.ts` and grep the chain for the rest (`npm run build 2>&1 >/dev/null | grep -c Error` gives the count to drive to zero).

## 0ap. STAFF-SS-26 to -33 — filed 2026-09-11 from the sport scientist review (leaderboards, analytics, injuries, settings, audit, exports, print)

Reviewed as Jane Pemberton at 1280×800 and 375×812, read-only — no board, export, import, threshold or retention run created. Reviews: `docs/walkthrough-reviews/staff-ss-26-to-33-review.md`; briefs `STAFF-SS-26` to `STAFF-SS-33`. Sub-44 phone controls went into the STAFF-SHELL sweep table per the 2026-09-11 decision, not here.

- [x] **The leaderboard builder promises "Tap one below to see why" and the tap can never land.** `LeaderboardBuilderForm.tsx:124-134` renders each ineligible metric as `<button disabled onClick={() => setShowWhyDisabled(m.key)}>`. A `disabled` button dispatches no `click` in any browser, so `showWhyDisabled` is never set and the `role="alertdialog"` reason at line 139 is unreachable — measured: tapping "Body mass" leaves zero `[id^=why-]` nodes at both widths. The copy above the row (line 118-121) was rewritten for 0056 to stop claiming which of two reasons applies and to send the reader to the tap; the tap has been dead the whole time. **Fix is one of two, and it is a design decision (brief SS-26):** keep the buttons focusable with `aria-disabled="true"` and let the click show the reason, or drop the tap and print each row's `ineligible_reason` inline. Either way the guard is a source test asserting no `disabled` attribute sits beside an `onClick` on the ineligible row. Both widths. **NOT BUILT (`220d8e9`): the builder recorded it as a design decision — Builder question 8 in `docs/overnight-questions-2026-09-12.md` with a recommendation. Waits on Isabella.** **CLOSED `11dc42f` (reviewer poll 2026-09-12 19:10, merged `7bea968`): `BlockedButton` — `aria-disabled`, focusable, the reason as a `role="status"` line beneath on tap or focus, no `disabled`, no `title`. *Measured as Jane on `/leaderboards/new`: "Body mass" carries `aria-disabled="true"`, is not `disabled`, and a click shows its reason beneath.* One rule for every blocked control (STAFF-SS-02-05 D2, decided 2026-09-12). The reason's copy cites an internal file ("see screens/leaderboards.md") — a separate small §0av bullet.**

- [ ] **The Settings hub's "Log out" row looks like a list row and only a 5px sliver of it works.** `settings/page.tsx:431-443`: a `<form action="/auth/sign-out" method="post" className="set-list-row">` holding the two lines of text and a `<button type="submit">` whose entire content is the "›" chevron. Measured at 1280: the row is 742×71 with `cursor: pointer` on the whole grid; the button is **4.8×23**. At 375 the same. Every other `.set-list-row` on the page is a `<Link>` where the whole row is the target, so the eye reads this one the same way and the click on "Log out" does nothing. **Fix:** make the button the row — `<button type="submit" className="set-list-row">` with the two spans and the chevron inside it — or give the form's row an explicit `<button>` that fills it. Not a 44px matter: the desktop target is 5px wide.

- [x] **Exports tells the sport scientist they have "Coach access".** `settings/exports/page.tsx:58`: `{claims.roles.includes('medic') ? 'Medical' : 'Coach'} access: every domain below, squad-wide.` — every non-medic role reads "Coach", the sport scientist and S&C included. The sentence exists to say which domains this role can export, so name the role correctly (the role model has the label) or drop the role word: "Every domain below, squad-wide." Low. **CLOSED `e59dceb` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): *measured on `/settings/exports` as Jane: "Every domain below, squad-wide." — the role word is gone.***

- [x] **Three screens scroll horizontally at 375px because a table (or a span row) has no overflow container.** Measured `document.scrollingElement.scrollWidth` at 375: `/settings/subject-access` **433** (a six-column `table.tbl` at 417px sitting directly in `main`, `overflow-x: visible`), `/settings/retention` **491** (a four-column `table.tbl` at 458px inside `section.card`, the three-column one fits at 336), `/leaderboards/{id}` **383** (the `.lb-gap` spans in the ranking run 8px past the edge). The page body scrolls sideways and the sidebar-stacked shell goes with it. **Fix:** the tables get the `overflow-x: auto` wrapper the reports already use (SS-16 to -22 measured clean), and the ranking row clamps or wraps its gap column. Phone only; the STAFF-SHELL brief is told, but this is content, not shell, so it is filed here. **CLOSED `6235ae5` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): *measured at 375 as Jane: `scrollWidth` 375 on `/settings/subject-access` (was 433), `/settings/retention` (was 491), `/leaderboards/58d1ce00…` (was 383).* 4 assertions in prebuild.**

**Recorded, not filed.** (1) The audit log's type chips are the distinct `entity_type` values in the club's log (`fetchEntityTypes`), so the absence of a session/schedule chip is §0al showing through, not a second defect. (2) `/analytics/build` is reachable by URL only, by the page's own stated decision. (3) "Your role requires two-factor authentication" on the hub is prompted, not enforced — the deferred RLS `aal` check (`09-security-and-compliance.md`, O-323), a recorded boundary. (4) Three gate claims in the walkthrough were wrong the same way §SS-28 records (in-page `hasAnyRole` read as the door): `/leaderboards/{id}` is open to every staff role; the subject-access list admits the medic as well as the admin; exports is `REPORT_ACCESS`. Corrected in the document.

## 0aq. Medium priority, filed 2026-09-11 — from Dublin, a failed sign-in takes longer than the floor it is supposed to be held to

- [ ] **Failed sign-ins on production answer in 1.1–1.6 s from `dub1`, above `FAILED_SIGN_IN_MIN_MS` (800 ms, `src/lib/signInSubmission.ts:90`), so the server-side work — not the floor — sets the response time.** Isabella measured 1.1–1.6 s after the region move; the reviewer's three unknown-address attempts at 16:53 BST: **1.42 s, 0.95 s** (401) and **2.56 s** for the threshold 429. The floor exists to make the real-account path (bcrypt in GoTrue, the `after()` audit write) indistinguishable from the unknown-address path by holding both to the same minimum. When the unknown path alone already exceeds the minimum, the hold never engages, and whatever difference the two paths have in server time shows through undisguised. C1 was closed on production this afternoon on the **minima** (1.027 vs 1.011 s; 1.198 vs 1.192 s), which were also above the floor — the closure rests on the two paths measuring the same, not on the floor covering them.

  **What to do, in order.** (1) Profile `POST /auth/sign-in` on the unknown path from `dub1`: the limiter RPC (`login_attempt_record_result`), the GoTrue round-trip, and the response — the Vercel function's own duration in its logs, not the curl figure, which includes the London-edge hop. (2) Decide whether the floor rises to cover production's real worst case (the item's original fail rule was ~900 ms) or whether the server work comes down — moving the function next to the database was expected to shorten it, and 0.95–1.42 s for a request whose real work is one bcrypt says something else is slow. (3) Re-run the interleaved real-vs-unknown comparison (§1158's method) after either change; the pass criterion is unchanged — bodies identical, times within ~40 ms at each step, both above the floor. The test guard (`test-sign-in-attempts-left.ts`) only asserts the floor is ≥ 700 ms; it cannot see this.
...
- [x] **`auth.sign_in_failed` rows carry `attempts_remaining` and `locked` only; `auth.signed_in` rows carry `method` and `user_agent`.** `src/lib/signInAudit.ts:309-314` builds the failure row's `metadata` from the `FailureContext` alone, while the success row (`:122-140`) reads `clientUserAgent(headers)` and stores it when present. Both already record `ip_address` from the same headers, so the failure path has the headers in hand and simply does not read the agent. **CLOSED `4c7a127` (reviewer, 2026-09-12 morning, merged `3103ad9`): the failure row's metadata now spreads `user_agent` from `clientUserAgent(headers)` exactly as the success row does (`signInAudit.ts`).**
...
- [x] **`settings/imports/page.tsx:136-138` tells the sport scientist "There is no revert and no duplicate detection yet — re-uploading the same file makes a second batch of the same rows." The second half has been false since 0064/0072:** `gps_records` carries `gps_records_one_per_athlete_session` (unique, nulls not distinct, on `org_id, athlete_id, record_date, session_id`) and `commitGpsImport` upserts on exactly those columns (`gpsImport.ts:272-288`), so a re-upload **replaces** the rows rather than doubling them. Both migrations are on production. The sentence is the one a club reads before deciding whether to re-upload a corrected file, and it tells them the opposite of what will happen. "No revert" is still true. **Fix:** rewrite the sentence to say what the upsert does — same athlete, date and session replaces; a different session adds — and keep "no revert". Copy only. **CLOSED `effc471` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): *measured on `/settings/imports` as Jane: "There is no revert. Re-uploading a corrected file replaces the rows it matches — the same athlete, date and session — and adds the rest; nothing is doubled. …"* One assertion pins the copy to the upsert's four conflict columns.**

## 0at. Gym tonnage is stored only after a correction, so My data's history says "Not logged" for almost every real session — found 2026-09-12 spot-checking ATH-ADULT-12

- [x] **CLOSED `5003959`, migration 0106 (reviewer, 2026-09-12 ~11:50, fast-forward merge to `1052354`), on Isabella's direction of 2026-09-12 ("derive gym tonnage from the live sets rather than storing total_volume_kg"): `gym_session_logs_current` now computes `total_volume_kg` as the sum of the live sets that carry both reps and a load (null when none — absent, never 0); the stored column is dead as a source. *Reviewer check, read-only on scratch: through the view 0 of 45 complete sessions with live sets are null (was 41); `c55723fe` reads 4762.0; on `/my-data?tab=gym&period=all` as Conor the 11 Aug row reads "4.8 t" and no `hist-value[data-missing]` remains.* The builder measured the whole org through the view at 0.455 ms. pgTAP 620 (8) green on scratch; `test-gym-tonnage-derived.ts` (17) in prebuild. Left: 0045's RPC still writes the dead column; the staff athlete gym page shows '—' for null. 0106 is on scratch, not production.** ~~`gym_session_logs.total_volume_kg` is written by exactly one path — 0045's correction RPC — and by nothing on ordinary set logging.~~ 0045's own comment says so: "total_volume_kg is not otherwise computed anywhere in this build (a real, separate, pre-existing gap)".** Measured on scratch, read-only: of 45 complete sessions with live sets, **41 have a null total and 4 have one — and those 4 are exactly the sessions that went through a correction.** The My data Gym history row reads the stored column and, since `e581211` renders absent values as words, shows **"Not logged"** in the value slot for a session whose own detail page sums the same live sets to **"4762 kg"** (Conor Moroney, Tue 11 Aug, 6 sets — `hist-value[data-missing]` on the row, `rd-value` 4762 on `/my-data/gym/c55723fe-…`). The 13 Aug row shows "4.2 t" only because it was corrected once. Staff see the same skew: `programmes.ts:1450` averages `total_volume_kg` "across the sessions that recorded one", so the positional and squad volume means are means of corrected sessions only.

  **Why it is group 1 on the pilot list:** a real athlete's first logged session will read "Not logged" in their own history the next day, and staff volume figures will be computed from a handful of sessions. Wrong data shown, on day one, on the two screens that show it. It was invisible until the words replaced whatever the null rendered as before; the words are right, the column is not.

  **Fix, one of two, and the second is the honest one:** (a) maintain the column — recompute on every set insert/revision (a trigger on `gym_set_logs`, same sum 0045 already uses), plus a one-off backfill migration for existing rows; or (b) stop storing it — derive the session total at read time from live sets in the two queries that read it (`programmes.ts:1146` history rows, `:1499` the means) and the export (`exportBuilder.ts`), and drop the column's role. (b) removes the class of bug; (a) keeps the reports cheap. Either way the guard is a test that logs one set into a fresh session and asserts the history row shows the tonnage, not "Not logged". This is the data-architecture briefing's open question 12 ("stored or derived? If stored, recompute on revise") answered by measurement: today it is stored, and not recomputed on log.

## 0au. The 44px floor on the built phone shell misses a set of control classes — from the STAFF-COACH-01 to -07 review, 2026-09-12

- [x] **`af09c17` floors `.back-btn`, `.btn-ghost`, `.btn-primary`, `select`, the schedule steppers, the lens tabs, the theme segment and the reorder buttons below 768px, and gives inline links a 44px hit box. Measured on the built shell at 375×812 as a coach, four screens, these are still under it:** `/dashboard` "Compliance ›" (`.tiny`) **19px**; `/squad` the athlete-name links **43px** (19px text + 12px padding, the −12px margin trick — one pixel short); `/squad/{id}` "Squad overview" 40, "Change plan" 37, "‹ Squad" 37, "Edit" (Goals) **33**, "+ Log injury" 37, "Thresholds ›" 19, "Acknowledge" ×2 **29**, "Edit ›" 19; `/schedule` the five group chips **39** (plain `<button>`s on this screen, not `.rhead-chip`), the Read / Edit segment **31**, "Week plan" (`<span>`) and "Today" 40, the week arrows `‹` `›` **32×32**. The builder's own handover names the gap: page-specific markup without the shared classes was not raised. **One follow-up to the shell build, not per flow (the 2026-09-11 decision stands):** raise the inline-link padding to 12.5px or the line to 20px so the roster names clear 44; add the group-chip buttons, the view segment, the week arrows, `.tiny` links and the flag "Acknowledge" to the floored set; re-run the STAFF-SHELL brief's sweep table on the result. Phone only. **CLOSED `6a2f1d4` (reviewer poll 2026-09-12 13:30, fast-forward merge to `ea1fcac`): `.main button` floored at 44 (session blocks excepted), the week arrows 44×44, view tabs / chips / segment / pill ghosts at 44, inline links on a 20px line. *Re-measured as the coach at 375: `/dashboard` 0 of 25 under 44, `/squad` 0 of 37, `/squad/{James Barnes}` 1 of 31 ("Edit ›" 29 wide × 44 — the height floor holds; the width is the word), `/schedule` 0 of 32.* 68 assertions in the shell guard.**

## 0av. Four smaller findings from the STAFF-COACH-01 to -07 review — 2026-09-12

- [x] **Three `disabled` buttons on the athlete profile carry their reason only in a `title`** — and, measured 2026-09-12 across all five roles (`docs/walkthrough-reviews/athlete-profile-role-inventory.md`), so do the five "Correct check-in" buttons for the S&C and the nutritionist and the header "Edit" for both; one rule for all of them (STAFF-SS-02-05 D2). ** `/squad/{id}`, Body weight: "+ Log weigh-in", "Set target range", "Edit entries" render `disabled` + `aria-disabled="true"` for a coach with `title="Logging a weigh-in belongs to the sport scientist, the medic, the S&C and the nutritionist."` (and the two siblings). A disabled button gets no hover on touch and no focus anywhere, so on a phone the coach sees three greyed controls and never the sentence. Same class as §0ap's leaderboard chips: either `aria-disabled` and focusable with the reason shown on focus/tap, or the sentence printed once under the trio. Both widths. **CLOSED `11dc42f` (reviewer poll 2026-09-12 19:10, merged `7bea968`) for the weigh-in trio, the bio Edit and the correction button (all `BlockedButton`). *Measured as the coach on James Barnes's profile at 375: "+ Log weigh-in" — `aria-disabled="true"`, no `title`, a tap shows "Logging a weigh-in belongs to the sport scientist, the medic, the S&C and the nutritionist." as a `role="status"` line.* The nutrition page's "Assign" and steppers and the medic's "+ Meal" are **not** in the converted list — their own bullets stay open.**
- [ ] **`/analytics/build` refuses the coach with copy about the admin that cites a superseded internal document.** `analytics/build/page.tsx:142-153`: for any role without `ANALYTICS` it renders "Not part of this role" and "Analytics is named-athlete performance data. Admin manages the club and does not read athlete performance data — see 01-roles-and-permissions.md (superseded) §1." The reader is a coach (or medic, S&C, nutritionist); the sentence is about a role the app no longer has and points at a file marked superseded. `/analytics` itself redirects to `/dashboard?e=no-analytics`. **Fix:** either redirect `/analytics/build` the same way, or say the true thing in the coach's terms — "Analytics is the sport scientist's screen." No data leaks; copy only.
- [ ] **Five settings-admin routes bounce a coach to a bare `/settings` with no message; the sixth carries `?e=no-sar-access`.** `/settings/audit`, `/settings/retention`, `/settings/users`, `/settings/users/bulk-invite`, `/settings/imports` each `redirect('/settings')`; `/settings/subject-access` redirects with an `e`. A coach who follows a bookmark or a colleague's link lands on the hub and learns nothing. One `e` value per gate, and the hub renders it, as the dashboard renders `no-analytics`. Design; low.
- [ ] **Seven disabled controls on `/nutrition` carry no reason at all for the coach — and for the S&C (measured 2026-09-12; "+ Meal" too, with the medic's).** "Assign" and the six day-type `−`/`+` steppers render `disabled` for a role without `NUTRITION_EDIT` — no `title`, no `aria-disabled`, no sentence on the page (the profile's Body-weight trio at least had a `title`). Measured 2026-09-12 as Mark Iremonger at 1280 and 375; the walkthrough had "Assign" as absent. Same rule as the two items above (STAFF-SS-02-05 D2): a reachable reason, or absence. Both widths.
- [ ] **The coach's — and the medic's — Settings hub links to pages that bounce or refuse.** For the medic (measured 2026-09-12) the same two Integrations links bounce, and the **"Thresholds" row** leads to `/settings/thresholds`' in-page "Not part of this role". One rule: a row or link renders only for a role that can follow it. ** The Integrations section renders "Connected" (Catapult Openfield) and "Open" (Vendor CSV import) as links to `/settings/imports` for every role; for a coach `/settings/imports` redirects to `/settings` (`GPS_IMPORT`), so both links return the coach to the hub with no message. Measured 2026-09-12 as Mark Iremonger. Render the state as text for roles without `GPS_IMPORT`, or hide the section's links as the admin rows are hidden. Both widths.
- [ ] **A blocked control's reason cites an internal document.** After `11dc42f`, the leaderboard builder's "Body mass" chip explains itself with "Body mass cannot be ranked. Body composition data. Never ranked: see screens/leaderboards.md \"Why bo…\"" — `ineligible_reason` carries a file path a club user cannot open (same class as §0av's `/analytics/build` copy). Say the reason in the reader's words and drop the reference. Copy only.
- [ ] **RULE, decided 2026-09-12 (Isabella), for every role: navigation lists only what the role can use — a nav row or tab that always refuses is removed for that role, not disabled; in-page controls the role cannot use stay `BlockedButton`s with their reason. Start with: (a) the nutritionist's sidebar and More sheet — eight rows for two working screens (Reports opens a hub that refuses four of six; Gym programme, Leaderboard and Schedule open read-only; Settings shows rows that refuse or are empty); the bar's role slot already varies, the sheet and the sidebar do not; (b) the profile's Gym tab for the nutritionist (below); (c) every hub row and Integrations link that refuses or bounces for a role (the coach's, medic's, S&C's and nutritionist's Settings hubs, measured today); (d) `/leaderboards/manage`'s "+ New leaderboard" for roles without `LEADERBOARD_EDIT` (§0az).** The per-role inventories in `docs/walkthrough-reviews/` (coach 01–07, 29–33; medic 19–30g, 16–33; S&C; nutritionist) list what refuses whom. **The athlete profile offers a "Gym" tab to the nutritionist, and the tab refuses them with the admin copy.** `/squad/{id}` renders Nutrition / Wellness / Gym tab links for every role; `/squad/{id}/gym` gates on `ATHLETE_GYM` (which excludes the nutritionist alone) and renders "Not part of this role. An athlete's gym detail is named performance and wellness data. Admin manages the club and does not read athlete performance data — see 01-roles-and-permissions.md (superseded) §1." — the `/analytics/build` copy again, to a nutritionist. Measured 2026-09-12 as Sana Mirza. Hide the tab without `ATHLETE_GYM`; and the shared refusal copy says the true thing in the reader's terms, without the file reference (one fix for both refusal pages). And **`/settings/notifications` renders "Nothing to configure"** for the nutritionist while the hub still lists the row — absent rows for absent content.
- [ ] **Two profile panels say "Edit" to roles that cannot edit.** The Goals panel's "Edit ›" (→ `/programmes/{id}`) and the Nutrition plan panel's "Edit" (→ `/nutrition`) carry the same label for all five roles; the coach, medic and S&C reach a read-only programme page (`PROGRAMME_EDIT` is sport scientist + S&C — the programme page itself already says "View full detail →" for them) and a nutrition page they cannot author (`NUTRITION_EDIT` is sport scientist + nutritionist). Label by permission, as the programme page does: "View ›" when the role cannot write. Found in the five-role inventory, 2026-09-12. **PARTLY CLOSED `702dd2c` (reviewer poll 2026-09-12 19:50, merged `22f06d7`): the programme link (Goals panel and the header pill) now reads "View full detail ›" / "View plan" for a role outside `PROGRAMME_AUTHOR` — *measured as the coach vs Jane ("Edit this programme ›" / "Change plan")*. The Nutrition plan panel's "Edit" (→ `/nutrition`) for roles without `NUTRITION_EDIT` is still open.**
- [ ] **DEFECT, direction decided 2026-09-12 (Isabella): keep the coach's permission to create an injury record and show "+ Injury" on the `/injuries` board to the coach too; the coach's form is the non-clinical one (PATTERN-S3 C9), clinical fields medic-only.** **One permission, two doors: the coach may create an injury record, and one of the two entry points hides it.** `/squad/{id}` shows "+ Log injury" to the coach and `/injuries/new` opens (0068 `injuries_staff_insert` — §3.2 "New injury VC VC VC VC X"); the `/injuries` board's "+ Injury" is medic-only (SS-28's own observation). Not a gate error — the spec gives all four roles the create — but a coach at the board who cannot see the button will not guess the profile has it. Decide which is right: show "+ Injury" on the board to every `INJURY_ACCESS` role, or hide "+ Log injury" on the profile from non-medics and state why. Design.

## 0aw. From the STAFF-COACH-08 to -11 review — 2026-09-12; the schedule on a phone, and two shell edges

- [ ] **The phone shell's title-bar group label does not follow an in-page filter change.** `StaffPhoneShell` reads the group from the §0ak cookie in the layout (`groupScopeLabel`), so it is right on navigation and wrong after a chip press: on `/schedule` at 375, pressing "Forwards" updated the chip row, the URL (`?groups=…000001`) and the grid, and the bar still read "Schedule · Whole squad"; pressing "Whole squad" back likewise. Measured twice, both directions, as the coach. The chip row and the title bar are the two places the scope is stated on a phone and they disagree until the next server render. **Fix:** the shell's label reads the same client state the chip row writes (or the chip row triggers a refresh of the layout's label). Phone only.
- [ ] **After "Add to {day}" in the wizard, the confirmation is 3,000px away on a phone.** At 375 the wizard card opens at 267px (good), and "Add to Sunday" closes it, drops the block into the grid at 1,480px and opens the selected-session panel — "Remove session", "Duplicate", "Publishes to Backs · appears under Today on the morning of Sunday 13 Sept" — at **3,364–3,636px**, with the page still at scroll 0; the banner "1 change not yet in the athlete app" is at 644px. The coach sees the wizard vanish and nothing else. At 1280 the same panel region sits below the grid at 1,644px (the document said "right-hand rail"; corrected). **Design, both widths:** either the page scrolls the panel into view when a block is selected or added, or the panel is placed where the selection happened (a sheet on a phone, the rail the document imagined on desktop). Recorded for the STAFF-SS-07/10 design surface; the STAFF-COACH-10 brief carries it.
- [ ] **"Back" renders on bottom-bar root screens.** Every staff page renders `.back-btn` above its chip row; on a phone `/schedule`, `/squad`, `/dashboard` are bottom-bar destinations and the "Back" above the chips is a root screen pointing backwards. Design, shell: hide `.back-btn` below 768px on the five bar routes, or make it the page's own "‹ Squad"-style parent link only where there is a parent.
- [ ] **An open wizard is not restored on reload.** `71035f5` writes `newDraft` to sessionStorage on every step, and `/schedule` on reload restores edits, added and removed but shows no wizard for a stored `newDraft`; the next "+ Session" starts a fresh draft over it. Measured as Jane at 1280 (name typed, reload, no wizard; the key still holds the title). Small: either re-open the wizard from the stored draft or do not store it.

## 0ax. Four report pages scroll sideways at 375, and their controls are under the floor — from the STAFF-COACH-16 to -22 review, 2026-09-12

- [ ] **`/reports/squad` (scrollWidth 437), `/reports/athlete` (579), `/reports/athlete/{id}` (489) and `/reports/training` (381) scroll the page sideways at 375×812 on the built shell.** Measured as the coach; the overflow roots are the `.attn-sev` severity spans in the squad report's attention list, spans in the athlete-list rows, a 189px div and two spans on the athlete report, and `.tr-dials` / `.tr-board-inner` (548px) on the training report. Same class as §0ap's three settings screens, fixed by `6235ae5` with an `overflow-x: auto` wrapper; these four were not in that list. **And the floor:** on the same pages the shell's generic 44px rules (`6a2f1d4`) do not reach `/reports/squad`'s week arrow "‹" (31px) and its athlete-name links (**15px, no padding** — not the `table .nm` markup the roster uses), `/reports/testing`'s "Manage tests →" (19px), `/reports/training`'s "Training" / "Match day" chips (35), "Heat" (22), "Day" / "Week" (40), "Rest of the week" (37). Listed in the STAFF-SHELL sweep table as well. Phone only; one follow-up, the same shape as §0au.

## 0ay. The injuries PDF prints "Restricted" for an athlete with nothing recorded — from the STAFF-MEDIC-01 to -05 review, 2026-09-12

- [ ] **`reports/injuries/pdf/route.tsx:96` renders DETAIL as `body_area`, else the restriction list, else the literal `'Restricted'`** — so a row whose status is Unknown (no availability recorded, no injury) prints as "Unknown · Restricted": Dan Okonkwo, Kai Mercer and the scratch-only "Walkthrough Reference" athlete on the 2026-09-12 export, for the coach and the medic alike. A printed document that asserts a restriction nobody recorded. The web page (`reports/injuries/page.tsx:308`) handles `unknown` as its own case. **Fix:** the fallback is "Not recorded" (the A1 word) or blank, never a status word; and the same check on the CSV export's detail column. Both roles, both widths.

## 0az. Three controls offered to the medic that the database then refuses — from the STAFF-MEDIC-19 to -30g review, 2026-09-12

- [x] **DIRECTION DECIDED 2026-09-12 (Isabella): the `BlockedButton` treatment — `aria-disabled`, the reason on tap — for every control the database refuses while the screen reports success, starting here.** **The group reorder arrows render for every role and a press by a role without `GROUP_EDIT` is a silent no-op.** `settings/groups/page.tsx:157-164` renders `GroupReorderButtons` whenever there are two rows, outside the `canEditGroups` branch that hides "+ New group". Measured as the medic: "Move Backs down" → `moveGroup` runs two UPDATEs → `groups_staff_update` (0078: sport scientist, coach) filters both to zero rows → PostgREST returns no error → `moveGroup` (`groups.ts:457-462`) checks `.error` only → the component's `onSuccess` refreshes → the order is unchanged (`Backs:1 | Forwards:2` before and after) and nothing is said. The database holds; the screen reports success for a refused write. **Fix:** render the arrows only with `canEditGroups` (one line), and make `moveGroup` check the affected row count so a filtered-out update is an error, not a success — the same shape as the silent-save class already closed elsewhere (`test-silent-saves`). Applies to the S&C and the nutritionist too. **CLOSED `3e7b806` (reviewer poll 2026-09-12 20:05, fast-forward to `00a7119`): the arrows are `BlockedButton`s for roles outside `GROUP_EDIT` (one reason for the pair, shown on tap) and `moveGroup` runs its two updates in order through `mustAffect` with `.select('id')`, refusing out loud when the policy filters a write. *Measured as the S&C: `aria-disabled="true"`, `data-blocked`, no `title`; a tap shows "Reordering groups belongs to the coach and the sport scientist."; order unchanged.* `test-silent-saves` gains the seventh screen and a `moveGroup`-specific check.**
- [ ] **`/leaderboards/manage` offers "+ New leaderboard" to roles without `LEADERBOARD_EDIT`** — the link goes to `/leaderboards/new`, which redirects straight back to `/manage`. Measured as the medic. Hide it without the role, as the board page already hides "Board actions".
- [ ] **`/nutrition`'s "+ Meal" renders `disabled` with no reason for the medic** (no `MEAL_LIBRARY_EDIT`) — the same reasonless-disabled pattern as the coach's "Assign" (§0av). One rule for all of them (STAFF-SS-02-05 D2).

## 0ba. SAR withholding: the admin reads a category, the medic's free text stays medic-only — decided 2026-09-12, a before-pilot schema change

- [ ] **`sar_clinical_reviews.reason` (free text, required on `withhold`, 0032) is readable by the admin at the database** — `sar_clinical_reviews_staff_select` admits `admin, medical` (0032:110-112, carried through 0066/0068 as sport scientist + medic) — measured 2026-09-12 with the admin's own session: 2 rows. No admin screen renders it. **Decision:** the admin sees a **withholding category** (e.g. third-party data, clinical harm — the security doc §6's exemption grounds), never the medic's free text; the free text stays medic-only; the audit log holds the full record. **Shape:** a `reason_category` column (enum or checked text) beside `reason`; the select policy or a view that gives the admin `decision` + `reason_category` and withholds `reason`; the review form asks for the category and the text; the pack assembly reads the category; `audit_log` records the full review (0085's clinical-write audit already covers the insert — confirm it captures both fields). Migration + the review form + the pack. **Before pilot, not blocking.**

## 0bb. The proposal gate for injured athletes applies to every role except the medic — decided 2026-09-12, before pilot, enforced at the database

- [ ] **Today only the S&C's assignment of a programme to an athlete with an open injury becomes a proposal for medical sign-off** (`proposesAgainstInjury = INJURY_PROGRAMME_PROPOSER && !isMedical`, `programmes/[programmeId]/page.tsx:62-63`; migrations 0079–0081). Measured 2026-09-12: the sport scientist choosing the same injured athlete in "+ Assign" gets "Assign" — a direct assignment, no note, no sign-off. **Decision:** the gate applies to every role except the medic, the sport scientist included; the medic assigning is the sign-off. **Enforced at the database, not only the screen:** the assignment write path (`programme_assignments` insert, or the RPC that creates one) refuses a non-proposal row for an athlete with an open injury unless the caller holds `medic` — a trigger or the insert policy's `WITH CHECK`, whichever 0079–0081 already shaped, so that a screen that forgets cannot assign around it. The screen half: `proposesAgainstInjury` becomes `!isMedical && athleteHasOpenInjury`, and the "+ Assign" note and "Propose" label render for every non-medic role. Guard: a pgTAP case that inserts as the sport scientist against an injured athlete and expects the refusal; the existing S&C case unchanged. **Before pilot.** The PATTERN-S3 C6 proposal states (Proposed / Approved / Returned) are the same work's UI.

## 0bc. A COMPLETE gym session accepts a NEW set at the database — found 2026-09-12 (reviewer, on Isabella's question), before pilot

- [x] **Nothing at the database stops a new set landing on a closed gym session log; only the screen does.** **CLOSED `f492f23` (builder, 2026-09-13), migration 0110 on SCRATCH only.** *Re-probed as Dan Okonkwo with his own session, the method that found it: `POST gym_set_logs` set 9 on his complete log `d5495a92` → 400 `P0001 session_log_closed` — "gym session log … is complete; a new set cannot be added to it · Correct a logged set instead. 0110_gym_closed_log_no_new_set.sql".* Isabella's rule, 2026-09-12: correcting a set on a complete session is decided and fine; *adding* one is not — it would mean "complete" means nothing and adherence changes after the fact.

  **What was read.** `revise_gym_set_log` (0045:261) is clean: identity columns come from the original row, never from the payload, so a correction can only re-occupy the slot it fixes — it cannot add a set. The screen is also clean: `GymSessionLogger.tsx:698` sets `isNext = !alreadyComplete && …`, so on a closed log every unlogged key is `disabled` (since `693dfdd`; the builder's 09 C1 note about a "live" key after "Correct a set" is not borne out by the gate — the key is disabled, the pill styling is what stays). The 150 kg bench press on Dan Okonkwo's Upper B that raised the question is a **correction** (`revision_of` set, supersedes set 1 at 20:28:39, six minutes after `completed_at`), not a new set — decided, fine.

  **What is open.** The write path is a plain insert — `submitGymSetLog` (`programmes.ts:1020`) → `gym_set_logs_self_insert` (0021:504), whose `with check` is org + "the log is the caller's"; no later migration narrows it (0045 drops the update policy, 0066 adds a staff select, 0095/0096/0097/0098 are a non-negative check, audit triggers and a truncate guard). No trigger and no constraint reads the parent log's `status`. `flushGymSets` (`gymOutboxFlush.ts`) does not check it either, so a set queued before Finish and flushed after it lands on the closed log through the app's own path.

  **Proved 2026-09-12 on scratch, net zero rows:** as Dan (`set local role authenticated`, his JWT claims) inside a transaction that was rolled back, `insert into gym_set_logs (…, set_number 5, reps 5, load 150)` against his `complete` Upper B log `d5495a92` **was accepted** — 12 live rows became 13 inside the transaction, 12 again after the rollback. `scratchpad/closed-log-insert-rollback.mjs`.

  **Why it matters.** Sets done, MET-041 session volume, MET-040 bests, the coach's positional band, compliance ("complete" logs only, `reports.ts:252`) and My data all sum live sets of a complete log. A set added after the fact changes adherence and the bests retrospectively, with `completed_at` unmoved — and by the athlete's own token, from any client, not only the app.

  **Proposed fix, for the builder — its own migration (0109), tests first, the old rule asserted ([[fydr-replacing-a-policy-assert-the-old-rule]]):** a `before insert` trigger on `gym_set_logs` that raises `session_log_closed` when the parent log's `status = 'complete'` **and** `new.revision_of is null` — a correction (`revision_of` set, written by `revise_gym_set_log`) still lands, a new set does not. A trigger rather than the policy's `with check`, so the security-definer correction path and the plain insert path meet one rule. Then the app: `submitGymSetLog` humanises `session_log_closed` ("This session was finished before this set was sent — correct a logged set instead") and `flushGymSets` treats it as a flagged item, not a retry (the same shape as §0aa's conflict, not silently dropped, not retried forever). The re-open path stays as it is: `gym_session_logs_self_update` (0045) lets the athlete update six columns of their own row, `status` among them — reopening a log is a separate question, not raised here.

  **Not affected:** `revise_gym_set_log`, the summaries, the logger's keys.

## 0bd. `users_self_update` lets every account rewrite its own `status`, `email`, `deleted_at` and `claims_version` — the last admin deactivated herself at the database — found 2026-09-12 (reviewer, test-club run step 8), GATE, before pilot

- [x] **CLOSED `6db21d1` (builder, 2026-09-13), migration 0109 on SCRATCH only** — *re-probed as Bethan Rees (coach) with her own session, the method that found it: `PATCH users {status: suspended}` → 400 `P0001 users_self_update_profile_only` ("a person cannot change their own status or delete themselves · Ask the sport scientist"); `{email: …}` → 400 (…"email, org_id, id, created_at and claims_version are not writable through the app"). The builder's pgTAP 650 (26) covers self-deactivate / rename / reactivate, the last-admin account, and the claims_version bump; his live check: a suspended coach was signed out on his next request.* The original finding: **Proved on Harlow Vale (scratch) as Priya Nair, the club's only sport scientist, with her own session over PostgREST:** `PATCH users?id=eq.<self> {"status":"deactivated"}` → **200**, row returned with `status: deactivated`, `claims_version` unchanged at 2. The screen had refused it — the Deactivate button on her own row is `disabled` — so the guard exists only in the browser. Reverted the same way sixty seconds later (200, `active`); both writes are in `audit_log` as `users.update {changed:[status]}`. The same run showed 0101 and 0102 holding at the database (`403`, their own messages) for the role-row routes; this is the route past them.

  **Why.** `users_self_update` (0012:123) is `using/with check (org_id = auth_org_id() and id = auth_user_id())` with no column restriction, and `authenticated` holds UPDATE on **every** column of `users` (`avatar_colour, avatar_url, claims_version, created_at, deleted_at, email, full_name, id, last_seen_at, org_id, phone, status, updated_at` — read from `information_schema.column_privileges` on scratch). The policy was written for "edit their own profile: name, contact details" (its own comment) and grants the whole row.

  **What it lets any account do to itself, staff or athlete:** set `status` to `deactivated` or `suspended` (an org whose only sport scientist does this has no admin left — 0101 guards the *role row*, not the *account*; recovery is service-role only); set `status` back to `active` while a still-valid token is held (the claims hook, 0010:178, only bites at the next refresh — deactivation does not bump `claims_version`, measured, so the token runs to expiry); set `deleted_at` (a self soft-delete, which the claims hook then treats as unknown); rewrite `email` on the `users` row so it no longer matches `auth.users` (every screen and export names the account by this column); rewrite `claims_version` and `last_seen_at`.

  **Proposed fix, for the builder — its own migration (0110), tests first, the old rule asserted ([[fydr-replacing-a-policy-assert-the-old-rule]]):** a `before update` trigger on `users` — when `auth_user_id() = old.id` (a self-update by anyone, the sport scientist included), refuse if any of `status, email, deleted_at, claims_version, org_id, id, created_at` changes (`P0001 users_self_update_profile_only`); when the actor is a sport scientist updating **another** row, refuse `email, org_id, id, created_at, claims_version` (the route `settings/users` never writes them; `claims_version` is the trigger's own). Then extend 0101's last-admin rule to the account: refuse `status <> 'active'` or `deleted_at is not null` on the row of the org's only active `sport_scientist` (the same count 0101 already makes). Bump `claims_version` on any status change so a deactivation takes effect at the next request, not the next hour. The app is unchanged: `setUserStatus` already routes through the sport scientist, and the self-profile form writes `full_name`, `phone`, `avatar_*` only.

  **Mirror, proved 2026-09-12 21:10 as Bethan Rees (coach, no admin role), her own session:** `PATCH users?id=eq.<self>` with `{"status":"suspended"}` → 200; back to `active` → 200; `{"email":"someone.else@harlowvale.example"}` → 200 (reverted → 200). A non-admin flips her own status both ways and renames the account's email at will. Four `users.update` audit rows, actor = herself.
...
- [x] **Real gap to fix**: the code's `app_role` enum currently only has four values (athlete, coach, medical, admin), not the five decided roles. It doesn't distinguish sport scientist, S&C, or nutritionist as separate roles yet, and still has an `admin` value that shouldn't exist. This needs a migration plus updated RLS policies, it's not just a naming tweak. **CLOSED 2026-09-11: `app_role` carries all five staff roles plus athlete; `medic` is the enum value (0063); every screen read in the sport scientist reviews gates on the five.**
- [x] **Finish the different staff logins and access, end to end.** The umbrella task the pieces above and below feed into: real accounts for all five staff roles, correctly gated on every screen, not just some. Ties together the `app_role` migration above, G-03 (credential handling), D-01 (injury gate), D-06 (schedule editing), and every "S&C, not built" or "nutritionist, not built" flag found screen by screen so far. Worth treating as one build push rather than fixing each screen in isolation, since they're all the same underlying gap surfacing in different places. **CLOSED 2026-09-11 as an umbrella: every gate read during STAFF-SS-01 to -33 resolves to a `require*` helper or an `access.ts` constant; `test:access-gates`, `test:role-model` and `test:report-visibility` are in prebuild. Individual role-gate corrections still open are listed by line in `docs/pilot-readiness-2026-09-12.md` (L728, L740, L755).**
