# Design brief — STAFF-NUT-23, Build a nutrition plan and assign it
...
**Read the STAFF-SHELL brief and the STAFF-SS-23 brief first.** The phone shell
...
1. **Identical to STAFF-SS-23 by fingerprint** — the one screen this role owns is the sport scientist's screen unchanged; the SS-23 brief is the design surface and its findings (§0am's selected state, the 6,099px phone page) are this role's first.
2. **Only `tokens.css` values.** Anything not in the file below is flagged as a
   proposed new token, never used.

## 5. What a proposal should address

1. **The role's home screen** — a nutritionist opens Fydr for this page; whether their bar slot (Nutrition) should land here rather than on the dashboard is the STAFF-SHELL question for this role.
