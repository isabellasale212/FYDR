# Design brief — STAFF-COACH-05, Open an athlete's profile
...
**Read the STAFF-SHELL brief and the STAFF-SS-05 brief first.** The phone shell
is now built (`af09c17`: bottom bar of four plus More, a More sheet, a 64px title
bar, the 44px floor below 768px); this flow is measured on it. The coach's screen
is identical to the sport scientist's by fingerprint — the SS-05 brief is the
...
*The flow itself, as the sport scientist section states it (the coach's is identical by measurement):*
...
**Full review: `docs/walkthrough-reviews/staff-coach-01-to-07-review.md`.**
...
1. **05a is the constraint:** the coach sees status, restrictions and expected return; never diagnosis or mechanism. The database enforces it (0063) and the injury detail page states it ("This is what coaching staff see …"). A proposal keeps a stated boundary, never a blank space where clinical content would be.
2. **The Subject access request section is absent** for the coach — absent, not locked.
3. **The Body weight trio is disabled by role** (`WEIGH_IN_EDIT`, `NUTRITION_EDIT`) with a reason — §0av: the reason must be reachable on touch (focusable `aria-disabled` with the sentence on tap, or the sentence printed once under the three). A proposal chooses one.
4. **"+ Log injury" is the coach's** (§3.2 gives all four roles the create) — keep it; §0av asks whether the board's "+ Injury" should match.
5. **Only `tokens.css` values.** Anything not in the file below is flagged as a
   proposed new token, never used.

## 5. What a proposal should address

1. **5,605px at phone** for an injured athlete — the coach's before-session question is "can he train, and what can't he do"; the Injury section that answers it is one of nine sections. Whether it moves up for a coach, or the page opens on it when the athlete is not fully available.
2. **The disabled trio** — drawn with the reason visible at 375.
3. **The page still scrolls sideways at 375** (measured `hscroll` true) — which table, and how it fits.
