# Design brief — STAFF-SS-04, Invite a person and hand over their link
...
the club's Fydr admin — at a desk, and on a phone at the side of the pitch.

**Read §0af first.** Below 768px the staff sidebar stacks 640px above every
screen. That is one design decision for the shell, not for this flow; a proposal
for this flow at phone width must say which shell it assumes.
...
**Full review: `docs/walkthrough-reviews/staff-ss-01-to-04-review.md`** — covers
all four flows at both widths, with what was and was not exercised and why.
...
1. **At least one role is required and roles are additive** — the route enforces
   the first; the copy states the second. Keep both.
2. **The intro stays verbatim** — it is the only place the admin learns that no
   SMS is sent and that a link may be the fallback.
3. **The single-use link and its caption stay** in both end-state variants (in
   `UserManagementPanel`, not exercised).
4. **Self-deactivation stays blocked in the UI**, and — once §0ae lands — the
   self-row admin toggle should be disabled on the same rule.

## 5. What a proposal should address

1. **37 users, six toggles each, no search, no filter** — 6,015px at desktop,
   14,301px at phone. The row an admin wants is a scroll-hunt.
2. **"tick everything that applies"** over toggle buttons — make the copy and the
   control agree.
3. **"+ Invite people"** as a disclosure with no `aria-expanded` (§0af).

Out of scope for design: §0ae, which is a database trigger.
