...
export async function POST(request: Request): Promise<NextResponse<SignInResult>> {
  ...
  if (gateError) {
    ...
    console.error('login_attempt_gate failed', gateError);
  }
  ...
  // Resolved for login_attempts.org_id only -- an admin-visibility/support convenience
  // (migration 0048's login_attempts_admin_select policy), never used to change the error
  // message returned below. An unmatched email resolves to null org_id, same as today.
  ...
  if (recordError) {
    console.error('login_attempt_record_result failed', recordError);
  }
