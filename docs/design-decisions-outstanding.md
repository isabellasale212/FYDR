...
| STAFF-SS-01 | C2 | The dashboard rebuild as a series. **Done:** A1, A2, A4; the thresholds line ("Thresholds set by Mark Iremonger · Wed 2 Sept · Change ›", `fetchThresholdProvenance`, 2026-09-12). **Done 2026-09-13:** the role versions (`lib/dashboardVersion.ts` — the S&C's four tiles with Gym today and Weigh-ins, load-only attention and badge, no week strip; the nutritionist's two, nothing availability-derived; D6/D7 below record where the board and the matrix part); **the matchday lead card** (`components/DashboardLeadCard`, `lib/dashboardLead.ts` — first on the page, the fixture as title, "27 of 30 have a current status · 3 not recorded · MD in 5 days", Full / Doubtful / Ruled out, one athlete a row with the status word and restriction line, the medic's reason line under "Medical · visible to medical staff", the week card leading with "No match in the next 14 days · Next fixture …" when there is no matchday; D8 below). **Remaining:** the week strip yielding on a heavy morning. | The remaining step is its own layout decision on the board; it needs no new data. | Next in the series: the week strip yielding · small |
...
| STAFF-SS-01 | D6 | The nutritionist's lead card "keeps the three availability counts with no names, because a nutritionist does not pick a team" (board frame 7). | `docs/access-matrix.md` §4.2 withholds "the availability split and its named lists, MET-013, wherever it appears" from the nutritionist — a permission rule, which outranks the board. Built 2026-09-13 as the matrix says: the Available tile and the whole Ready-for card are absent for a nutritionist-only account. | **Keep the matrix.** If the counts-without-names are wanted, §4.2 changes first (a permissions edit, with its test), then the card gains a nutritionist shape · small after |
