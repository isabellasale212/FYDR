# Design brief — STAFF-SS-03, Add an athlete
...
1. **Date of birth stays `required`.** The document once said an empty date made
   the athlete a minor silently; the form already prevents that, and it must go
   on preventing it — the age gates fail safe on a null and that safety should
   never be exercised by this form.
2. **The email hint stays** — "Leave blank to add them to the roster with no app
   access. You can invite them later from their profile." — it is the one line
   that explains what the optional field decides.
3. **`method="post"`** stays on the form.

## 5. What a proposal should address

1. **Nothing of substance at desktop.** Six fields, two buttons, one hint.
2. **Phone width** — see §0af.
