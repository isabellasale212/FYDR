 ...
 * adds their glyphs and draws the desktop sidebar and the 64px rail. */
...
/* The rows, from rows.ts (the shared table), each with its glyph. */
export const SIDEBAR: readonly Row[] = SIDEBAR_ROWS.map((row) => ({ ...row, icon: ICONS[row.id] ?? null }));
