# Design brief — STAFF-NUT-05, Athlete profile — weigh-ins only

**For Claude Design.** Everything below is the current state, measured on the
running application on 2026-09-12 at **1280×900 and 375×812** as the nutritionist (Sana
Mirza, one role). Nothing here is aspirational.
...
design surface; this brief carries what differs for the nutritionist — and what the sport scientist's review
could not see.
...
*From the nutritionist section (identical table):*
...
**Full review: `docs/walkthrough-reviews/staff-nut-01-to-33-review.md`.**
...
1. **The nutritionist's profile is eight panels**: the censored Injury panel included (decided 2026-09-06), Availability and SAR absent, bio and corrections blocked with a reason, the Body-weight trio live. A proposal keeps absence for what the role cannot see and a stated block for what it can see but not change.
2. **The Gym tab is not this role's** (`ATHLETE_GYM`) — it goes, rather than refusing (§0av).
...
1. **The nutritionist's order** in the role-ordered panel library (SS-02-05 C4): Body weight, Nutrition plan, Entries and corrections (read), then the rest — the inventory gives eight panels.
2. **One working tab of three** on the profile header for this role; whether the header varies by role.
