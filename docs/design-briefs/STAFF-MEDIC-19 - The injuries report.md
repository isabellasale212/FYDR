# Design brief — STAFF-MEDIC-19, The injuries report
...
**Read the STAFF-SHELL brief and the STAFF-SS-19 brief first.** The phone shell
...
1. **Censored by construction for every role, the medic included** — the report never selects `injury_clinical`; the medic's PDF adds a confidentiality banner and a body-area count and nothing else. A proposal keeps the report clinical-free for all five roles.
2. **The "MEDICAL IN CONFIDENCE" banner stays on the medic's PDF.**
...
1. **The DETAIL fallback "Restricted" for an unknown status** (§0ay) — the word for nothing recorded is "Not recorded".
2. **Nothing medic-specific otherwise**; the SS-19 brief is the design surface.
