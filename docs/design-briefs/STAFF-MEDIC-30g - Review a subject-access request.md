# Design brief — STAFF-MEDIC-30g, Review a subject-access request
...
**Read the STAFF-SHELL brief and the STAFF-SS-30 brief first.** The phone shell
...
1. **The review is `CLINICAL_ONLY` and refuses the admin** (measured); the admin administers, the medic reviews — neither completes alone. A proposal keeps the two halves in two roles.
2. **Withholding needs a positive act and a reason** (0032) — a proposal keeps the reason required on "withhold".
...
1. **The entry point**: on a released request the list links to nothing for either role; what a pending request shows the medic (a "Review" control, a due date) was not observable on scratch — the proposal draws it.
2. **Whether the admin should read the withholding reason at the database** is a question for Isabella (questions file, item 10), not a design matter.
