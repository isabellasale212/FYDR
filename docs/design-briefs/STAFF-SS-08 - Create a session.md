# Design brief — STAFF-SS-08, Create a session
...
1. **Duration becomes required** (§0ah, decided). The default of 60 stays; the
   field cannot be emptied.
2. **Name, date and time stay validated inline with focus** — "Give the session
   a name." / "Set a date and time." — and the same treatment extends to
   whichever of type and groups are made required (§0ai).
3. **Date pre-fills from `?date=`** so a session created from a day in the grid
   lands on that day.
4. **"Cancel" returns to `/schedule` with nothing written.**

## 5. What a proposal should address

1. **Which of type and groups are required**, and how the form says so before
   submit rather than after.
2. **"Rehab" appearing in both chip rows** — whether the group needs a
   distinguishing label when it sits under "Who's in it".
