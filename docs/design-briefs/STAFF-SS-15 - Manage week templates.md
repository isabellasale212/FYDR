# Design brief — STAFF-SS-15, Manage week templates
...
1. **Apply goes through a preview** (`/schedule/planner/apply`), never straight
   onto the week.
2. **The read-only branch** for roles without `SESSION_EDIT` renders only its
   sentence — no controls in the DOM.

## 5. What a proposal should address

1. **The single-template case** is what scratch shows; a proposal should design
   for several and for none ("No templates yet." on the toolbar).
