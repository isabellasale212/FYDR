...
export const metadata = { title: 'GPS report · Fydr' };
...
async function GpsReportPageContent({ searchParams }: { searchParams: SearchParams }) {
  ...
  const header = (tabsNode?: React.ReactNode, period?: React.ReactNode, exportDescriptor?: ExportDescriptor) => (
    <ReportHeader
      groups={groups}
      groupIds={groupIds}
      title={mode === 'training' ? 'GPS report' : 'Match day GPS report'}
      /* PATTERN-S7 C1, reconciled 2026-09-13 against the catalogue's addendum:
         the per-session GPS board is the GPS report and carries the confirmed
         GPS sentence. The match board is kept (decision batch) but its sentence
         is not yet written, so it draws none. */
      definition={mode === 'training' ? (reportDefinition('gps') ?? undefined) : undefined}
      sub={
        <>
      ...
      actions={
        <>
            ...
            <Link href={`/reports/gps${q({ mode: 'training', groups: groupsQs })}`} aria-current={mode === 'training'}>
            Training
            </Link>
            <Link href={`/reports/gps${q({ mode: 'match', groups: groupsQs })}`} aria-current={mode === 'match'}>
            Match day
            </Link>
            </div>
            ...
            <Link
            href={`/reports/gps${qs({ mode, session: sessionParam, groups: groupsQs, heat: heatPref ? 'off' : undefined })}`}
            ...
            {exportDescriptor ? (
              <ExportDialog href={`/reports/gps/export${q({ mode, session: sessionParam, groups: groupsQs })}`} descriptor={exportDescriptor} />
            ) : (
              <a href={`/reports/gps/export${q({ mode, session: sessionParam, groups: groupsQs })}`} className="rhead-btn">
                Export CSV
              </a>
            )}
            <a href={`/reports/gps/pdf${q({ mode, session: sessionParam, groups: groupsQs })}`} className="rhead-btn">
            Export PDF
            </a>
        </>
      }
    ...
    return (
      <>
        {header(
          ...
          <Link
          key={s.sessionId}
          href={`/reports/gps${q({ mode: 'match', session: s.sessionId, groups: groupsQs })}`}
        ...
        {!overview ? (
          ...
          <EmptyState
            ...
            action={groupIds.length > 0 ? { href: `/reports/gps${qs({ mode: 'match', session: sessionParam })}`, label: matchFilterEmpty.action!.label, clearsGroupFilter: true } : null}
          />
        ) : (
          <>
  ...
  const trainingTabs = (activeRange: 'day' | 'week') => (
    <div className="tr-mode-switch">
      <Link href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, range: 'day' })}`} aria-current={activeRange === 'day'}>
        Day
      </Link>
      <Link href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, range: 'week' })}`} aria-current={activeRange === 'week'}>
        Week
      </Link>
    </div>
  );
    ...
    <>
      ...
      {!overview ? (
        ...
        <EmptyState
          ...
          action={groupIds.length > 0 ? { href: `/reports/gps${q({ mode: 'training', session: sessionParam })}`, label: trainingFilterEmpty.action!.label, clearsGroupFilter: true } : null}
        />
      ) : (
        <>
          <div className="card tr-overview">
                  ...
                  return (
                    <div key={unit}>
                      ...
                      {inUnit.map((row) => (
                        ...
                        <Link
                          key={row.athlete_id}
                          href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, scope, lens, athlete: row.athlete_id })}`}
...
<div className="card" style={{ marginTop: 'var(--sp-14)' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--sp-10)' }}>
              ...
              <div className="tr-scope-chips">
                {([
                  ...
                  <Link
                    key={key}
                    href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, scope: key })}`}
          ...
          <div className="card" style={{ marginTop: 'var(--sp-14)' }}>
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', flexWrap: 'wrap', gap: 'var(--sp-10)' }}>
                ...
                <div className="tr-scope-chips">
                  <Link
                    href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, scope, lens: 'self', athlete: selectedAthleteId ?? undefined })}`}
                    className="tr-scope-chip"
                  ...
                  <Link
                    href={`/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, scope, lens: 'position', athlete: selectedAthleteId ?? undefined })}`}
                    className="tr-scope-chip"
              ...
              <div style={{ marginTop: 'var(--sp-14)' }}>
                <TrainingScatter
                  ...
                  lens={lens}
                  hrefFor={(id) => `/reports/gps${q({ mode: 'training', session: selected.sessionId, groups: groupsQs, scope, lens, athlete: id })}`}
                />
              </div>
            </div>
                  </>
