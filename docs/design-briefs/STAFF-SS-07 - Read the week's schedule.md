# Design brief — STAFF-SS-07, Read the week's schedule
...
1. **Edit mode on load for `SESSION_EDIT` holders; the Read/Edit control absent
   from the DOM for everyone else.** Not hidden — absent.
2. **The two "+ Session" entry points stay distinct** — the header chip opens
   the full form, the toolbar starts a grid draft. Do not merge them.
3. **The group filter is the same global cookie** as everywhere else.

## 5. What a proposal should address

1. **"Week plan" / "Today" as a real view switcher** with tab semantics and
   `aria-current` (§0ai).
2. **3,692px at phone** under a 640px sidebar — which is the shell brief's
   problem, but the week grid's own phone layout is this flow's.
