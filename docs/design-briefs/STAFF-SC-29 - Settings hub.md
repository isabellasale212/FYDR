# Design brief — STAFF-SC-29, Settings hub
...
1. **Seven sections, five rows for the S&C**; absent rows absent; the two that go nowhere (Thresholds, Integrations) become absent too (§0av).
...
1. **A hub whose three live rows are Exports, Groups (read) and Notifications** on a 2,804px page.
