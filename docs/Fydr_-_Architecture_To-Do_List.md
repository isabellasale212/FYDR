...
- [ ] **The other two thirds of the reported schedule bug are NOT fixed, and (c) is the one that actually caused the report.** The report was "no Save button, an unresponsive one, no Cancel option". The `handlePublish` fix above was real but latent; (c) is what a coach actually hits.
  ...
  **(c) `mode` defaults to `'read'`, AND THIS IS THE ACTUAL BUG — it accounts for all three complaints at once.** Confirmed live, 2026-09-09, as a coach: `/schedule` loads with the Read/Edit toggle on **Read** (`aria-pressed="true"`). Clicking a session in that state highlights it and renders **nothing** — `.sg-stepper-btn` count 0, `.sg-panel-actions` buttons 0, no Publish, no Discard, no read-only fields, no panel at all. So the coach clicks a session, finds no Save, nothing responds, and there is no Cancel: *"no Save button, an unresponsive one, no Cancel option"*, exactly as reported, from one cause.

  **The earlier note that this was "mitigated by click-to-switch at line 424" is wrong** — clicking a session in Read mode did not switch to Edit. In Edit mode everything works: selecting the same session gave steppers (09:30 / 75 min), bumping duration to 80 made "Publish to athletes" and "Discard" appear, and Discard (with its "Yes, discard" confirm) cleared it cleanly.

  **The fix is a product decision, not a mechanical one**, which is why it is still open: default a coach to Edit, or make Read mode say it is read-only rather than rendering an inert panel. Either answers the report; the second also keeps a deliberate read-only view for roles outside `SESSION_EDIT`.
