...
| PATTERN-S6 | C8 | Staff empty states to the one grammar (what, why, what would fill it, the most recent on record, "Nothing is missing", one action that widens the window). | A most-recent-on-record read per screen. | Sweep, one screen a commit; the athlete's My data version is 12 C6 · medium per screen |
| PATTERN-S6 | C9 | One `role="status"` and one `role="alert"` region per surface, written on transition only. | 19 / 77 files carry one each today. | Audit, then restructure · medium |
