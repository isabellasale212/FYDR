...
**C7 built in full** — `/denied`, where every staff role gate lands, and the denial log
(0112) whose reference the screen shows and the audit log finds.
...
## PATTERN-S11 — Installability and offline: Step 1 inventory (2026-09-13)

**Source.** `docs/platform-decision.md` (Isabella, 13 Sept): both apps installable web
apps with a robust offline outbox and push. No board yet; this is the inventory, from the
code and the running app on scratch, of what exists today. Nothing designed here.

### What `manifest.webmanifest` actually contains

Generated by `src/app/manifest.ts` (Next's metadata route); served `200
application/manifest+json`. Its whole content:

```
name "Fydr" · short_name "Fydr" · description "Athlete performance management for sports clubs."
start_url "/" · display "standalone" · background_color #202b4e · theme_color #202b4e
icons: /icon.svg (sizes any, image/svg+xml), /apple-icon.png (180×180, image/png)
```

Both icon files exist and serve 200 (`src/app/icon.svg`, `src/app/apple-icon.png` 180×180
RGB PNG; `opengraph-image.png` 1200×630 beside them). Not in it: no PNG at 192 or 512 (the
Chromium install criteria want a 192 and a 512 PNG; an `any` SVG alone does not satisfy
them on every version), no `purpose: maskable` icon, no `id`, no `scope`, no
`orientation`, no `screenshots`, no `shortcuts`, no `categories`. `start_url "/"` routes
by role on load, which is fine. No `apple-mobile-web-app-capable` /
`apple-mobile-web-app-status-bar-style` meta in `src/app/layout.tsx` (Safari 16.4+ honours
the manifest's `display`, older builds need the meta); the apple-touch-icon link is
emitted by the app-dir convention. **No splash screens**: no `apple-touch-startup-image`
links for any device size; iOS generates none from the manifest, so an installed athlete
app opens on a blank ground the colour of `background_color`. No install prompt handling
anywhere: nothing listens for `beforeinstallprompt` (Chromium) and nothing on any screen
teaches the iOS Share › Add to Home Screen path.

### Service worker

**None.** `/sw.js` 404s; no file under `src/` or `public/` (there is no `public/`
directory) registers or defines one; nothing calls `navigator.serviceWorker`; `package.json`
has no next-pwa, serwist or workbox. Consequences today: no offline app shell (a navigation
with no network lands on the browser's own offline page — §0al found exactly this on the
schedule's publish), no background sync, no push subscription possible, and the installed
app has no caching of its own beyond the browser's.

### Every offline queue in the codebase, and where it lives

All four queues are **`localStorage`**, one key per domain, all in `src/lib/outbox.ts`,
all flushed by one client component:

| Queue | Key | Written by | Flushed by | Storage |
|---|---|---|---|---|
| Morning check-in | `fydr-outbox-wellness` | `CheckInForm` (enqueue on submit) | `OutboxFlusher`, mounted on `/today` only | localStorage |
| Session RPE | `fydr-outbox-training` | `RpeForm` | `OutboxFlusher` | localStorage |
| Weekly nutrition check-in | `fydr-outbox-nutrition` | `NutritionCheckinForm` | `OutboxFlusher` | localStorage |
| Gym set logs | `fydr-outbox-gym-set` | `GymSessionLogger` (`onMutate`, with the session context since S6 C1) | `OutboxFlusher` and `lib/gymOutboxFlush.ts` from the logger | localStorage |

Beside them, not queues: the last successful send (`fydr-outbox-last-sent`, S6 C1, for the
queue screen's empty state); the gym logger's per-log stepper draft
(`fydr-gym-draft-<gymSessionLogId>`, localStorage, cleared on finish); the schedule
workspace's **held week** (`fydr-schedule-pending-<org>-<weekStart>`, **sessionStorage**,
S6 C6 / §0al — staff, restored on reload, cleared on publish or Discard, never sent by
itself); the theme (`fydr-theme`), the athlete's own leaderboard-hide flag, a once-per-session
login flag (sessionStorage), and the group-filter cookie.

How the flush behaves: on mount of `/today` and on the window's `online` event; every item
retried on every flush with no schedule, no backoff, no cap and no expiry (S6 C10 on the
sheet); a duplicate-key hit is resolved against the live row and a genuine conflict is
flagged and shown once on Today with Discard (§0aa, S6 B2/C9); a queued item is dequeued
only when its own write lands. Reconnect detection is the `online` event and page load.
**Staff writes are not queued at all** — the schedule's failed publish holds the week and
offers Try again (S6 C6); every other staff write fails in place.

### Against the platform decision, what exists and what does not

| The decision asks for | Today |
|---|---|
| Manifest, icons, splash, standalone, offline app shell | Manifest and two icons, standalone; no 192/512 PNG, no maskable, no splash, no shell |
| One shared foundation for both apps | Four per-domain athlete queues; no staff queue |
| IndexedDB | localStorage (four keys) |
| A service worker | None |
| Retry with backoff | Retry every flush, no schedule |
| A visible queue state | Yes: the count on Today and `/today/waiting` (S6 C1) |
| A manual "send now" | **No — and the S6 board deliberately has none** ("the send is not the athlete's job"; the queue screen says "there is nothing to press"). The decision, later, asks for one: **⚠ decision** — the platform decision wins as the later ruling, but say which |
| The last successful sync time | Yes, on the queue screen (`fydr-outbox-last-sent`) |
| Append-only writes first | The four athlete writes are append-only inserts with client ids; corrections (revise_*) are online-only by design |
| Push: service worker, VAPID keys, a subscription table, a send path, per-person preferences, quiet hours | None of the first four. `notification_preferences` exists with the quiet-hours columns and no screen sets them; `push_tokens` holds 43 seeded rows from the abandoned Expo plan that no code has ever written |
| iOS: teach the install on screen; say push needs the installed app | Nothing on any screen |

### Open, for the board

1. "Send now" against the S6 board's "nothing to press" — which stands.
2. Whether the held staff week (sessionStorage, §0al) becomes a queued staff write under C4 or stays a held draft.
3. The install-teaching surface: where the iOS Share › Add to Home Screen instruction lives (Today? Me? sign-in?).

---

## PATTERN-S12 — Premium: Step 1 inventory (2026-09-13)

**Source.** Isabella, 13 Sept: "there is no complete statement anywhere of what premium
contains, what a free club sees where a premium feature would be, and what happens at the
boundary when a club downgrades with premium data already in the database." From the code
and the live policies on scratch, not from the specs. Nothing designed here.

### The tier itself

`organisations.tier`, enum `subscription_tier` = `core` | `performance` (scratch: two
performance, one core). The words on screen: Basic / Premium (`lib/tier.ts` `tierLabel`).
Read per request by `requireStaff` / `requireAthlete`; staff see the *effective* tier — the
real one, or Basic when the `fydr-tier-preview` cookie is set (the Plan card's preview,
`lib/tierPreview.ts`); athletes always the real one. Nothing in the product changes the
tier: no UI, no billing ("Billing: out of scope permanently", `architecture/to-do.md`); the
hub says "Plan changes are a sales conversation with your Fydr contact." A change is a
database update to that one column.

### What premium contains, by gate (the app)

Every gate is `isPremium(tier)` in the app, plus the one database rule below.

| Feature | Basic sees | Where the gate is |
|---|---|---|
| **Analytics** (the whole destination) | The sidebar row is absent (`PREMIUM_ONLY = {staff.analytics}`, sidebar and phone shell); `/analytics` typed refuses with a full-page `PlanGate` upsell | `Sidebar/rows.ts`, `StaffPhoneShell/shell.ts`, `analytics/page.tsx` |
| **Analytics bar chart** (inside Analytics) | A `PlanGateCard` in the chart's place; the table view of the same numbers stays | `lib/analyticsBuilder.ts chartIsPremium`, `analytics/page.tsx`, `analytics/build/page.tsx` |
| **Training report** (the GPS session board, both modes) | The reports hub card badged Premium; `/reports/training` refuses with a `PlanGate`; its CSV and PDF routes answer `403` (`premiumOnlyResponse`) | `reports/page.tsx`, `reports/training/*` |
| **GPS panel on the athlete report** | A locked panel in words where the GPS figures would be | `reports/athlete/[athleteId]/page.tsx` (and the PDF) |
| **GPS vendor import** (Catapult, STATSports, Polar CSV) and the parsed-record export | `/settings/imports` refuses with a `PlanGate`; upload, template and batch-export routes `403`; the hub's two GPS rows read Locked with a Premium badge | `settings/imports/*`, `settings/page.tsx` |
| **GPS leaderboards** (`gps.*` metrics) | The new-board catalogue omits gps metrics; an existing gps board's page, CSV and PDF are gated; the athlete's My data omits gps boards | `leaderboards/new`, `leaderboards/[id]/*`, `(athlete)/my-data/boards/*` (`gpsMetricBlocked`) |
| **The Plan card** | "Basic · wellness, gym, nutrition, schedule, reports and exports." vs "Premium · GPS, the training report and the analytics bar chart are on." | `settings/page.tsx` |

Everything else — wellness, RPE, nutrition, gym, schedule, the other five reports and
their exports, leaderboards on non-GPS metrics, flags, thresholds, audit, retention,
subject access, the athlete app — is on both tiers. Apple Health, the one other Premium
line, is removed from the product (13 Sept).

### What premium contains, by gate (the database, live on scratch)

`pg_policies` on scratch: **no policy on any table reads the tier** (nothing in any
`qual` or `with_check` mentions premium). Three functions do: `auth_org_is_premium()`
(0061, a read of `organisations.tier`), `consent_write_allowed()` (0061 — the
`healthkit_sync` consent, now dormant with Apple Health removed) and
`compute_leaderboard()` (0094 — a `gps.*` board for a `core` organisation raises
`metric_not_eligible`). So the only live revenue rule at the database is the leaderboard
one. `gps_records` reads are **deliberately not tier-gated** (0061's header: a downgraded
club must still answer a subject access request, `sarPackAssembly.ts` reads them) — which
means a Basic club's staff token can read `gps_records` straight from PostgREST; the
training report's and the athlete report's GPS gates are app-only. Tests: `330_tier_rls_test`
(19 assertions, the "14 passing" of the brief plus later ones), `500_compute_leaderboard_tier_gate_test`,
`scripts/verify-tier-rls.mjs` (the healthkit half is now dormant), `scripts/test-premium-routes.ts`.

### The downgrade boundary, as the code behaves today

*(Ruled while this inventory was being written: `docs/decisions/premium-downgrade.md`,
Isabella, 13 Sept — premium data is **kept and hidden** under D-20, restored in full on
upgrade, and kept only under the club's normal retention clock; delete-after-grace and
read-only-nag rejected; S12 is the mechanism, not the contents. What follows is what the
code does today against that ruling.)*

A downgrade is `update organisations set tier = 'core'`. Then, immediately (the tier is
read per request, not from the token):

- **GPS data stays.** `gps_records`, `gps_import_batches` rows are neither deleted nor
  hidden by RLS. They stop being shown: the training report, the athlete report's GPS panel,
  the imports screen and its export refuse with the Premium words; a gps leaderboard's page
  gates and `compute_leaderboard` refuses. Nothing tells the club, in one place, "your N
  GPS sessions from <dates> are kept and not shown; they return on upgrade" — the sentence
  lives only in the individual PlanGate copies ("Basic clubs…").
- **Analytics disappears** from the nav and the URL; there is no saved-analytics entity
  to orphan.
- **Leaderboards**: a gps board keeps its rows and its `publish` state; staff who open it
  see the gate; athletes stop seeing it in My data. A board on a non-GPS metric is untouched.
- **Consents**: `athlete_consents` rows stay (0061: never destroy a consent record).
- **The athlete app** loses only gps boards; nothing else it shows is Premium.
- **Exports**: the SAR pack still includes gps rows (by design); the staff data-export
  builder's GPS section — gated? (`settings/exports/generate` gates on report access, not
  tier; the GPS sheet there needs checking against D-20 when S12 is designed).
- **Upgrade** is the same update the other way; everything reappears with its history.

### What D-20 (`architecture/to-do.md` §1) says and where the code departs

D-20: a wholly premium destination disappears from the sidebar and refuses at the URL; a
premium region inside a base page shows an upsell card and never vanishes silently. The
code follows it for Analytics (wholly premium) and for the bar chart, the athlete report's
GPS panel and the hub rows (regions). The training report is a *destination* that stays on
the hub as a badged card and refuses at the URL — a region-shaped treatment of a
destination; either the card goes (D-20) or D-20 gains a "badged card" case. Imports
likewise (hub rows Locked, URL refuses).

### Open, for the board

1. ~~The one-place downgrade statement~~ — ruled (`premium-downgrade.md`): kept and hidden,
   restored on upgrade, under the normal retention clock. What remains is where the
   sentence lives on screen.
2. Whether `gps_records` reads should be tier-gated at the database for staff tokens while
   the SAR path keeps a service-role read — or stay app-only, stated. ("Kept and hidden" is
   app-only today.)
3. The training-report card on the hub under D-20.
4. Whether the staff data-export builder's GPS sheet is Premium.
5. Who flips the tier, and whether the flip writes an audit row (it does not today: no
   trigger on `organisations`, excluded from the audit batch "for shape").
6. Retention: the club's retention policy today runs per athlete record (`retention.*`);
