...
ExtraBold wordmark in `--text`, the trace running beneath it in the **solid** accent, ending
in the ringed dot to the right of the "r" — the mark's dark-theme "one flat ink" treatment,
not the sidebar's translucent tints (Isabella's call, 15 September, after seeing the first
build at 60pt: at 0.45 the trace was a faint grey line and the ringed dot had effectively
disappeared at the size the icon is actually seen at; rendered at 0.45, 0.7, 0.85 and 1 and
read at 60, 120, 180 and 512, full strength is the first tone at which the ring reads at 60pt,
and at 512 the thin trace still sits behind the word). The wordmark is **outlined paths**,
...
inside the ring. Full-bleed squares with no rounded corners (iOS and
...
| No print styles | Clubs print the availability board | **Superseded 14 September 2026 (PATTERN-S7 C4, ruled 13 September, batch B10): one renderer for print and PDF.** The `@media print` blocks are gone from both stylesheets; every Print control (`components/PrintLink`) opens the screen's PDF in a new tab (`?open=1`, an inline disposition) and printing is done from the viewer — two steps, accepted. The availability board is its own PDF (`/dashboard/pdf`), the availability history too; Injuries opens the injury report's PDF; the testing screens open theirs. A dark-mode user cannot print a dark sheet because nothing prints the screen. The board's approved `--print-paper`/`--print-ink` tokens (S7 A18) are therefore not needed in CSS; the PDF's own light palette is `PDF_COLOR` in `lib/pdf.tsx`. |
