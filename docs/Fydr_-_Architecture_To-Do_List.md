...
- [ ] **Nothing at the database stops a new set landing on a closed gym session log; only the screen does.** Isabella's rule, 2026-09-12: correcting a set on a complete session is decided and fine; *adding* one is not — it would mean "complete" means nothing and adherence changes after the fact.
