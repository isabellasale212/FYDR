# Design brief — STAFF-SS-13, Publish the week to athletes
...
1. **"Nothing changes on their phone until you publish."** stays, verbatim in
   substance, in the banner, first thing on the schedule.
2. **"Published" is disabled when nothing is pending** — the banner must never
   offer a no-op publish.
3. **The banner text renders for every role; the actions only with
   `SESSION_EDIT`.** Absent from the DOM, not hidden.
4. **A failed publish returns the button to enabled** and says "Not
   published:" in a `role="alert"`.

## 5. What a proposal should address

1. **Nothing in the design.** The banner is the single most important control
   in the staff app and it is right. Sizes are the shell's (STAFF-SHELL §5.4).
