# Design brief — STAFF-SS-05, Open an athlete's profile
...
implemented, and a proposal for this flow at phone width must say which shell it
assumes.
...
**Full review: `docs/walkthrough-reviews/staff-ss-05-to-08-review.md`** — covers
...
1. **The clinical boundary is not a layout choice.** Diagnosis, mechanism and
   problem reports render only for `CLINICAL_ONLY`; a proposal must not
   introduce a slot for them on the sport scientist's view.
2. **Seven permission sets decide what renders.** The ten panels and eleven
   controls measured are the *sport scientist's* profile; other roles see
   fewer. A proposal designs the superset and must say how each panel degrades.
3. **The desktop pairing of Flags and Athleticism at the top stays** — it is
   what this persona reads first.
4. **Every write control keeps its panel** — availability chips with
   availability, "Correct check-in" with entries. Do not collect actions into a
   bar.

## 5. What a proposal should address

1. **Seven screens at phone width**, with availability at y=2706 and
   corrections at y=5102 — the two panels a pitch-side admin needs most are
   furthest down.
2. **Nothing on the screen says which panels are read-only for which role.**
   The document assumed nutrition was; for this role it is not.
