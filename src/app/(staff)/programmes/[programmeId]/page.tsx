...
import { fetchAssignments, fetchExercises, fetchProgrammeDetail } from '@/lib/queries/programmes';
 ...
 *  lib/queries/programmes.ts's header for exactly what and why (no
 *  exercise_overrides, no change-event/divergence tracking, no drag reorder).
 *  Edit affordances are hidden for medical on a gym programme and for coach on
 *  a rehab one — a client-side courtesy; migration 0022's RLS is what actually
...
export default async function ProgrammeBuilderPage({
  ...
  const [exercises, assignees, athletes, groups] = await Promise.all([
  ...
  return (
    <>
      ...
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 14 }}>
        ...
        <span className="pill pill-neutral">{enumLabel(detail.programme.status)}</span>
