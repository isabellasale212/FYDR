# Design brief — STAFF-SS-28, Injuries
...
1. **The sport scientist sees the coach column** — the list, Print, Team allocation, Rehab groups; no "+ Injury", no problem reports. Content gated by `CLINICAL_ONLY` stays invisible, not greyed.
2. **The list prints** — `.injuries-board` has its own 9pt print rules for the physio's wall; a proposal may not restructure the row into something the print block does not cover without saying so.
...
1. **The two `.tiny` links at 19px** (Team allocation →, Rehab groups →) — in the sweep; they are the two places a sport scientist goes next from here and deserve to look like it.
2. **Nothing else.** The document was right and the screen fits one desktop viewport.
