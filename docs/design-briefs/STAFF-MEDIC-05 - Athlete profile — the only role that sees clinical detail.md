# Design brief — STAFF-MEDIC-05, Athlete profile — the only role that sees clinical detail

**For Claude Design.** Everything below is the current state, measured on the
running application on 2026-09-12 at **1280×900 and 375×812** as a medic (Ruth
Callaghan, one role). Nothing here is aspirational.

**Read the STAFF-SHELL brief and the STAFF-SS-05 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. Where the medic's
screen is identical to the sport scientist's by measurement, the SS brief is the
design surface; this brief carries what differs for the medic — above all the
clinical content only this role sees — and what the sport scientist's review
could not see.
...
*From the medic section (identical table):*
...
*The flow as the sport scientist section states it:*
...
**Full review: `docs/walkthrough-reviews/staff-medic-01-to-05-review.md`.**
...
1. **The clinical Injury panel is the medic's alone, and the database says so** (0063; verified for all five roles 2026-09-12). A proposal keeps every clinical field inside that panel — never in the header, the roster, an export or a PDF.
2. **The medic's write set on the profile** — availability (clinical path), weigh-ins, corrections, bio, the injury's Edit / Manage — stays; the SAR section stays absent.
3. **Body mass is not the coach's** (Q27) — irrelevant to the medic's render but the same panel library serves both.
...
1. **The physio's first question is the sixth panel down** (6,134px at phone): the STAFF-SS-02-05 board's role-ordered panels (C4) is where this is answered; the five-role inventory (`docs/walkthrough-reviews/athlete-profile-role-inventory.md`) gives the medic's order — Injury and Availability first.
2. **Two links, "Edit" and "Manage injury & programme →", to the same page** — one control, or two clearly different acts.
