# Design brief — STAFF-COACH-15, Manage week templates
...
**Read the STAFF-SHELL brief and the STAFF-SS-15 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-15 brief is the
...
1. **Identical to STAFF-SS-15 by fingerprint**, both widths; read-only for medic, S&C and nutritionist, stated in place — a proposal keeps the stated refusal, never a blank.
2. **The apply screen's three modes stay named by what they do** (Add alongside / Replace planned / Fill gaps only).
...
1. **Saturday evening:** with next week empty, "Apply" on the one template is the fastest fill; whether the empty next week on `/schedule` points at it.
2. **One template on scratch** — the list with five is not drawn; a proposal draws the list at three and at ten.
