# Design brief — STAFF-COACH-08, Create a session
...
**Read the STAFF-SHELL brief and the STAFF-SS-08 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-08 brief is the
...
1. **Identical to STAFF-SS-08 by measurement**, both widths; at 375 every control on the form clears 44px. The SS-08 brief is the design surface.
2. **The toolbar wizard is the phone path** and its three steps already sit at the floor; the two unlabelled inputs (§0aj) are build work, not design.
3. **A session with no group is refused** (`eff27bb`) and duration is required (`586520d`); a proposal keeps both refusals visible in place.
...
1. **The silent add on a phone** (§0aw): after "Add to {day}" the wizard closes and the confirmation panel sits ~3,000px below. Where the coach's eye goes next — a sheet, a scroll, a toast that names the day — is the question.
2. **Saturday evening, next week empty:** the wizard, "Week templates" and "Duplicate" are the three ways to fill it; whether the empty next week says so.
