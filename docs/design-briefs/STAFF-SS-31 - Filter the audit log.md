# Design brief — STAFF-SS-31, Filter the audit log
...
1. **Type chips are the log's own entity types** — derived, not a fixed list; a proposal draws the row for a club whose log has three types and one whose log has twelve.
2. **Append-only stays stated on the page** — "nothing here can be edited or deleted, by any role" is the sentence a safeguarding reviewer needs to see.
3. **Actors are the full roster, not the names in the log** — so "did X ever touch this record" can be answered as a confirmed no.
...
1. **6,287px at phone** — the longest settings screen; the filter block (chips, two dates, two selects, search, three buttons) sits above every row, and the rows are what the page is for.
2. **No session or schedule type** — nothing to draw until §0al lands, but the proposal should leave room for the chip row to grow.
3. **Pagination as two ghost links** — whether page position is stated anywhere.
