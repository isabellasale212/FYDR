# Design brief — STAFF-SS-22, Training report
...
**This flow is one of the six reports reviewed together.** The flow text below is
the shared entry for STAFF-SS-17 … 22 in the walkthrough document, which carries
each report's inventory.
...
**Full review: `docs/walkthrough-reviews/staff-ss-17-to-22-review.md`.**
...
## 4. The constraint every report proposal must satisfy

1. **Every page, CSV and PDF handler stays behind `requireReport('{key}')`.**
   Eighteen handlers were checked; a proposal must not add a nineteenth surface
   without it.
2. **The period select keeps its self-explaining option labels** — they are why
   the narrow choice is rarely wrong-footed. Extend the pattern; do not flatten it.
3. **Exports carry their scope** in the query string and, for CSV, in the file's
   first line. Keep both.
4. **The group filter is the shared cookie** (§0ak, decided) — a report reads and
   writes it like every other multi-athlete screen; `?groups=` overrides one load.

## 5. What a proposal should address

1. **4,711px at phone width** — six chart sections. The phone question for
   this report is whether it has a phone reading at all, or whether the phone
   use is "send the PDF".
2. **Two selects and a mode toggle above six sections** — the controls decide
   everything below; at phone width they scroll away.
3. **"GPS · premium"** on the `/reports` card is a plan gate; nothing on this
   screen shows the gate for an account that has it.
