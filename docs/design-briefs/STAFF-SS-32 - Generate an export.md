# Design brief — STAFF-SS-32, Generate an export
...
1. **Six domain checkboxes, the group filter as "who", a date range as "when", one Generate** — the hub row's own summary; a proposal keeps the three questions in that order.
2. **The scope is read back in the heading** ("Whole squad · 30 athletes in scope") — keep it; it is the one line that stops a wrong export.
3. **The role sentence names the role correctly or not at all** — "Coach access" for a sport scientist is filed (§0ap).
...
1. **Native checkboxes at 15×13** with the label as the target — the row is fine as a target; the proposal shows the checked/unchecked state readable at a glance at 375.
2. **2,293px at phone** — six checkbox rows with descriptions, then the dates; whether the descriptions collapse.
3. **"Generating…"** as the in-flight state on a button that downloads six files — what the page says when it is done.
