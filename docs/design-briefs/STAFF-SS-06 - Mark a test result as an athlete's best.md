# Design brief — STAFF-SS-06, Mark a test result as an athlete's best
...
1. **Confirm in place, with Cancel.** "Mark best" → "Mark as best" + "Cancel"
   on the same row; keep both.
2. **"lower is better on this test" stays beside the trend** — it is what makes
   "+0.6%" readable as an improvement.
3. **Print / Export CSV / Export PDF stay** — the page is a record other people
   receive.

## 5. What a proposal should address

1. **Nothing.** Verified in full; the control is on the row it applies to and
   backs out cleanly. Recorded so the flow is not redesigned for its own sake.
