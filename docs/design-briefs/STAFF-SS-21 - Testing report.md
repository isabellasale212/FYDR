# Design brief — STAFF-SS-21, Testing report
...
1. **"+ Log a result" is a write on a report screen** — the only one of the
   six. Decide whether it belongs here or on the test page.
2. **Per test definition** — the export carries `test={id}`; the proposal
   should make the current test as visible as the period.
