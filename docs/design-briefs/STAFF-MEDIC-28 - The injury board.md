# Design brief — STAFF-MEDIC-28, The injury board
...
**Full review: `docs/walkthrough-reviews/staff-medic-19-to-30g-review.md`.**
...
1. **The medic-only content is database-enforced** (problem reports, notes, the clinical record) and the page shows it as sections, not as unlocked columns — keep the structural difference.
2. **"+ Injury" is on the board for the medic today and, by the 2026-09-12 decision, will be for every `INJURY_ACCESS` role** (the coach's form non-clinical, S3 C9).
3. **Team allocation stays read-only for the medic**, stated in place; rehab-group allocation stays the medic's and the S&C's.
...
1. **The injury's own page is the physio's working surface** — clinical record, timeline, the availability write set with the coach-visible hint — 2,450px at phone. PATTERN-S3 C2 (the status screen) and C3 (the stage ladder) are its redesign; this brief points at them.
2. **Problem reports on a phone**: four open reports with three controls each on the board, above the injury list — what the physio reads first on the touchline.
