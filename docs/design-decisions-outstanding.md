...
| STAFF-SS-01 | C2 | The dashboard rebuild as a series. **Done:** A1, A2, A4; the thresholds line ("Thresholds set by Mark Iremonger · Wed 2 Sept · Change ›", `fetchThresholdProvenance`, 2026-09-12). **Remaining:** the matchday lead card, the week strip yielding on a heavy morning, the role versions (S&C four toggles, nutritionist two). | Each remaining step is its own layout decision on the board; none needs new data. | Next in the series: the role versions · medium each |
