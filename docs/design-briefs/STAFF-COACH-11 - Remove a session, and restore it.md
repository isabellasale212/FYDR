# Design brief — STAFF-COACH-11, Remove a session, and restore it
...
**Read the STAFF-SHELL brief and the STAFF-SS-11 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-11 brief is the
...
1. **The draft path is verified as the coach at 375**: "Remove this draft? It was never published, so there is nothing to undo." with both buttons at 44px; the draft vanishes, no ghost, the banner returns to "up to date". The copy split (`b2062d3`) stands.
2. **The published-session path keeps the ghost and Restore** (SS-11, verified 2026-09-11); a proposal keeps the ghost in place, struck through, with Restore on the panel.
...
1. **Where the confirmation appears** — the same panel placement question as 10; on a phone the confirmation for a block tapped at 1,480px renders at 3,364px.
2. **Nothing else coach-specific.**
