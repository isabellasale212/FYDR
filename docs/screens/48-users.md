...
| Deactivate / Reactivate | The user's record | Changes the account's status. **Held at the database since 13 September 2026 (migration 0109, §0bd):** a person cannot change their own status (the screen's refusal was only in the browser before); the club's only active sport scientist cannot be deactivated by anyone; and any status change bumps `claims_version`, so the account is signed out on its next request. The screen says each refusal in words (`STATUS_REFUSALS`). **Deactivate is the revoke** (PATTERN-S8 D5, decision batch B1, built 13 September 2026): the write goes through `POST /settings/users/[id]/status`, which changes the row and then bans the sign-in itself (`auth.admin.updateUserById`, `ban_duration`), so an outstanding invite or magic link is refused at the token from the same moment — before this, a deactivated account's unfollowed invite still signed the person in and the access-token hook handed them an empty token. There is no separate Revoke control. Reactivating lifts the ban; an invite never followed works again until it expires. If the ban itself fails the response says the account is deactivated in Fydr but the link may still work, and the audit row (`user.invites_revoked`, `sign_in_blocked_at_auth`) records which happened | Stays | `users.status`, the auth ban, two audit entries | Sport scientist, on another account | The typed confirmation | — |
