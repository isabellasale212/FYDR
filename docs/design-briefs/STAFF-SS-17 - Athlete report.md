# Design brief — STAFF-SS-17, Athlete report
...
1. **`/reports/athlete` is a "Pick an athlete" roster** whose `h1` is an
   instruction — the only one in the staff app. Decide whether it is a screen
   or a picker.
2. **Four sections at 3,100px on a phone** for one athlete.
