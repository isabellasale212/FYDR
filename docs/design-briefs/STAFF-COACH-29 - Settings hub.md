# Design brief — STAFF-COACH-29, Settings hub
...
screen is identical to the sport scientist's by measurement, the SS-29 brief is
...
**Full review: `docs/walkthrough-reviews/staff-coach-29-to-33-review.md`.**
...
1. **The coach's hub is seven sections and six list rows** — Plan (read-only), Integrations, Profile, Photo, Edit profile, Password and sign-in, Two-factor; Thresholds, Password and two-factor, Exports, Groups, Notifications, Log out. Absent rows stay absent (the admin pattern), never locked.
2. **The Integrations links go nowhere for the coach** (§0av) — a proposal renders the state as text for roles without `GPS_IMPORT`.
3. **The 2FA prompt stays** (O-323).
...
1. **A hub for a role with three things in it** — thresholds, groups, notifications — on a 2,804px page; whether the list leads for the coach.
