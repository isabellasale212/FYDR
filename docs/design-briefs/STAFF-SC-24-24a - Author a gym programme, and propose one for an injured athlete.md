# Design brief — STAFF-SC-24-24a, Author a gym programme, and propose one for an injured athlete

**For Claude Design.** Everything below is the current state, measured on the
running application on 2026-09-12 at **1280×900 and 375×812** as the S&C (Owen
Hartnell, one role). Nothing here is aspirational.

**Read the STAFF-SHELL brief and the STAFF-SS-24 brief first.** The phone shell
...
design surface; this brief carries what differs for the S&C — and what the sport scientist's review
could not see.
...
*From the S&C section (identical table):*
...
**Full review: `docs/walkthrough-reviews/staff-sc-01-to-33-review.md`.**
...
1. **Standard programmes are the S&C's to author and assign** (`PROGRAMME_EDIT`); **rehab programmes are read-only** for them (`REHAB_PROGRAMME`); `/programmes/new` is gym-only. A proposal keeps the label as the tell on the list and the read-only shape on a rehab programme.
2. **The proposal note stays where it is** — before the act, inside "+ Assign", the moment an injured athlete is chosen: "This athlete has an open injury, so this goes to the medic as a proposal…" and the button reads "Propose". It is the best permission copy in the app.
...
1. **What the S&C sees after proposing** — PATTERN-S5 (Programme authoring) and PATTERN-S3 C6 (proposal states: Proposed / Approved / Returned with a required return reason) are the design surface; this brief points at them.
2. **The sport scientist's bypass** (assigns an injured athlete directly, no sign-off) is a role-model question for Isabella, not a design matter.
