# Design brief — STAFF-NUT-07, Schedule — read-only, and it says so
...
1. **Read-only stays stated in place** ("Read only. The schedule is authored by the sport scientist and the coach.") — the right pattern; no segmented control, no toolbar, no banner actions.
...
1. **Nothing nutritionist-specific**; the SS-07 brief is the design surface. Whether a role that can only read needs the week grid or the timetable is the persona question.
