# Design brief — STAFF-COACH-10, Edit a session in the grid
...
**Read the STAFF-SHELL brief and the STAFF-SS-10 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-10 brief is the
...
*The flow itself, as the sport scientist section states it (the coach's is identical by measurement; corrections from this pass applied):*
...
**Full review: `docs/walkthrough-reviews/staff-coach-08-to-11-review.md`.**
...
1. **Identical to STAFF-SS-10 by driven comparison** at 1280 (same panel facts, steppers, actions). The SS-10 brief is the design surface.
2. **The Expects line is computed** ("RPE due from 11:30", `5f68cb6`) and must stay computed.
3. **Past sessions offer Duplicate only** — right, and on a Saturday evening it is every block; a proposal keeps Duplicate reachable on a past block.
4. **The panel is not a right-hand rail today** (1,644px below the grid at 1280; 3,364px at 375) — a proposal may move it, and if it does, both widths move together.
...
1. **Where the selected-session panel lives** (§0aw): below the grid, off-screen at both widths after a click. Pitch-side at 375 the coach adjusts one session; the steppers and the preview should be where the block was tapped.
2. **The title bar's group label** goes stale after a chip press (§0aw, shell) and **"Back" sits on a root screen** (§0aw, shell) — both visible on this screen first.
3. **Edit mode at 375** — the Read/Edit segment, chips and arrows now clear 44 (`6a2f1d4`); what the grid itself does at 375 (seven columns in 343px) is the open layout question the SS-07 brief already carries.
