# Design brief — STAFF-SS-33, Print a screen
...
1. **`window.print()` through `base.css`'s print block** — there is no separate print view to keep in sync, and a proposal does not add one.
2. **The print block hides every `<form>` and every button** — anything a proposal puts inside a `<form>` on a printable screen disappears from the printout; state it.
3. **The board's "Print PDF" is a different control** (a PDF route with the redistribution footer) and stays one.
...
1. **Which screens carry Print** — dashboard, injuries, the testing report and the testing detail today; the proposal says whether the other reports (SS-17 to -22) should, and where the button sits at 375.
2. **The printout itself** — the injuries board has its own 9pt row rule; the dashboard and the testing report print whatever is on screen. A proposal may draw the printed page for one of them.
