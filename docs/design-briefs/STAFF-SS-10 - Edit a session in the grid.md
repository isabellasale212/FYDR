# Design brief — STAFF-SS-10, Edit a session in the grid
...
**Full review: `docs/walkthrough-reviews/staff-ss-09-to-12-review.md`** — covers
...
1. **"Held on your screen. Publish to athletes, at the top of this page, puts it
   on their phones."** stays, verbatim in substance, on every pending change.
2. **The three-step wizard's staff-only explanation stays** — "Nobody selected
   means staff only — no athlete will see this in their app."
3. **The athlete preview must be true.** "RPE due by …" is computed from
   `rpeDueAt`, not a fixed string (§0aj — build work, out of the brief's scope,
   but a proposal must not design around the placeholder).
4. **Past published sessions stay read-only** in the panel.

## 5. What a proposal should address

1. **The steppers as icons at 40px** — whether Earlier/Later/Shorter/Longer
   should be visible words, and the size in any case (§0aj).
2. **Two panels in one rail** — the wizard and the selected-session panel look
   different and are reached differently; a proposal should say whether that
   is intended.
3. **Unlabelled wizard inputs** (§0aj — build work).
