# Design brief — STAFF-COACH-13, Publish the week to athletes
...
**Read the STAFF-SHELL brief and the STAFF-SS-13 brief first.** The phone shell
is built (`af09c17` + `6a2f1d4`); this flow is measured on it. The coach's screen
is identical to the sport scientist's by measurement — the SS-13 brief is the
...
**Full review: `docs/walkthrough-reviews/staff-coach-12-to-15-review.md`.**
...
1. **Identical to STAFF-SS-13**; the banner text renders for every role, the actions only with `SESSION_EDIT` — a proposal keeps the split.
2. **The pending subtitle stays** ("Nothing changes on their phone until you publish.").
3. **Publish never refreshes away a failure** (`71035f5`); a proposal shows the "Not published:" state in place.
...
1. **The banner at 644px on a phone** — the coach's only confirmation after any edit, off-screen after the wizard closes and while editing a block below the fold; whether it becomes sticky, or a count on the bar, or moves with the panel.
2. **The Sunday-night case:** one glance to know whether next week is on athletes' phones. What that glance is.
