# Design brief — STAFF-COACH-16, Open the squad report and export it
...
1. **Identical to STAFF-SS-16 by fingerprint**, both widths; the SS-16 brief is the design surface.
2. **The export carries the period on screen** and is audited against the coach's name; a proposal keeps both visible before the press.
...
1. **`/reports/squad` at 375 scrolls sideways (437px)** and its attention-list name links are 15px with no padding (§0ax) — the Monday-morning screen on a phone.
2. **4,202px at phone** — what the coach reads first.
