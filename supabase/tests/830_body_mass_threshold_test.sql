...
--      (body_composition) when there is one, else the athlete's own check-in
--      figure (wellness_entries.body_mass_kg); a check-in-only athlete fires.
...
-- Merge: ten daily check-ins at 80.0 kg for the baseline; on D both a staff weigh-in
-- (78.0) and a check-in (79.5). The staff figure is the day's figure.
...
select is(
  ...
  78.000::numeric(10,3),
  'with both a staff weigh-in and a check-in figure on the day, the weigh-in is the day''s figure'
);
...
select is(
  ...
  78.000::numeric(10,3),
  'an athlete who only ever types a figure on the check-in is still evaluated'
);
...
select is(
  (select count(*)::int from public.flags where metric = 'body.mass_kg'),
  3,
  'the nutritionist reads all three body-mass flags'
);
