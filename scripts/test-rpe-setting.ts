...
console.log('\n4. every dependent surface says so');
{
  const surfaces: [string, RegExp, string][] = [
    ...
    ['src/app/(staff)/analytics/page.tsx', /!collectsRpe && isRpeAnalyticsMetric\(metric\.key\)/, 'the analytics presets'],
    ['src/app/(staff)/analytics/build/page.tsx', /!collectsRpe && isRpeAnalyticsMetric\(metric\.key\)/, 'the analytics builder'],
