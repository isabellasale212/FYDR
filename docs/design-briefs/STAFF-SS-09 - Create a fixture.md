# Design brief — STAFF-SS-09, Create a fixture
...
1. **The refusal keeps moving focus to the field** — "Name the opponent." on a
   long form, with the button at the bottom, is only useful if the eye is taken
   to the problem. The document records why; keep it.
2. **The "names nobody" note stays on the form.** A fixture reaches every
   athlete's Today the moment it is created; staff must read that before
   pressing.
3. **"Home" and "Normal" stay the defaults.**

## 5. What a proposal should address

1. **Nothing.** Verified on every claim. Recorded so it is not redesigned for
   its own sake.
