...
| 2.8 | Dashboard (the medic) | — | a **Current injuries** card at the top of the dashboard, below the lead card and above the tiles (Isabella's placement, after the walkthrough): every athlete with an open injury who is expected at one of the day's sessions (`fetchInjuredInDay`), the site as the medic reads it, when, and the session — additive, the medic's alone (`CLINICAL_ONLY`) | unchanged |
