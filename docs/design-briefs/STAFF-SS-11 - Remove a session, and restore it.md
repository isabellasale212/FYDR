# Design brief — STAFF-SS-11, Remove a session, and restore it
...
1. **Confirm-and-Never-mind stays** — removal is the one destructive act in the
   grid and it is correctly guarded.
2. **The ghost stays for published sessions** — a removed session must stay
   visible, struck through, in place, until Publish, so the person who removed
   it can see what they did.
3. **Draft removal is final** and the copy must say so, not promise an undo
   (§0aj).

## 5. What a proposal should address

1. **"Yes, remove" at 37px** — the destructive control is the smallest one in
   the panel (§0aj).
2. **The ghost state was not reachable on scratch** — a proposal should not
   assume its appearance from the document alone; the capture PDF for SS-11
   shows it.
