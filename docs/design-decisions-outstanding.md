...
| ATH-ADULT-12 | C5 | The gym hero becomes "Back squat best 102.5 kg, up 5 kg this block". | 09 C6's MET-040 read exists now (`fetchPersonalBestsBefore`); dated programme blocks are the remaining piece. Carry into it: the hero headlines "0 sets logged · last 4 weeks" over an empty period — a zero as a headline, and a four-week window that ignores the period (found building 12 C6; PATTERN-S6's "never a zero"). | Next on My data · medium |
...
| ATH-ADULT-13 | C2 | Tapping a set row on the session detail opens the logger's correction; the footer becomes Save correction / Cancel; the per-row "Correct" buttons go. | The logger's correction is now the chip-and-footer shape (09 C1 / 11 C1, 2026-09-12); the detail page still has its inline table panel. Recommend: the detail links each row to the logger with the set to correct (`/gym/[sessionId]?correct=<setId>`) so the panel is one component. | Next on the gym history · medium |
