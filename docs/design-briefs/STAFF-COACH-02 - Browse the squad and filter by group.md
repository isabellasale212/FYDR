# Design brief — STAFF-COACH-02, Browse the squad and filter by group
...
**Read the STAFF-SHELL brief and the STAFF-SS-02 brief first.** The phone shell
...
is identical to the sport scientist's by fingerprint — the SS-02 brief is the
...
1. **Identical to STAFF-SS-02 minus "Add athlete"** — the control is absent, not disabled, for a role without `SETTINGS_ADMIN`; keep it absent.
2. **The athlete-name links are the coach's most-tapped control on a phone** — they measure 43px on the built shell (§0au); the proposal must not shrink them further.
3. **`/squad` still scrolls sideways at 375** (§0ap, the roster table) — a proposal draws the roster at 375 without the page scrolling.
...
1. **The pitch-side roster** — a coach on a phone before a session wants availability status per name at a glance; the current row carries name · position · status; whether that survives 375 without the sideways scroll is the question.
2. **`/squad/new` bounces silently** for the coach (§0av) — a proposal need not draw it; noted so the flow is not designed with an Add control.
